...
parser = argparse.ArgumentParser(description="Создать справочный документ по TVM-инструкциям")
...
parser.add_argument("out_file", type=str, help="выходной файл")
args = parser.parse_args()

TABLE_HEADER = \
        "| xxxxxxx<br>Opcode " +\
        "| xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx<br>Fift syntax " +\
        "| xxxxxxxxxxxxxxxxxxxxxxxxx<br>Stack " +\
...
def make_table(cat):
    if cat not in categories:
        print("Нет такой категории", cat, file=sys.stderr)
        return ""
    table = [TABLE_HEADER]
