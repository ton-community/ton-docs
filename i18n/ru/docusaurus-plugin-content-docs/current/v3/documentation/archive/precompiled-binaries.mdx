...
```mdx-code-block
<Button href="/v3/documentation/smart-contracts/getting-started/javascript" colorType="primary" sizeType={'sm'}>Migrate to Blueprint SDK</Button>
```
...
5. To add a new value `(path)` to the system variable in the next window, click the  button `New`.
...
8. Download [fiftlib.zip](/ton-binaries/windows/fiftlib.zip)

9. Open the zip in some directory on your machine (like **`C:/Users/%USERNAME%/ton/lib/fiftlib`**)

10. Create a new (click button `"New"`) environment variable `FIFTPATH` in "_User variables_" section.

11. In the "_Variable value_" field, specify the path to the files: **`/%USERNAME%/ton/lib/fiftlib`** and click `OK`. Done.
