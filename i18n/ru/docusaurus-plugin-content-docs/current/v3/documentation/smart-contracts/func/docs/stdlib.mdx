...
То есть, если `hash` вычисляется как хеш некоторых данных, эти данные хешируются *дважды*, второе хеширование происходит внутри `CHKSIGNS`.
...
4. **+2 flag** is important in external messages (e.g., to wallets).
   :::
...
* `addr_extern` is represented by `t = (1, s)`, where slice `s` contains the
  field `external_address`. Другими словами, `t` - это пара (кортеж, состоящий из двух записей), содержащая целое число, равное единице, и срез `s`
...
A primitive *preloads* data when it returns only the extracted data, leaving the original slice unchanged. Его можно использовать как [немодифицирующий метод](/v3/documentation/smart-contracts/func/docs/statements#non-modifying-methods).
...
Говорят, что примитив *сохраняет* значение `x` в конструкторе `b`, если он возвращает измененную версию конструктора `b'` со значением `x`, сохраненным в конце. It can also be used as a [non-modifying method](/v3/documentation/smart-contracts/func/docs/statements#non-modifying-methods).
