...
4. Эффективность потребления газа, заложенная в основу: контракты могут стоить до **50% дешевле**, чем их эквиваленты на FunC, благодаря оптимизации компилятора и **ленивой** функциональности.
...
1. Покопайтесь в [документации смарт-контрактов](/v3/documentation/smart-contracts/overview), чтобы получить базовые знания о разработке в TON.
2. [Настройте](https://ton.org/v3/documentation/smart-contracts/tolk/environment-setup.mdx) свой первый проект счётчика на Tolk с помощью команды
...
5. Продолжайте изучать документацию. Когда столкнётесь с кодом на FunC или Tact, сравните с эквивалентом на Tolk — зачастую он покажется вам чище и декларативнее.
...
1. С помощью blueprint создайте новый контракт Tolk (например, счётчик с помощью команды `npm create ton@latest`) и поэкспериментируйте. Вы быстро заметите, как стековая логика меняется на выразительные конструкции со структурированными типами, объединениями (unions), сопоставлением с образцом, `toCell()` и многим другим.
...
3. Прочитайте [Сравнение Tolk и FunC: коротко](/v3/documentation/smart-contracts/tolk/tolk-vs-func/in-short).
4. Попробуйте [конвертер FunC-to-Tolk](https://github.com/ton-blockchain/convert-func-to-tolk) для постепенной миграции существующих кодовых баз.
...
Доступен целый набор инструментов для поддержки разработки с Tolk — включая поддержку в IDE, языковые сервисы, и утилиты миграции.
...
<table style={{ width: "100%", tableLayout: "fixed" }}>
  ...
  <tbody>
    ...
    <tr>
      [Конвертер из FunC в Tolk](https://github.com/ton-blockchain/convert-func-to-tolk) — конвертирует файл `.fc` в файл `.tolk` с помощью одной команды `npx`.
    </tr>
...
Сам компилятор Tolk размещён в [репозитории](https://github.com/ton-blockchain/ton) `ton-blockchain`.
...
Компилятор новый, поэтому ещё возможны некоторые крайние случаи. Однако в целом Tolk уже служит **заменой FunC в статусе production-ready**, подходящей для реального использования.
