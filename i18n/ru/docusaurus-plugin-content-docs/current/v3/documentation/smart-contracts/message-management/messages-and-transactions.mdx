...
Предположим, что существует два контракта – _A_ и _B_. _A_ получает внешнее сообщение, которое запускает отправку двух внутренних сообщений контракту _B_, назовем их сообщения _1_ и _2_. Let's call these messages _1_ and _2_. В этом простом случае мы можем на 100% быть уверены, что сообщение _1_ будет обработано _B_ раньше, чем _2_ поскольку оно имеет меньшее _lt_.
...
Consider three contracts - _A_, _B_, and _C_. Предположим, что существует два контракта – _A_ и _B_. _A_ получает внешнее сообщение, которое запускает отправку двух внутренних сообщений контракту _B_, назовем их сообщения _1_ и _2_. However, their processing order at the destinations isn't guaranteed because:
