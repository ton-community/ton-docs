...
| :--------------------------- | :------------------------------------------------------------------------- | :-------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| No args                      | `update`                                                                   | `update`                                                              | Обновление из текущего репозитория                                          |
...
| Формат имени                 | Формат                                                                      | Пример                                                              | Описание                                                                    |
| :--------------------------- | :-------------------------------------------------------------------------- | :------------------------------------------------------------------ | --------------------------------------------------------------------------- |
| No args                      | `upgrade`                                                                   | `upgrade`                                                           | Обновление из текущего репозитория                                          |
...
| Формат имени | Формат        | Пример        | Описание                                                                                                           |
| ------------ | ------------- | ------------- | ------------------------------------------------------------------------------------------------------------------ |
| No args      | `status`      | `status`      | Полный отчет о состоянии, включая эффективность валидатора и онлайн-валидаторы                                     |
...
2. **Validator index in the range [`max_main_validators`, `max_validators`)**:
   The `validator efficiency` should still be above 90% for the full round (this percentage may be adjusted in the future based on statistics). В настоящее время штраф не применяется, но это может измениться в будущих обновлениях.

3. **The user is not a validator**:
   No penalties apply, but the user is also not eligible for rewards. Нет `validator efficiency` для проверки. Это может быть вызвано низким стейком или неправильной конфигурацией узла. Кроме того, убедитесь, что `mytonctrl` работает без перебоев.
...
| `archive-ttl`      | ttl блоков, которые хранит узел. *Чтобы заставить узел не хранить архивные блоки, используйте значение `86400`* | 2592000, если режим liteserver был включен во время установки, в противном случае 86400 |
