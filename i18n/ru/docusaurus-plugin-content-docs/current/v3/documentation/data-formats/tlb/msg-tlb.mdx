...
Сериализованное `Message X` помещается в список действий с помощью метода FunC send_raw_message(), затем смарт-контракт выполняет это действие и отправляет сообщение. The smart contract then executes this action, and the message is sent.
...
Each nested structure is defined in a separate TL-B scheme, which must be referenced and resolved recursively. Каждую вложенную структуру мы должны получать из другой схемы TL-B по ссылке рекуррентно, пока сериализация для верхней структуры не станет явной - каждый бит будет определен булевым или битовым типом (bits, uint, varuint).

Структуры, которые в настоящее время не используются в обычной разработке, будут отмечены `*` в столбце Тип, например \*Anycast обычно пропускается при сериализации. For example, `*Anycast` is typically omitted during serialization.
...
| init      | [StateInit](#stateinit-tl-b)    | Optional    | Общая структура, используемая в TON для инициализации новых контрактов. Может быть записана как ссылка на ячейку или корневую ячейку.       |   |
...
| ihr_disabled   | Bool                                      | Обязательно | Flag indicating whether hypercube routing (IHR) is disabled.                                                                                                           |
...
| ihr_fee        | [VarUInteger 16](#varuinteger-n)          | Обязательно | Fee for delivering the message using hypercube routing.                                                                                                                |
| fwd_fee        | [VarUInteger 16](#varuinteger-n)          | Обязательно | Validators set the fee for forwarding the message.                                                                                                                     |
...
| Структура   | Тип                      | Обязательно | Описание                                                                                                                                                                                                                                                                                                                                                                      |
| ----------- | ------------------------ | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| split_depth | (## 5)                   | Optional    | Параметр для highload контрактов определяет поведение разделения на несколько экземпляров в разных шардах. В настоящее время StateInit используется без него.                                                                                                                                                                                                                 |
| special     | TickTock\*               | Optional    | Используется для вызова смарт-контрактов в каждом новом блоке блокчейна. Доступно только в мастерчейне. Обычные пользовательские контракты используются без него.                                                                                                                                                                                                             |
| code        | Cell                     | Optional    | Сериализованный код контракта.                                                                                                                                                                                                                                                                                                                                                |
| data        | Cell                     | Optional    | Начальные данные контракта.                                                                                                                                                                                                                                                                                                                                                   |
| library     | HashmapE 256 SimpleLib\* | Optional    | В настоящее время используется StateInit без библиотек                                                                                                                                                                                                                                                                                                                        |
...
`MsgAddress` - это схема различных сериализаций для адресов. The structure depends on the participant, either off-chain or a smart contract, sending the message. В зависимости от того, какой участник (off-chain или смарт-контракт) отправляет сообщения, используются различные структуры.
...
| anycast      | Anycast\*    | Optional    | Represents additional address data. Дополнительные данные адреса, в настоящее время не используются в обычных внутренних сообщениях                                                                                                                                                                      |
...
| anycast      | Anycast\*    | Optional     | Дополнительные данные адреса, в настоящее время не используются в обычных внутренних сообщениях                                                       |
...
| other        | ExtraCurrencyCollection | Optional    | ExtraCurrencyCollection — это словарь, предназначенный для дополнительных валют, который обычно пустой         |
...
| value      | (uint (len \* 8)) | Optional    | значение uint для целого числа, записанного в (len \* 8) бит                                                                   |
