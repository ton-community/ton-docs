...
Нам нужно сравнить Hash_1 этой ссылки доказательства Меркла только с хешем, который мы получили из функции `check_block_header` выше, чтобы мы могли доверять этой ячейке: If they match, the cell can be trusted:
