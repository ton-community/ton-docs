...
Теперь мы можем доверять `state_cell`, который выглядит так: Its structure is as follows:
...
Нам нужно запомнить `last_trans_hash` и `last_trans_lt`, потому что мы можем использовать их для получения транзакций по аккаунту, и давайте проверим всю ячейку этих данных: Let's examine the entire cell containing this data.
