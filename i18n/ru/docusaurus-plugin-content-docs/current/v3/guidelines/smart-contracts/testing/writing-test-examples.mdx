...
В этом разделе будут приведены тестовые случаи для [кодов](/v3/documentation/tvm/exit-codes) выхода TVM, которые могут возникнуть во время обработки транзакции. Эти коды выхода находятся в самом коде блокчейна. При этом необходимо различать код выхода во время [Compute Phase](/v3/documentation/tvm/overview#compute-phase) и код выхода во время Action Phase.
...
TVM - это [стековая машина](/v3/documentation/tvm/overview). При взаимодействии с различными значениями они появляются в стеке. Если вдруг в стеке нет элементов, но какой-то опкод их требует, то будет выброшена эта ошибка.
...
Все данные в TON хранятся в ячейках ([cells](/v3/documentation/data-formats/cells/overview)). Ячейка может хранить 1023 бита данных и 4 ссылки на другие ячейки. Если Вы попытаетесь записать более 1023 бит или более 4 ссылок, будет выброшен код выхода 8.
...
Action Phase начинается после Compute Phase и обрабатывает действия, которые были записаны в [регистр c5](/v3/documentation/tvm/initialization#control-register-c5) во время Compute Phase. Если данные в этот регистр записаны неверно, то будет выброшен код выхода 32.
...
Во время создания части сообщения [CommonMsgInfo](/v3/guidelines/smart-contracts/howto/wallet#commonmsginfo) Вы должны указать правильный адрес источника. Он должен быть равен либо [addr_none](/v3/documentation/data-formats/layout/messages#addr_none00), либо адресу аккаунта, который отправляет сообщение.
...
То же самое, что и в коде выхода `37`, но относится к отсутствию [extra currency](/v3/documentation/dapps/assets/extra-currencies) на балансе.
...
Библиотека - это ячейка, которая хранится в [Мастерчейне](//v3/guidelines/dapps/transactions/foundations-of-blockchain#мастерчейн-и-бейсчейн) и может использоваться всеми смарт-контрактами, если она [публичная](https://github.com/ton-blockchain/ton/blob/9728bc65b75defe4f9dcaaea0f62a22f198abe96/crypto/block/transaction.cpp#L1844).
