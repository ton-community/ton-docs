...
2. Проверьте статус режима *single-nominator*:
...
4. Запустите тест, чтобы убедиться, что все настроено правильно:

```bash
$ npm run test
```

5. Замените скрипты MyTonCtrl `nominator-pool`: https://raw.githubusercontent.com/orbs-network/single-nominator/main/mytonctrl-scripts/install-pool-scripts.sh.
...
3. Создайте адрес с которого будет происходить развертывание:
...
4. Пополните этот адрес на 2.1 TON.
...
7. Выполните бэкап приватного ключа диплоера `./build/deploy.config.json` и `single-nominator.addr` файлов.
8. Скопируйте `single-nominator.addr` в `mytoncore/pools/single-nominator-1.addr`.
9. Отправьте ставку с адреса владельца на адрес single nominator.
...
4. После перехода в кошелек введите сумму TON и отправьте транзакцию, таким образом средства будут добавлены в стейкинг. The TON coins will then be added to staking.
...
| Contracts               | Интерфейс для управления                              |
| ----------------------- | ----------------------------------------------------- |
...
7. После того, как `single_nominator_pool` будет внесен в whitelist, вы сможете отправлять заблокированные токены из `vesting`-контракта в `single_nominator_pool` с помощью [vesting.ton.org](https://vesting.ton.org/):
   * c. с помощью интерфейса [vesting.ton.org](https://vesting.ton.org/) верните остаток средств (~ 8 TON) обратно в `vesting` с сообщением `amount 0, comment w`.
   * d. с помощью интерфейса [vesting.ton.org](https://vesting.ton.org/) верните остаток средств (~ 8 TON) обратно в `vesting` с сообщением `amount 0, comment w`.
