...
2. Создайте папку проекта и файл `go.mod` со следующим содержанием (для выполнения этого процесса может потребоваться изменить **версию Go**, если текущая используемая версия устарела): If the current version of Go is outdated, update it to the required version to proceed with this process:
