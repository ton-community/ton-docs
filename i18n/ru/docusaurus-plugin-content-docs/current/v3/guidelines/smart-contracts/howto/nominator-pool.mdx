...
1. Настройте аппаратное обеспечение валидатора, для этого потребуется: 8 vCPUs, 128 ГБ RAM, 1 ТБ SSD, фиксированный IP-адрес и скорость канала 1 Гб/с.
   ...
   >
   > Обратите внимание, что с мая 2021 года компания Hetzner запретила майнинг на своих серверах, данный запрет распространяется как на алгоритмы PoW, так и на PoS. This ban includes both PoW and PoS algorithms. Даже установка обычного узла может быть расценена как нарушение условий обслуживания.
   >
...
3. Переведите 1 TON на адрес кошелька валидатора.
...
6. Создайте два пула (для четных и нечетных раундов проверки):
   ...
   > (!) Pool configurations do not have to be identical. Конфигурации пулов не обязательно должны быть одинаковыми, можно добавить 1 к минимальной ставке одного пула, чтобы сделать их разными.
   >
