...
> Во-вторых, существует проблема "загрязнения пространства имён".\
> Давайте разберёмся, в чем она заключается, на другом примере из аудита. В середине функции входной параметр читается:  
>
> ```func
...
Согласно нашей диаграмме потока сообщений, мы можем оценить стоимость каждого обработчика в каждом из сценариев и вставить проверку на достаточность msg_value. Avoid demanding excessive gas, as it must be divided among subsequent messages. Все усложняется, если во время разработки ваш код начинает отправлять больше сообщений. Требования к газу необходимо перепроверять и обновлять.
