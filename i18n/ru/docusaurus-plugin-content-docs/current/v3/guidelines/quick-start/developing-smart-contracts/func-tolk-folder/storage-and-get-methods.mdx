...
:::tip
Если вы застрянете на любом из примеров, вы можете найти [здесь](https://github.com/ton-community/onboarding-sandbox/tree/main/quick-start/smart-contracts/Example/contracts) исходный шаблон проекта со всеми изменениями, сделанными в этом руководстве.
:::

Почти всем смарт-контрактам требуется хранить свои `данные` между транзакциями. В этом руководстве объясняются стандартные способы управления `хранилищем` для смарт-контрактов и использование `get-методов` для доступа к нему извне блокчейна.
...
Давайте изучим структуру [ячейки](/v3/concepts/dive-into-ton/ton-blockchain/cells-as-data-storage), чтобы понимать, как управлять хранилищем контракта:

## Структура ячейки

Блокчейн TON использует структуру данных под названием **«Ячейка»** в качестве фундаментальной единицы хранения данных. Ячейки являются «строительными блоками» данных смарт-контрактов и обладают следующими характеристиками:
...
- После своего создания ячейка становится неизменяемой.
...
Давайте изменим наш смарт-контракт в соответствии со стандартными шагами, описанными в разделе [Обзор Blueprint](/v3/guidelines/quick-start/developing-smart-contracts/func-tolk-folder/blueprint-sdk-overview/).

### Шаг 1: редактирование кода смарт-контракта

Если ручная сериализация и десериализация ячейки хранения становятся неудобны, стандартная практика заключается в создании двух методов-«обёрток», которые обрабатывают эту логику. Если вы не меняли код смарт-контракта, то в файле `/contracts/hello_world.fc` должны быть следующие строки:
...
Давайте немного изменим пример, применив другой популярный подход к управлению хранением данных при разработке смарт-контрактов:
...
Также давайте добавим в наше хранилище ещё и **256-битное целое число** как глобальную переменную `ctx_counter_ext`. Изменённая реализация должна выглядеть следующим образом:
...
Главная цель `get-методов` — предоставить внешний доступ для чтения данных хранилища через удобный интерфейс. В первую очередь для извлечения информации, необходимой для подготовки **транзакций**.
...
И не забудьте обновить переменные в уже существующих get-методах, чтобы они совпадали с распаковкой из `load_data()`.
...
И это всё! На практике все **get-методы** следуют этому прямолинейному паттерну и не требуют дополнительной сложности. Помните, что вы можете проигнорировать возвращаемые значения, используя синтаксис заглушки `_`.
...
Это означает, что HelloWorld был успешно скомпилирован. Был создан хеш, а скомпилированный код был сохранён в файле `build/HelloWorld.compiled.json`.
...
Затем реализуйте метод для вызова нового get-метода `get_counters`, который получает значения обоих счётчиков в одном запросе:
...
2. Исполните каждый `get-метод` для извлечения сохранённых данных.
3. Проверьте, что значения соответствуют изначальной конфигурации.
...
Теперь вы можете запустить обновлённый тестовый скрипт с помощью этой команды:
...
Вы написали свой первый смарт-контракт с помощью Tolk или FunC, протестировали его и изучили, как работают хранение данных и get-методы.
