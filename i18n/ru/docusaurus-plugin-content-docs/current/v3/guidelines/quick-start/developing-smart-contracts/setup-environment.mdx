...
В этом руководстве мы используем [Blueprint](https://github.com/ton-org/blueprint/) и стек [Node.js](https://nodejs.org/en)/TypeScript для написания обёрток, тестов и скриптов развёртывания вашего смарт-контракта. Этот позволяет получить самую простую и готовую к использованию среду для разработки смарт-контрактов.

:::info
Using native tools and language-dependent SDKs for smart contract development is covered in more advanced sections here:

- [Компилируйте и собирайте смарт-контракты в TON](/v3/documentation/archive/compile#ton-compiler).
- [Создайте исходное состояние для развёртывания](/v3/guidelines/smart-contracts/howto/wallet#creating-the-state-init-for-deployment).
  :::
...
В этом гайде мы приводим примеры на языках `Tolk`, `FunC` и `Tact`. Вы можете выбрать любой из них и даже комбинировать смарт-контракты на разных языках. Для дальнейшего изучения этого гайда не требуется глубокое понимание выбранного языка — достаточно будет базовых программистских знаний.

:::info
Краткий обзор языков доступен в разделе [Языки программирования](/v3/documentation/smart-contracts/overview#языки-программирования).
:::
...
3. **Project template** (шаблон проекта): **simple counter contract** на языке `FunC`, `Tolk` или `Tact`

Наконец, перейдите в директорию, которая была создана при генерации шаблона, и установите все необходимые зависимости:
...
Сообщество TON разработало плагины для поддержки синтаксиса языков в различных IDE и редакторах кода. Вы можете найти их на странице [Плагины для IDE](https://docs. ton. org/v3/documentation/smart-contracts/getting-started/ide-plugins).
...
- **Tolk или FunC**: предполагаемое время обучения 25 минут для каждого.
