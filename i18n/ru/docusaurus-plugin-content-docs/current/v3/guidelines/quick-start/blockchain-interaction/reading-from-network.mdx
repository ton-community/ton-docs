...
В итоге вы поймёте, как взаимодействовать с [TON API на основе HTTP](/v3/guidelines/dapps/apis-sdks/api-types#http-api). В этом руководстве используется [TON Center](https://toncenter.com/) — быстрый и надёжный HTTP API для TON.
...
- `data`: Сериализованные данные контракта, хранящиеся в [*ячейке*](/v3/concepts/dive-into-ton/ton-blockchain/cells).
