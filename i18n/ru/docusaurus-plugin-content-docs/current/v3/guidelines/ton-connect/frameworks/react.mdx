...
После установки пакета вам следует создать файл `manifest.json` для вашего приложения. Более подробную информацию о том, как создать файл manifest.json, можно найти [здесь](/v3/guidelines/ton-connect/creating-manifest).
...
Используйте этот хук для извлечения текущего кошелька TON пользователя. Если кошелек не подключен, хук вернет `null`. Объект `wallet` предоставляет общие данные, такие как адрес пользователя, провайдер, [TON proof](/v3/guidelines/ton-connect/verifying-signed-in-users) и другие атрибуты (см. интерфейс [Wallet](https://ton-connect.github.io/sdk/interfaces/_tonconnect_sdk.Wallet.html)).
...
- Больше примеров здесь: [Отправка сообщений](/v3/documentation/smart-contracts/message-management/sending-messages)
...
:::tip
Узнайте, как подписывать и проверять сообщения: [Подпись и проверка](/v3/guidelines/ton-connect/verifying-signed-in-users)
:::
...
Пример аутентификации можно найти на этой [странице](/v3/guidelines/ton-connect/verifying-signed-in-users#react-example)
