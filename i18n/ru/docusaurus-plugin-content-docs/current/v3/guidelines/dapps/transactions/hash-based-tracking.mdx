...
Рассмотрим, как использовать хеши для отслеживания сообщений и транзакций в блокчейне. Вы узнаете, чем различаются разновидности хешей, как вычислять и применять их для верификации активности в блокчейне.
...
Тело относится к полю `body: (Either X ^X)` в TL-B схеме:
...
Давайте для примера рассмотрим тело сообщения с комментарием «Привет, TON!» и вычислим его хеш.
...
**Когда используют хеш тела сообщения:**
...
**Когда используют хеш полного сообщения:**
...
Если существует входящее сообщение, также выводится его хеш.
...
4. **Тело**: всегда хранится как ссылка
...
- **Оригинальный хеш** — хеш отправленного сообщения. Вы можете использовать его для поиска транзакции в блокчейне. Вы можете использовать его для поиска транзакции в блокчейне.
...
С [TON Connect](/v3/guidelines/ton-connect/guidelines/verifying-signed-in-users#backend-example) часто требуется проверить, что адрес кошелька соответствует его `StateInit`.
Хеш `StateInit` должен соответствовать хеш-части адреса.
...
Мы рассмотрели, как каждому сообщению и транзакции в TON присваивается уникальный криптографический отпечаток — хеш. Эти хеши позволяют нам верифицировать содержимое, обнаруживать дубликаты и отслеживать весь поток выполнения в блокчейне.
...
<Button href="/v3/guidelines/dapps/transactions/api-based-retrieval" colorType={'primary'} sizeType={'sm'}>
  Обработка транзакций с помощью API
</Button>
