...
# Извлечение данных с помощью API

Рассмотрим, как получить данные о транзакциях из блокчейна с помощью различных API. Вы узнаете о доступных API-сервисах в TON и о том, как получать данные по конкретному аккаунту, сообщению, или через полное сканирование блокчейна.
...
TON Center был разработан командой TON Core. Он предлагает два уровня доступа к API:

- **TON Center API V3:** предоставляет индексированные и обогащённые данные блокчейна в удобном формате с поддержкой гибкой фильтрации. Данные проверяются один раз во время индексации.
- **TON Center API V2:** предоставляет данные напрямую с узлов TON. Он поддерживает ограниченную фильтрацию и проверяет данные при каждом запросе.
...
Данные, которые подвергаются минимальной постобработке, часто называют **«сырыми» или «необработанными» данными** (raw data). Этот формат обычно используется в решениях, архитектура которых отдаёт приоритет безопасности и точности, а не лёгкости интеграции или простоте. Эти решения переносят интерпретацию и анализ необработанных данных на сторону клиента, что позволяет повысить контроль и надёжность за счёт дополнительного потребления локальных ресурсов.
...
Таким образом, при чтении блокчейна сервисы итеративно обрабатывают блоки, начиная с последнего обработанного блока и продвигаясь до последнего финализированного. Извлечённые транзакции считаются **необработанными данными** и могут быть переданы следующим сервисам для дальнейшего использования. Кроме того, специализированные сервисы могут анализировать и обогащать эти данные, стремясь достичь баланса между сложностью системы и её бездоверительным (trustless) подходом.
...
Он хорошо работает для переводов wallet-v4 и wallet-v5 и в основном используется для поиска транзакций, инициированных внешними сообщениями.
...
У вас есть выбор: либо простая реализация, требующая высокий уровень доверия к крупных поставщикам данных, либо более сложная, но не требующая такого доверия.
...
| `utime`                   | Нативный                    | Временная метка UNIX для транзакции. Название поля в TL-B: `now`                                             | `1736738146`                                         |
| `transaction_id.hash`     | Постобработанные данные     | Хеш текущей транзакции                                                                                       | `mkma+u5TOi12BSu6m6YhZaatY2b2ozdabOdBnVuWmWM=`       |
| `transaction_id.lt`       | Нативные данные             | Параметр транзакции logical time; уникален в рамках каждого аккаунта. Названия поля в TL-B: `lt`             | `52863169000074`                                     |
...
| `storage_fee`             | Нативные данные             | Комиссии за хранение данных. Название поля TL-B: `Trans Descriptor` -> `TrStoragePhase` ->                   | `1207600`                                            |
...
| `lt`                   | Нативные данные             | Параметр logical time транзакции                                          | `"15371766000003"`                                                     |
