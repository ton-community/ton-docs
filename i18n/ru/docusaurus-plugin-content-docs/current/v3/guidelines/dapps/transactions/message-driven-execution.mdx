...
Здесь мы рассмотрим, как в TON сообщения инициируют транзакции. Вы узнаете, как аккаунты общаются с помощью сообщений, чем различаются типы сообщений, а также как конструировать и отправлять сообщения, чтобы запустить исполнение кода в блокчейне.
...
<details>
  <summary>Жизненные примеры транзакций и сообщений</summary>
  ...
  Люди получают сообщения с просьбой выполнить задание, выполняют его у себя дома, незаметно для других, а затем отправляют результат другим.
  ...
  В этом случае они должны назвать попытку выполнения провалившейся, и, в лучшем случае, вернуть оставшиеся средства вместе с обрывками первоначальных инструкций.

  Если задание выполнено успешно, человек в соответствии с указаниями исходного сообщения:
  ...
  Хотя внешние и внутренние сообщения выполняют схожую роль с точки зрения актора, он может обрабатывать их по-разному. Некоторые акторы могут обрабатывать все, в то время как другие могут полностью игнорировать сообщения из внешнего мира — словно человек, отказывающийся открывать письма от незнакомцев.
</details>

#### Структура сообщения: TL-B

Теперь давайте посмотрим на данные, содержащиеся в сообщениях. В TON различаются внешние и внутренние сообщения, но оба этих типа способны переносить данные.
...
**Внешние входящие** сообщения служат внешнему миру основной точкой входа для взаимодействия с блокчейном TON. Любой пользователь может отправить произвольные данные любому смарт-контракту, и дальше от контракта зависит, как они будут обработаны.
Технически любой аккаунт может получать внешние сообщения. Однако то, как конкретный контракт обрабатывает их, полностью зависит от его внутренней логики. Самый распространённый тип контрактов, готовый иметь дело с такими сообщениями — **контракт кошелька**.
...
Хотя внешние входящие сообщения редко используются в основной логике смарт-контрактов, они необходимы для поддержки внешних действий в блокчейне. Любое взаимодействие, которое исходит извне TON — например, отправка TON или использование DEX — начинается с внешнего входящего сообщения.
...
- **Транзакция** — это результат выполнения смарт-контракта в ответ на входящее сообщение. Во время выполнения контракт может обновить своё состояние и создать одно или несколько исходящих сообщений.

**Внешнее входящее сообщение** — это сообщение, чей заголовок `CommonMsgInfo` использует структуру `ext_in_msg_info$10`.
...
Чтобы отправить внешнее входящее сообщение смарт-контракту, необходимо:
...
Контракт всегда инициирует внутренние сообщения — они являются результатом выполнения транзакции.
...
| `bounce`         | Если значение 1, при ошибке обработки сообщение будет возвращено.                                |
| `bounced`        | Флаг, описывающий, что само сообщение является результатом bounce.                               |
...
В этом примере внешнее сообщение отправляется контракту кошелька с дополнительными инструкциями по отправке внутреннего сообщения. В результате обработки транзакции контракт кошелька отправляет исходящее внутреннее сообщение с указанными значениями `value` и адресом получателя `address`.
...
Большинство внутренних сообщений между смарт-контрактами должны быть возвратными (то есть иметь установленный флаг возврата), чтобы в случае сбоя:
...
- проверять флаг возврата во входящих сообщениях
- и либо игнорировать возвращённые сообщения (завершать исполнение с `code = 0`)
- либо явно обрабатывать ошибку

Вы можете запустить в Blueprint приведённый далее скрипт, чтобы визуально сравнить различия между возвратными и невозвратными сообщениями.

Используйте один из следующих методов для запуска скрипта:
...
Внутренним сообщениям эту необязательную полезную нагрузку включают в поле `body`.
...
TON SDK предоставляет предопределённые примитивы для самых распространённых структур, поэтому сложные обычно кодируются с помощью **встроенных помощников**.
...
### Сериализатор внутреннего сообщения в соответствии с кодом контракта
...
Сначала мы изучим, как контракт обрабатывает операцию передачи, разбирая тело внутреннего сообщения:
...
Теперь давайте посмотрим, как эта функция вызывается во время обработки сообщения:
...
2. Запросите тестнет-версию USDT у [https://t.me/testnet_usdt_giver_bot](https://t.me/testnet_usdt_giver_bot)
...
Это ограничение вызвано тем, что TON Connect **не** предоставляет низкоуровневые возможности подписи по следующим причинам:

1. **Безопасность в основе**: кошельки (например, Tonkeeper) не предоставляют dApp приватные ключи в исходном виде.
...
Этот метод является наименее эффективным и наиболее подверженным ошибкам, но может быть полезен при работе с устаревшими или недокументированными контрактами.

Пример: [Простой контракт](https://verifier.ton.org/EQAyVZ2rDnEDliuaQJ3PJFKiqAS-9fOm9s7DG1y5Ta16zwU2)

## Следующий шаг

Мы только что рассмотрели, как сообщения функционируют в качестве основного механизма взаимодействия в TON — как они структурируются, передаются и используются для инициирования транзакций.
...
<Button href="/v3/guidelines/dapps/transactions/hash-based-tracking" colorType={'primary'} sizeType={'sm'}>
  Отслеживание сообщений и транзакций с помощью хешей
</Button>
