...
Сначала давайте создадим папку `simple-zk/circuits`, а затем создадим в ней файл и добавим в него следующий код: Inside this folder, create a new file and add the following code::
...
Далее мы создадим папку для наших файлов сборки и переместим туда данные, выполнив следующее (находясь в папке `simple-zk`): While inside the `simple-zk` folder, run the following commands:
