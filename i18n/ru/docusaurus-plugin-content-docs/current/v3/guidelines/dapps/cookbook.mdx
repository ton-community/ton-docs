...
Адрес TON однозначно идентифицирует контракт в блокчейне, указывая его workchain и хеш исходного состояния. [Используются два основных формата](/v3/documentation/smart-contracts/addresses/address-formats): **raw** (workchain и хеш в HEX-формате, разделенные символом ":") и **user-friendly** (base64-кодировка с определенными флагами).
