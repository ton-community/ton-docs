...
| ------------- | ------------ | ----------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `uri`         | ASCII string | optional    | URI, указывающий на JSON-документ с метаданными, который используется в формате "Semi-chain content layout". |
| `name`        | UTF8 string  | optional    | идентифицирует asset                                                                                                         |
| `description` | UTF8 string  | optional    | описывает актив                                                                                                              |
| `image`       | ASCII string | optional    | URI, указывающий на ресурс с типом mime image                                                                                |
| `image_data`  | binary\*     | optional    | либо двоичное представление изображения для on-chain размещения, либо base64 для off-chain размещения                        |
...
- _n-of-total_ — количество jetton от общего количества выпущенных jetton. Например, если totalSupply выпускаемых jetton равно 1000 и у пользователя есть 100 jetton в кошельке, то должно отображаться в кошельке пользователя как 100 из 1000 или другим текстовым или графическим способом, чтобы показать соотношение токенов пользователя к общему количеству доступных токенов.
- _%_ — процент от общего количества выпущенных jetton. Например, если общее количество выпускаемых jetton равно 1000 и у пользователя есть 100 jetton, то процент должен быть отображен как 10% от баланса кошелька пользователя (100 ÷ 1000 = 0,1 или 10%).
...
| `uri`          | ASCII string | optional    | URI, указывающий на JSON-документ с метаданными, который используется в формате "Semi-chain content layout".                                                                                                                                                                                                                                                                                     |
| `name`         | UTF8 string  | optional    | идентифицирует asset                                                                                                                                                                                                                                                                                                                                                                                             |
| `description`  | UTF8 string  | optional    | описывает asset                                                                                                                                                                                                                                                                                                                                                                                                  |
| `image`        | ASCII string | optional    | URI, указывающий на ресурс с типом mime image                                                                                                                                                                                                                                                                                                                                                                    |
| `image_data`   | binary\*     | optional    | либо двоичное представление изображения для on-chain размещения, либо base64 для off-chain размещения                                                                                                                                                                                                                                                                                                            |
| `symbol`       | UTF8 string  | optional    | символ токена - например, "XMPL" и используется в форме "Вы получили 99 XMPL"                                                                                                                                                                                                                                                                                                                                    |
| `decimals`     | UTF8 string  | optional    | количество десятичных знаков, которые использует токен. Если не указано, по умолчанию используется значение 9. A UTF8-encoded string with numbers from 0 to 255. Строковое значение с числом от 0 до 255, например 8, означает, что количество токенов должно быть разделено на 100000000 для получения его пользовательского представления.     |
| `amount_style` |              | optional    | необходим внешним приложениям, чтобы они понимали формат отображения количества jetton. Определяется с помощью _n_, _n-of-total_, _%_.                                                                                                                                                                                                                                           |
| `render_type`  |              | optional    | Необходим для внешних приложений, чтобы они понимали в какую группу относится jetton и как его отображать. "currency" — отображается как валюта (значение по умолчанию). "game" — используется для игр, которые отображаются как NFT, но также показывают количество jetton и учитывают значение параметра amount_style. |
...
- _game_ - используется для игр, которые отображаются как NFT, но также показывают количество jetton и учитывают значение параметра `amount_style`.
