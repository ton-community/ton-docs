...
채굴 과정은 대부분 `mytonctrl`에 의해 자동화되어 있습니다. 채굴 과정에 대한 자세한 정보는 PoW Giver 문서(아카이브)에서 확인할 수 있습니다.

## <a id="advanced"></a>고급

채굴에 진지하게 관심이 있고 두 대 이상의 기계/채굴장을 운영하고자 하는 경우, TON과 채굴 작동 방식을 배워야 하며 자세한 내용은 [TON 문서](/v3/documentation/ton-documentation)에서 심도 있는 정보를 확인하세요. 여기 몇 가지 일반적인 조언이 있습니다:
...
- 마이닝 프로세스가 어떻게 작동하는지 이해하려고 노력하세요. 대부분의 대규모 마이너는 여러 대의 마이닝 머신이 있는 환경에서 `mytonctrl`보다 많은 이점을 제공하는 자체 스크립트를 사용합니다.
...
이는 이 가이드의 범위를 벗어납니다. [풀 노드 실행](/v3/guidelines/nodes/running-nodes/full-node) 및 [mytonctrl 지침](https://github.com/igroman787/mytonctrl) 을 참조하세요.

#### <a id="faq-software-build"></a>내 OS에서 소프트웨어를 빌드하는 데 도움을 줄 수 있나요?

이는 이 가이드의 범위를 벗어납니다. [풀 노드 실행](/v3/guidelines/nodes/running-nodes/full-node) 및 [Mytonctrl 설치 스크립트](https://github.com/igroman787/mytonctrl/blob/master/scripts/toninstaller.sh#L44) 를 참조하여 종속성 및 프로세스에 대한 정보를 확인하세요.
