...
- 새로운 스토리지 컨트랙트 수락 여부
- 최소 및 최대 _Bag_ 크기(바이트 단위)
- 요금 - 스토리지 비용. 일당 메가바이트당 nanoTON으로 지정됨
- 최대 스팬 - 제공자가 얼마나 자주 _Bag_ 저장 증명을 제공해야 하는지
...
`accept` 매개변수를 `1`로 설정한 후, 스마트 컨트랙트는 클라이언트의 요청을 수락하고 스토리지 컨트랙트를 생성하기 시작하며, 스토리지 데몬은 자동으로 이를 처리합니다: _Bag_의 다운로드 및 배포, 저장 증명 생성.
