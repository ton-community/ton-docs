...
| `금액`             | coins  | Tthe total `ton coin` amount sent with the message.                                                                                                                                    |
...
| `custom_payload` | 어쩌면 셀  | Always at least 1 bit in size. Custom data used by either the sender or receiver jetton wallet for internal logic.                                                     |
| `전송_톤_금액`        | coins  | Must be greater than 0 to send a transfer notification message with a `forward payload`. It is **part of `amount` value** and **must be lesser than `amount`**.        |
