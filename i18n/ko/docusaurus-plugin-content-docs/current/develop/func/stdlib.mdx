...
Some useful information regarding smartcontract invocation can be found in [c7 special register](/learn/tvm-instructions/tvm-overview#control-registers). Those primitives serve for convenient data extraction.
#### now
```cpp
int now() asm "NOW";
```
...
```
Loads serialized amount of Toncoin (any unsigned integer up to `2^128 - 1`).
