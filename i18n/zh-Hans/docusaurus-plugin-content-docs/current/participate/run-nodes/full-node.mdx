...
[MyTonCtrl](https://github.com/ton-blockchain/mytonctrl)是一个控制台应用程序，它是fift、lite-client和validator-engine-console的方便包装器。它专门开发用于简化在Linux操作系统上的钱包、域和验证器管理任务。
...
这些要求适用于**带验证器的全节点**。如果您想运行没有验证器的全节点（例如[liteserver](/participate/run-nodes/full-node#hardware-requirements-1)），您可以使用较低配置的机器。
...
* **mytoncore状态**：应为绿色。
* **本地验证器状态**：也应为绿色。
* **本地验证器未同步**：最初显示一个大数字。一旦新创建的验证器与其他验证器连接，数字将在250k左右。随着同步的进行，这个数字会减少。当它降至20以下时，验证器已同步。
...
最近（_在2023年底_），大致数字为最低赌注约__340K TON__，最高约__1M TON__。

查看[tonscan.com](https://tonscan.com/validation)以了解所需硬币数量。

阅读更多关于[如何计算最大和最小赌注](/participate/network-maintenance/staking-incentives#values-of-stakes-max-effective-stake)。
...
**mytoncore** 将自动加入选举。它将钱包余额分为两部分，并使用它们作为赌注参加选举。您也可以手动设置赌注大小：

```sh
set stake 50000
```

`set stake 50000` — 这将赌注大小设置为50k硬币。如果赌注被接受并且我们的节点成为验证器，那么赌注只能在第二次选举中提取（根据选民的规则）。
...
与[验证器](/participate/run-nodes/full-node#hardware-requirements)相比，liteserver模式需要较少的资源。然而，仍建议使用功能强大的机器运行liteserver。
...
Hetzner和OVH被禁止运行验证器，但您可以使用它们运行liteserver：
...
3. 这个文件将帮助您连接到您的liteserver。将位于指定路径的配置文件复制到您的家目录以保存它。
...
如果您遇到`Validator console is not settings`错误，这表明您正在以安装以外的用户运行`MyTonCtrl`。
...
__答：__ 这是正常的，通常这意味着您试图检索尚未到达您请求的节点的
...
如果节点状态中没有本地节点部分，通常意味着安装过程中出了问题，跳过了创建/分配验证器钱包的步骤。
还要检查是否指定了验证器钱包。
...
:::info
将所有密钥和配置从旧节点转移到新的工作节点并启动它。以防新的出现问题，还有原始设置好的来源。
:::
...
2. 在两台机器上停止`mytoncore`和验证器`服务`，在源和新的上进行备份：
