...
在这篇文章中，我们将考虑TON探索者，他们的能力和特点，从开发者的角度出发。

## 什么是探索者？

探索者是一个网站，允许您查看区块链中的信息，如账户余额、交易历史、区块等。
...
这种分类大体上是有条件的，一个探索者可以同时属于几个类别。所以我们不要过多地关注这个。
...
让我们从所有探索者中都存在的通用功能开始。

几乎所有的探索者都能查找关于余额、交易历史和智能合约的信息，如果部署在地址上的话。

接下来，我们将考虑几个可以归为这些类别的探索者。

## TON Scan.org

适合日常使用的好探索者。它提供了TON区块链的全面视图，允许用户搜索交易、地址、区块等。任何搜索都是针对公共[地址簿](https://github.com/catchain/tonscan/blob/master/src/addrbook.json)（TON基金会，OKX等）进行的。
...
DTON是另一个面向开发者的探索者。它以方便的形式提供了大量的交易信息。
...
这个探索者专为TON区块链上的非同质化代币（NFT）设计。它允许用户探索、跟踪和验证NFT交易、合约和元数据。
