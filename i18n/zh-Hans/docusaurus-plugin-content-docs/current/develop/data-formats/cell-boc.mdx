...
:::
现在让我们按照父cell不指向后面的顺序排列它们。被其余cell指向的cell必须在指向它的那些cell之后出现在列表中。我们得到：
```json
1[8_]      -> 索引 0 (根cell)
7[FE]      -> 索引 1
24[0AAAAA] -> 索引 2
...
第一个字节 - **引用描述符** - 计算为 `r+8s+32l`，其中 `0 ≤ r ≤ 4` 是cell引用（链接）的数量，`0 ≤ s ≤ 1` 对于[异域](#special-exotic-cells)cell为1，对于普通cell为0，`0 ≤ l ≤ 3` 是cell的[层级](
...
因此，我们根据其[架构](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/tl/boc.tlb#L25)构建头部。
...
### 异域cell层级
异域cell对其层级的设置有不同的规则，这些规则在[此文](/develop/data-formats/exotic-cells)中有描述。

## cell哈希
在大多数情况下，用户使用的是层级为0的普通cell，它们只有一个哈希，称为表示哈希（或无限哈希）。

层级为`Lvl(c) = l`的cell`c`，其中`1 ≤ l ≤ 3`，除了表示哈希外，还有`l`个**"更高"**的哈希。
...
连接前面步骤的字节，我们得到 `00080000000f`，其SHA256为 `57b520dbcb9d135863fc33963cde9f6db2ded1430d88056810a2c9434a3860f9` - 这就是cell表示哈希。
...
，其SHA256为 `f345277cc6cfa747f001367e1e873dcfa8a936b8492431248b7a3eeafa8030e7` - 这就是cell表示哈希。

### 更高哈希的计算

普通cell`c`的更高哈希与其表示哈希的计算类似，
但使用其引用的更高哈希而不是它们的表示哈希。

异域cell有其自己的计算更高哈希的规则，这些规则在[此文](/develop/data-formats/exotic-cells)中有描述。


## 另见

[//]: # (* [原文RU]&#40;https://github.com/xssnick/ton-deep-doc/blob/master/Cells-BoC.md&#41;)
* [异域（特殊）cell](/develop/data-formats/exotic-cells)
