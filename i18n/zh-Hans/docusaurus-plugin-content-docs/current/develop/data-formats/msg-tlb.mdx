...
这里 `Message X` - 是常见消息结构，`MessageRelaxed X` 是带有 CommonMsgInfoRelaxed 体的附加类型，而 `Message Any` 是两者的并集。
消息结构与 X:Type 统一，换句话说就是一个 Cell。
根据 TL-B，如果数据能够适应 1023 位，我们可以将所有数据组合在一个cell中，或者使用带有插入符号 `^` 的引用。
...
MsgInfo][11][^StateInit][1][^X]` - `Message X` 带引用
...
| ihr_disabled   | 布尔                                      | 必需     | 超立方体路由标志。                                                                                                    |
| bounce         | 布尔                                      | 必需     | 如果处理过程中出现错误，消息应该被反弹。如果消息的 flat bounce = 1，它被称为可反弹。                                 |
| bounced        | 布尔                                      | 必需     | 描述消息本身是反弹结果的标志。                                                                                        |       
...
| ihr_disabled       | 布尔                                 | 必需     | 超路由标志。（目前始终为真）                                                                                           |
...
| src                 | [MsgAddressInt](#msgaddressint-tl-b) | 必需     | 超路由标志。                                                                                                 |
...
| external_address  | (bits len) | 必需     | 地址是长度等于前面的 `len` 的位字符串                                                                         |
...
| grams       | (VarUInteger 16)        | 必需     | 以纳顿表示的消息价值                                                                                    |
...
```func
  var msg = begin_cell()
    ...
    .store_uint(0,  1) ;; 没有 init-field 标志（Maybe）
    .store_uint(0,  1) ;; 原位消息体标志（Either）
