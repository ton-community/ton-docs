...
### 添加简单的机器人代码
创建一个 `.env` 文件，并在其中添加你的机器人令牌、DAppmanifest 和钱包列表缓存生存时间：
...
bot.on('message', msg => {
  ...
  bot.sendMessage(chatId, '收到了你的消息');
});
```

我们可以了。你可以运行 `npm run compile` 和 `npm run start`，然后给你的机器人发送任何消息。机器人将回复“收到了你的消息”。我们准备好进行 TonConnect 集成了。
...
bot.onText(/\/connect/, async msg => {
  ...
  connector.onStatusChange(wallet => {
    if (wallet) {
      bot.sendMessage(chatId, `${wallet.device.appName} 钱包已连接！`);
    }
  });
...
export async function handleSendTXCommand(msg: TelegramBot.Message): Promise<void> {
    ...
    if (!connector.connected) {
        await bot.sendMessage(chatId, '连接钱包以发送交易');
        return;
    }

    connector
        .sendTransaction({
            validUntil: Math.round(Date.now() / 1000) + 600, // 超时时间是秒
            messages: [
                {
                    amount: '1000000',
        ...
        .then(() => {
            bot.sendMessage(chatId, `交易成功发送`);
        })
        .catch(e => {
            if (e instanceof UserRejectsError) {
                bot.sendMessage(chatId, `你拒绝了交易`);
                return;
            }

            bot.sendMessage(chatId, `发生未知错误`);
        })
    ...
    await bot.sendMessage(
        chatId,
        `打开 ${walletInfo?.name || connector.wallet!.device.appName} 并确认交易`,
        {
            reply_markup: {
                inline_keyboard: [
                    [
...
export async function handleDisconnectCommand(msg: TelegramBot.Message): Promise<void> {
    ...
    await connector.restoreConnection();
    if (!connector.connected) {
        await bot.sendMessage(chatId, "你还没有连接钱包");
        return;
    }

    await connector.disconnect();

    await bot.sendMessage(chatId, '钱包已断开连接');
}
...
如果用户在 `DELETE_SEND_TX_MESSAGE_TIMEOUT_MS`（10分钟）内未确认交易，交易将被取消并且机器人将发送消息“交易未被确认”。

你可以将此参数设置为 `5000`，编译并重新运行机器人，测试其行为。

我们此时每次通过钱包连接菜单步骤时都会创建一个新的连接器。
这样做效率很低，因为在创建新的连接器时我们没有关闭之前连接器的连接。
让我们改进这一行为，为用户的连接器创建一个缓存映射。
...
我们定义了`newConnectRequestListenersMap`来为每个用户存储最后一个连接请求的清理回调。
如果用户多次调用`/connect`，机器人将删除上一个带有 QR 码的消息。
此外，我们订阅了连接器过期超时以在 QR 码消息过期时删除它。
...
然而，我们应该改进重定向策略以提供更好的用户体验。此外，让我们在第一个（“通用 QR”）屏幕上添加 `连接 @wallet` 按钮。
...
这里我们为 First screen（通用 QR 屏幕）添加独立的 @wallet 按钮。接下来的工作是在 connect-wallet-menu 和 command-handlers 中使用这个函数：
        ...
        `打开 ${walletInfo?.name || connector.wallet!.device.appName} 并确认交易`,
        {
            reply_markup: {
                inline_keyboard: [
                    [
                        {
                            text: `打开 ${walletInfo?.name || connector.wallet!.device.appName}`,
                            url: deeplink
                        }
                    ]
                ]
