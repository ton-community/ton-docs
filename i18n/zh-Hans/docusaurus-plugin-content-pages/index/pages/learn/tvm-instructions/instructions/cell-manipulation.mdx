...
| **`C9`** | `ENDC` | _`b - c`_ | 将 _构建器_ 转换为普通的 _单元格_。 | `518` |
...
| **`CC`** | `STREF` | _`c b - b'`_ | 将 _单元格_ `c` 的引用存入 _构建器_ `b` 中。 | `18` |
...
| **`CF23`** |  | _`b x - c`_ | 如果 `x!=0`，从 _构建器_ `b` 创建一个 _特殊_ 或 _异类_ 单元格。<br/>异类单元格的类型必须存储在 `b` 的前 8 位中。<br/>如果 `x=0`，它相当于 [`ENDC`](#instr-endc)。否则，将在创建异类单元格之前对 `b` 的数据和引用执行一些有效性检查。 | `526` |
...
| **`CF30`** | `BDEPTH` | _`b - x`_ | 返回 _构建器_ `b` 的深度。如果 `b` 中没有存储单元格引用，则 `x=0`；否则 `x` 是对 `b` 中引用的单元格的深度的最大值加 1。 | `26` |
| **`CF31`** | `BBITS` | _`b - x`_ | 返回已经存储在 _构建器_ `b` 中的数据位数。 | `26` |
| **`CF32`** | `BREFS` | _`b - y`_ | 返回已经存储在 `b` 中的单元格引用数。 | `26` |
| **`CF33`** | `BBITREFS` | _`b - x y`_ | 返回 `b` 中数据位数和单元格引用数。 | `26` |
...
| xxxxxxx<br/>操作码 | xxxxxxxxxxxxxxxxxxxxxxxxxxxx<br/>Fift 语法 | xxxxxxxxxxxxxxxxx<br/>堆栈 | xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx<br/>描述 | xxxx<br/>Gas |
|:-|:-|:-|:-|:-|
| **`D0`** | `CTOS` | _`c - s`_ | 将 _单元格_ 转换为 _切片_。请注意，`c` 必须是普通单元格，或者是自动 _加载_ 以产生普通单元格 `c'` 的异类单元格，然后转换为 _切片_。 | `118/43` |
...
| **`D4`** | `LDREF` | _`s - c s'`_ | 从 `s` 中加载一个单元格引用 `c`。 | `18` |
...
| **`D704`** | `LDIXQ` | _`s l - x s' -1 or s 0`_ | [`LDIX`](#instr-ldix) 的静默版本：类似地从 `s` 中加载一个有符号 `l`-位整数，但在成功时返回一个成功标志 `-1`，失败时（如果 `s` 没有 `l` 位）返回 `0`，而不是抛出单元格下溢异常。 | `26` |
...
| **`D726`** | `SDBEGINSX` | _`s s' - s''`_ | 检查 `s` 是否以 `s'`（数据位）开始，并在成功时从 `s` 中移除 `s'`。失败抛出单元格反序列化异常。原语 [`SDPFXREV`](#instr-sdpfxrev) 可以认为是 [`SDBEGINSX`](#instr-sdbeginsx) 的静默版本。 | `26` |
...
| **`D739`** |  | _`c - s？`_ | 将普通或异类单元格转换为 _切片_，就好像它是一个普通单元格一样。返回一个标志，指示 `c` 是否是异类的。如果是这样，其类型可以稍后从 `s` 的前八位中反序列化。 |  |
| **`D73A`** |  | _`c - c'`_ | 加载异类单元格 `c` 并返回一个普通单元格 `c'`。如果 `c` 已经是普通的，则不执行任何操作。如果 `c` 无法加载，抛出异常。 |  |
| **`D73B`** |  | _`c - c' -1 或 c 0`_ | 加载异类单元格 `c` 并返回一个普通单元格 `c'`。如果 `c` 已经是普通的，则不执行任何操作。如果 `c` 无法加载，返回 0。 |  |
| **`D741`** | `SCHKBITS` | _`s l - `_ | 检查 _切片_ `s` 中是否至少有 `l` 个数据位。如果不是这种情况，抛出单元格反序列化（即单元格下溢）异常。 | `26/76` |
...
| **`D748`** | `PLDREFVAR` | _`s n - c`_ | 返回 _切片_ `s` 的第 `n` 个单元格引用，`0 <= n <= 3`。 | `26` |
| **`D749`** | `SBITS` | _`s - l`_ | 返回 _切片_ `s` 中的数据位数。 | `26` |
| **`D74A`** | `SREFS` | _`s - r`_ | 返回 _切片_ `s` 中的引用数。 | `26` |
| **`D74B`** | `SBITREFS` | _`s - l r`_ | 返回 `s` 中的数据位数和引用数。 | `26` |
| **`D74E_n`** | `[n] PLDREFIDX` | _`s - c`_ | 返回 _切片_ `s` 的第 `n` 个单元格引用，`0 <= n <= 3`。 | `26` |
| **`D74C`** | `PLDREF` | _`s - c`_ | 预加载 _切片_ 的第一个单元格引用。 | `26` |
...
| **`D764`** | `SDEPTH` | _`s - x`_ | 返回 _切片_ `s` 的深度。如果 `s` 没有引用，则 `x=0`；否则 `x` 是从 `s` 引用的单元格的最大深度加 1。 | `26` |
| **`D765`** | `CDEPTH` | _`c - x`_ | 返回 _单元格_ `c` 的深度。如果 `c` 没有引用，则 `x=0`；否则 `x` 是从 `c` 引用的单元格的最大深度加 1。如果 `c` 是 _空（Null）_ 而不是 _单元格_，返回零。 | `26` |
