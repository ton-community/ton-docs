...
| کتابخانه     | HashmapE 256 SimpleLib\* | Optional | Currently, `StateInit` is used without libraries.                                                                                                                                                                                                                                                                                                                        |
...
| آدرس_خارجی       | (طول بیت)   | Required | A bitstring address, where the length of the address equals the previously defined `len`.                              |
...
| پخش هر        | Anycast\*    | Optional | Represents additional address data. Currently not used in ordinary internal messages.                                                                                                                                                                      |
...
| آدرس          | (bits256)    | Required | The smart contract account ID number.                                                                                                                                                                                                                      |
...
| پخش هر        | Anycast\*    | Optional  | Additional address data is not currently used in regular internal messages.                                                                         |
...
| آدرس          | (bits256)    | Required  | The payload address (this could be the account ID).                                                                                                 |
...
| Structure      | Type                    | Required | Description                                                                                                                         |
| -------------- | ----------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| واحدهای پولی$_ | Constructor             | Required | The `$_` empty tag indicates that no bits are added at the beginning during the serialization of `CurrencyCollection`.              |
| گرم‌ها         | (VarUInteger ۱۶)        | Required | Represents the message value, expressed in nanoTons.                                                                                |
| دیگر           | ExtraCurrencyCollection | Optional | ExtraCurrencyCollection is a dictionary intended for additional, typically empty currencies.                                        |
...
| مقدار      | (uint (len \* ۸)) | Optional | An optional unsigned integer encoded in `(len * 8)` bits.                                                                   |
