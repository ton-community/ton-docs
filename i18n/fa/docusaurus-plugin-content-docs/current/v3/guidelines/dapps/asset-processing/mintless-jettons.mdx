...
- **قابلیت گسترش**: فرآیندهای ضرب سنتی می‌توانند هنگام توزیع توکن‌ها به تعداد زیادی کاربر بار منابع و هزینه‌بر باشند.
...
- ریشه `merkle_hash` را محاسبه کنید.

2. Deploy a jetton master contract:

- `merkle_hash` را در ذخیره‌سازی قرارداد درج کنید.
...
### mintless-proof-generator (از Core)
...
- فایل کامپایل شده به صورت `build/crypto/mintless-proof-generator` ذخیره می‌شود.
