...
![ربات تلگرام](/img/registration-process/telegram-bot.png)

Press the `Manage API Keys` or `Toncenter` button to open the Telegram Mini App.

![مینی اپلیکیشن اصلی Toncenter](/img/registration-process/toncenter-main-miniapp.png)

دکمه `ایجاد کلید API` را فشار دهید.

![ایجاد کلید API](/img/registration-process/create-api-key.png)
...
برای به‌روزرسانی طرح اشتراک خود، این مراحل را دنبال کنید:
