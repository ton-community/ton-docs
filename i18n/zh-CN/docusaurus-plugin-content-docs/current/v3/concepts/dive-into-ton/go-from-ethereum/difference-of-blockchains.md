...
- 外部拥有账户（EOA）--外部管理账户由用户使用公钥和私钥对控制。公用密钥允许他人向该账户发送付款。 The public key allows others to send payments to the account.
- 合约账户--由智能合约代码而非私钥控制。由于没有私钥，合约账户无法自行启动交易。 Contract accounts cannot initiate transactions independently because they do not have a private key.
