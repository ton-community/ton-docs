...
从技术角度看，TON网站非常类似于标准网站，但它们是通过[TON网络](/v3/documentation/network/protocols/adnl/overview)（互联网内的一个覆盖网络）而不是互联网访问的。更具体地说，它们拥有一个[ADNL](/v3/documentation/network/protocols/adnl/overview)地址（而不是更常见的IPv4或IPv6地址），并通过[RLDP](/v3/documentation/network/protocols/rldp)协议（这是建立在ADNL之上的高级RPC协议，ADNL是TON网络的主要协议）接受HTTP查询，而不是常规的TCP/IP。所有加密由ADNL处理，所以如果入口代理托管在用户设备上，就没有必要使用HTTPS（即TLS）。
...
1. 从[TON自动构建](https://github.com/ton-blockchain/ton/releases/latest)下载**rldp-http-proxy**。
   ...
   或者你可以按照这些[指示](/v3/guidelines/smart-contracts/howto/compile/compilation-instructions#rldp-http-proxy)自己编译**rldp-http-proxy**。

2. [下载](/v3/guidelines/smart-contracts/howto/compile/compilation-instructions#download-global-config)TON全局配置。
...
3. 从[TON自动构建](https://github.com/ton-blockchain/ton/releases/latest)下载**generate-random-id**。
   ...
   或者你可以按照这些[指示](/v3/guidelines/smart-contracts/howto/compile/compilation-instructions#generate-random-id)自己编译**generate-random-id**。
...
:::tip 教程找到了！
嘿！不要从初学者友好的教程[如何运行TON网站？](/v3/guidelines/web3/ton-proxy-sites/how-to-run-ton-site)开始
:::
...
1. 从 [TON Auto Builds](https://github.com/ton-blockchain/ton/releases/latest) 下载 **rldp-http-proxy** 。
   ...
   或者你可以按照这个[指示](/v3/guidelines/smart-contracts/howto/compile/compilation-instructions#rldp-http-proxy)自己编译**rldp-http-proxy**。
...
如果你愿意，你可以[注册](/v3/guidelines/web3/ton-proxy-sites/site-and-domain-management)一个TON DNS域名，比如'example.ton'，并为这个域名创建一个指向你TON网站的持久ADNL地址的`site`记录。然后，在客户端模式下运行的RLDP-HTTP代理将会解析http://example.ton为指向你的ADNL地址，并访问你的TON网站。
