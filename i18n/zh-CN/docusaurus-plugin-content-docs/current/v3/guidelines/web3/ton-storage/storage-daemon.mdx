...
您可以使用此[说明](/v3/guidelines/smart-contracts/howto/compile/compilation-instructions#storage-daemon)从源代码编译`storage-daemon`和`storage-daemon-cli`。

## 关键概念

- *文件包*或*包* - 通过TON存储分发的文件集合
- TON存储的网络部分基于类似于种子的技术，因此术语*种子*、*文件包*和*包*将互换使用。但重要的是要注意一些区别：TON存储通过[ADNL](/v3/documentation/network/protocols/adnl/overview)通过[RLDP](/v3/documentation/network/protocols/rldp)协议传输数据，每个*包*通过其自己的网络覆盖层分发，merkle结构可以存在两个版本 - 用于高效下载的大块和用于高效所有权证明的小块，以及[TON DHT](/v3/documentation/network/protocols/dht/overview)网络用于查找节点。
- *文件包*由*种子信息*和数据块组成。
...
- `-v` - 详细程度（INFO）
- `-C` - 全局网络配置（[下载全局配置](/v3/guidelines/smart-contracts/howto/compile/compilation-instructions#download-global-config)）
- `-I` - ADNL的IP地址和端口
- `-p` - 控制台接口的TCP端口
- `-D` - 存储守护程序数据库的目录
