...
通过`from`（合约发送方地址）、`to`（合约目的地地址）、`op`（操作码值） - 我们将仅检索此组合的一个交易。 This combination retrieves only one transaction.
...
- `destroyed` - `true` - 如果现有合约因执行某个交易而被销毁。否则 - `false`。 Otherwise, it is `false`.
...
- **`endStatus`**: AccountStatus after transaction execution. `oldStatus` - 交易执行前的账户状态。值：`'uninitialized'`, `'frozen'`, `'active'`, `'non-existing'`。
...
收到来自Fireworks launcher 1的入站消息，交易[ID:6]在the Launcher中进行。此消息以`send mode = 2`发送。 This message is sent with `send mode = 2`.
...
收到来自Fireworks launcher 2的入站消息，交易[ID:8]在the Launcher中进行。此消息以`send mode = 64`发送。 This message is sent with `send mode = 64`.
...
此测试验证启动fireworks的交易费用是否符合预期。可以为佣金费用的不同部分进行自定义定价。 You can define custom assertions for different parts of the commission fees.
...
在本节中，将提供在交易处理期间可能发生的TVM [exit codes（退出代码）](/learn/tvm-instructions/tvm-exit-codes)的测试用例。这些exit codes在区块链代码本身中。同时，必须区分在[Compute Phase（ Compute Phase ）](/learn/tvm-instructions/tvm-overview#compute-phase)和Action Phase（行动阶段）期间的exit code。 These exit codes are part of the blockchain code itself. It is necessary to distinguish between exit codes during the [Compute Phase](/v3/documentation/tvm/tvm-overview#compute-phase) and the Action Phase.
...
An alternative exit code for denoting the success of the Compute Phase is `1`. 标记Compute Phase成功的另一种exit code是`1`。要获得此exit code，您需要使用[RETALT](https://github.com/ton-community/fireworks-func/blob/ef49b4da12d287a8f6c2b6f0c19d65814c1578fc/contracts/fireworks.fc#L20)。

This opcode must be called in the main function (e.g., `recv_internal`). 值得注意的是，这个操作码应该在主函数中调用（例如，recv_internal）。如果在另一个函数中调用，则该函数的exit将为`1`，但总体exit code将为`0`。
...
首先，我们[构建](https://github.com/ton-community/fireworks-func/blob/ef49b4da12d287a8f6c2b6f0c19d65814c1578fc/contracts/fireworks.fc#L31-L32)这样的continuation。在这种情况下，这只是一个空的continuation，什么也不做。接下来，使用操作码`0 SETNUMARGS`，我们指示在执行开始时堆栈中不应有值。然后，使用操作码`1 -1 SETCONTARGS`，我们调用continuation，传递1个值。由于本来应该没有值，因此我们得到了StackOverflow错误。 In this case, it is an empty continuation that does nothing. Next, using the opcode `0 SETNUMARGS`, we indicate that no values should be on the stack at the start of execution. Then, we call the continuation using the opcode `1 -1 SETCONTARGS`, passing one value. Since there should have been no values, we get a StackOverflow error.

### Compute Phase | exit code = 4

在TVM中，`integer`可以在-2<sup>256</sup> \< x \< 2<sup>256</sup>范围内。如果在计算过程中值超出此范围，则抛出exit code 4。 If a value exceeds this range during calculation, exit code 4 is thrown.

### Compute Phase | exit code = 5

如果`integer`值超出预期范围，则抛出exit code 5。例如，如果在`.store_uint()`函数中使用了负值。 For example, if a negative value is used in the `.store_uint()` function.
...
This is a common error that occurs when receiving the wrong data type. 这是一个发生在接收到错误的数据类型时的很常见的错误。在[示例](https://github.com/ton-community/fireworks-func/blob/ef49b4da12d287a8f6c2b6f0c19d65814c1578fc/contracts/fireworks.fc#L79-L80)中，`tuple`包含3个元素，但在解包时尝试获取4个。
...
This error is thrown when working with [dictionaries](/v3/documentation/smart-contracts/func/docs/stdlib/#dictionaries-primitives). 此错误在处理[字典](/develop/func/stdlib/#dictionaries-primitives)时抛出。例如，当值属于键时[存储在另一个cell中](https://github.com/ton-community/fireworks-func/blob/ef49b4da12d287a8f6c2b6f0c19d65814c1578fc/contracts/fireworks.fc#L100-L110)作为引用。在这种情况下，您需要使用`.udict_get_ref()`函数来获取这样的值。
...
**注意：** 实际上，字典的结构（数据如何放置在cell中）比上面的示例更复杂。因此，它们被简化了，以便理解示例。 These examples are simplified for clarity.
...
It also occurs when attempting to call a non-existent method. 此外，它还在尝试调用不存在的方法时发生。开发人员通常是在调用不存在的GET方法时面临这种情况。

### Compute Phase | exit code = -14 (13)

The system throws this error if there is insufficient TON to handle the Compute Phase. 如果处理Compute Phase的TON不足，则抛出此错误。在枚举类`Excno`中，其中指示了Compute Phase中各种错误的exit code，[指示的值为13](https://github.com/ton-blockchain/ton/blob/9728bc65b75defe4f9dcaaea0f62a22f198abe96/crypto/vm/excno.hpp#L39)。  

然而，在处理过程中，对此值应用了[NOT操作](https://github.com/ton-blockchain/ton/blob/9728bc65b75defe4f9dcaaea0f62a22f198abe96/crypto/block/transaction.cpp#L1574)，将此值更改为`-14`。这样做是为了这个exit code不能被伪造，例如使用`throw`函数，因为所有这些函数只接受exit code是正值。 This ensures that the exit code cannot be faked using functions like `throw`, as these functions only accept positive values for exit codes. 
...
目前，一个交易中最多可以有`255`个动作。如果超过这个值，则Action Phase将以exit code 33 结束。 The Action Phase ends with exit code 33 if this limit is exceeded.
...
在构建消息的 [CommonMsgInfo](/develop/smart-contracts/tutorials/wallet#commonmsginfo) 部分时，必须指定正确的源地址。它必须等于[addr_none](/develop/data-formats/msg-tlb#addr_none00) 或发送消息的账户地址。 It must be either [addr_none](/v3/documentation/data-formats/tlb/msg-tlb#addr_none00) or the address of the account sending the message.
...
此exit code类似于Compute Phase的`-14`。在这里，它意味着余额不足以发送指定金额的TON。 It indicates insufficient balance to send the specified amount of TON.
...
:::info 
Since the order of lines may change when updating the code, some links may become outdated. 由于更新代码时行的顺序可能会改变，一些链接变得不相关。因此，所有链接都将使用提交[9728bc65b75defe4f9dcaaea0f62a22f198abe96](https://github.com/ton-blockchain/ton/tree/9728bc65b75defe4f9dcaaea0f62a22f198abe96)时的代码库状态。\
:::
