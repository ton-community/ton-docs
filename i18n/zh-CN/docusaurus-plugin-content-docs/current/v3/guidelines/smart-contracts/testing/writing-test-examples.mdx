...
在本节中，将提供在交易处理期间可能发生的TVM [exit codes（退出代码）](/v3/documentation/tvm/exit-codes)的测试用例。这些exit codes在区块链代码本身中。同时，必须区分在[Compute Phase（ Compute Phase ）](/v3/documentation/tvm/overview#compute-phase)和Action Phase（行动阶段）期间的exit code。
...
TVM是[堆栈机](/v3/documentation/tvm/overview)。与不同值交互时，它们会出现在堆栈上。如果突然堆栈上没有元素，但某些操作码需要它们，那么将抛出此错误。
...
在较低层级，使用操作码而不是熟悉的函数名称，可以在[此表](/v3/documentation/tvm/instructions)中以HEX形式看到。在这个例子中，我们[使用](https://github.com/ton-community/fireworks-func/blob/ef49b4da12d287a8f6c2b6f0c19d65814c1578fc/contracts/fireworks.fc#L25)`@addop`，添加了一个不存在的操作码。
...
TON 中的所有数据都存储在 [cells](/v3/documentation/data-formats/cells/overview) 中。一个 cell 可存储 1023 位数据和 4 个指向其他 cell 的引用。如果尝试写入超过 1023 位的数据或超过 4 个引用，将抛出 exit code 8。
...
此错误在处理[字典](/v3/documentation/smart-contracts/func/docs/stdlib#dictionaries-primitives)时抛出。例如，当值属于键时[存储在另一个cell中](https://github.com/ton-community/fireworks-func/blob/ef49b4da12d287a8f6c2b6f0c19d65814c1578fc/contracts/fireworks.fc#L100-L110)作为引用。在这种情况下，您需要使用`.udict_get_ref()`函数来获取这样的值。
...
此错误发生在未知情况。例如，在使用[SENDMSG](/v3/documentation/tvm/changelog/tvm-upgrade-2023-07#sending-messages)操作码时，如果传递了[错误](https://github.com/ton-community/fireworks-func/blob/ef49b4da12d287a8f6c2b6f0c19d65814c1578fc/contracts/fireworks.fc#L116)（例如，空的）的消息cell，那么就会发生这种错误。
...
Action Phase在Compute Phase之后开始，它处理在Compute Phase期间写入[寄存器c5](/v3/documentation/tvm/initialization#control-register-c5)的动作。如果此寄存器中的数据写入不正确，则抛出exit code 32。
...
在构建消息的 [CommonMsgInfo](/v3/documentation/data-formats/layout/messages#commonmsginfo) 部分时，必须指定正确的源地址。它必须等于[addr_none](/v3/documentation/data-formats/layout/messages#addr_none00) 或发送消息的账户地址。
...
与exit code 37相同，但指的是余额中缺乏[ExtraCurrency](/v3/documentation/dapps/assets/extra-currencies)。
...
库是存储在[Masterchain](/v3/guidelines/dapps/transactions/foundations-of-blockchain#masterchain)中的cell，如果它是[公开的](https://github.com/ton-blockchain/ton/blob/9728bc65b75defe4f9dcaaea0f62a22f198abe96/crypto/block/transaction.cpp#L1844)，可以被所有智能合约使用。
