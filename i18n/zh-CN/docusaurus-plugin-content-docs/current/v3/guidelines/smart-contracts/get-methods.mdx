...
:::note
在继续之前，建议读者对[FunC编程语言](/v3/documentation/smart-contracts/func/overview)和TON区块链上的[智能合约开发](/v3/documentation/smart-contracts/overview)有基本的了解。这将有助于您更有效地理解这里提供的信息。
:::
...
这些函数对于大多数智能合约来说都非常常见。例如，默认的[钱包合约](/v3/documentation/smart-contracts/contracts-specs/wallet-contracts)有几个get方法，如`seqno()`、`get_subwallet_id()`和`get_public_key()`。它们被钱包、SDK和API用来获取有关钱包的数据。
...
返回特定钱包中交易的序列号。这个方法主要用于[重放保护](/v3/guidelines/smart-contracts/howto/wallet#replay-protection)。
...
- [什么是Subwallet ID？](/v3/guidelines/smart-contracts/howto/wallet#subwallet-ids)
...
在TON区块链中，智能合约通过消息进行通信，而不是直接从另一个合约调用方法。向目标合约发送请求执行特定方法的消息。这些请求通常以特殊的[操作码](/v3/documentation/smart-contracts/message-management/internal-messages)开头。
...
- [用于操作码的CRC32哈希](/v3/documentation/data-formats/tlb/crc32)
