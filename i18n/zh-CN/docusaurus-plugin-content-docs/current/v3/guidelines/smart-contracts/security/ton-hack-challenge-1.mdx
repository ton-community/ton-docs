...
:::note 安全规则
始终检查函数是否有[`impure`](/v3/documentation/smart-contracts/func/docs/functions#impure-specifier)修饰符。
:::
...
:::note 安全规则
始终检查[修改/非修改](/v3/documentation/smart-contracts/func/docs/statements#methods-calls)方法。
:::
...
:::note 安全规则
在执行[`rand()`](/v3/documentation/smart-contracts/func/docs/stdlib#rand)之前，始终随机化seed。
:::
...
:::note 安全规则
始终检查[bounced](/v3/documentation/smart-contracts/message-management/non-bounceable-messages)消息。
不要忘记由[标准](/v3/documentation/smart-contracts/func/docs/stdlib)函数引起的错误。
尽可能使条件严格。
:::
...
如果用户发送“支票”，资金库没有弹回处理程序或代理消息到数据库。在数据库中，我们可以设置`msg_addr_none`作为奖励地址，因为`load_msg_address`允许它。我们向资金库请求支票，数据库尝试解析`msg_addr_none`使用[`parse_std_addr`](/v3/documentation/smart-contracts/func/docs/stdlib#parse_std_addr)，并失败。消息从数据库弹回到金库，并且op不是`op_not_winner`。

### 7. 更好的银行

:::note 安全规则
永远不要为了好玩而销毁账户。做[`raw_reserve`](/v3/documentation/smart-contracts/func/docs/stdlib#raw_reserve)而不是把钱发给自己。考虑可能的竞争条件。小心哈希映射的gas费用消耗。
:::
...
在合约中安全执行第三方代码是不可能的，因为[`out of gas`](/v3/documentation/tvm/exit-codes)异常不能被`CATCH`处理。攻击者可以简单地[`COMMIT`](/v3/documentation/tvm/instructions)合约的任何状态，并引发`out of gas`。
