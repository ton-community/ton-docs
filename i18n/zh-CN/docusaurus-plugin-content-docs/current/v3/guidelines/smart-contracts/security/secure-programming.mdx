...
在这里，你需要弄清楚这是一个什么样的请求，是否有足够的 gas 来完成它，以及有效载荷中的所有内容是否正确。在这一阶段，不应使用`throw_if()/throw_unless()`，因为这样会导致 Jettons 丢失，请求无法执行。值得使用 try-catch 语句[从 FunC v0.4.0 开始可用](/v3/documentation/smart-contracts/func/docs/statements#try-catch-statements)。
