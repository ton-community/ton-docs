...
详细信息请参阅：[惩罚政策](/v3/documentation/nodes/validation/staking-incentives#decentralized-system-of-penalties)。

### 运行 Fullnode

在阅读本文之前，请先启动 [Full Node](/v3/guidelines/nodes/running-nodes/full-node)。

使用 `status_modes` 命令检查验证模式是否启用。如果未启用，请参阅 [mytonctrl enable_mode 命令](/v3/documentation/nodes/mytonctrl/overview#enable_mode)。
...
1. 向钱包发送必要数量的硬币并激活它。
   ...
   阅读更多内容[最大和最小质押的计算方法](/v3/documentation/nodes/validation/staking-incentives#values-of-stakes-max-effective-stake)。
...
了解有关 [slashing policy](/v3/documentation/nodes/validation/staking-incentives#decentralized-system-of-penalties) 的更多信息。
:::
...
- [激励机制](/v3/documentation/nodes/validation/staking-incentives)
