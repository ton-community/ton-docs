...
:::important
`StateInit` (short for *state initialized*) can change as the contract evolves.
...
<BlockMath math="{Address} := ({workchain\_id},{account\_id})" />

, where

<BlockMath math="{account\_id}:= {HASH(StateInit)}" />
