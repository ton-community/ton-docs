...
TON地址在区块链中唯一标识合约，指示其工作链和原始状态哈希。使用两种常见格式[详见](/v3/documentation/smart-contracts/addresses/address)：原始（工作链和用":"字符分隔的HEX编码哈希）和用户友好（带有特定标志的base64编码）。
...
- 使用秘钥和工作链（通常为 0，代表 [basechain](/v3/guidelines/dapps/transactions/foundations-of-blockchain#workchain-and-basechain#workchain-blockchain-with-your-own-rules)）创建正确版本（大多数情况下为 v3r2；另请参阅  [wallet versions](/v3/documentation/smart-contracts/contracts-specs/wallet-contracts)）的钱包包装器（程序中的对象）。
