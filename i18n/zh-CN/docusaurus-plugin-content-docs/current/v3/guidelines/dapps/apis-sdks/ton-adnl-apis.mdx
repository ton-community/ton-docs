...
2. **ADNL连接**：您正在连接一个[liteserver](/v3/guidelines/nodes/running-nodes/liteserver-node)。它们可能无法访问，但通过一定程度的验证（在库中实现），它们无法作恶。
...
您可以从全局配置（[Mainnet](https://ton.org/global-config.json) 或 [Testnet](https://ton.org/testnet-global.config.json) ）连接到公共 Liteservers，也可以运行自己的 [Liteserver](/v3/documentation/nodes/overview) 并使用 [ADNL SDKs](/v3/guidelines/dapps/apis-sdks/api-types#adnl) 进行处理。

阅读更多关于 [Merkle 证明](/v3/documentation/dapps/proofs)的信息[TON白皮书](https://ton.org/ton.pdf) 2.3.10, 2.3.11。

公共 liteservers（来自全局配置）的存在是为了让你快速开始使用 TON。它可用于学习 TON 编程，或用于不需要 100% 正常运行时间的应用程序和脚本。

建设生产基础设施――建议使用准备完善的基础设施：

- [设置自己的 liteserver](/v3/guidelines/nodes/running-nodes/liteserver-node),
...
请求和对服务器的响应在 [TL](/v3/documentation/data-formats/tl) schema 中描述，它允许您为某个编程语言生成一个输入的接口。
