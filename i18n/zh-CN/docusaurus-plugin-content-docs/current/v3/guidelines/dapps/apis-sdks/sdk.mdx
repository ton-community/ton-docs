...
2. ADNL 连接：您正在连接一个 [liteserver](/v3/guidelines/nodes/running-nodes/liteserver-node)）。它们可能无法访问，但通过一定程度的验证（在库中实现），无法作恶。
...
| [tonkite/adnl](https://github.com/tonkite/adnl) | [ADNL](/v3/documentation/network/protocols/adnl/tcp) 本地/通过 WebSocket                                                                     | ADNL TypeScript 实现。                                                                 |
| [tonutils](https://github.com/thekiba/tonutils) | 本地 [ADNL](/v3/documentation/network/protocols/adnl/tcp)                                                                                  | 基于 TypeScript 的界面，用于构建 TON 生态系统中的应用程序并与之交互。由于依赖于本地 ADNL，因此不能用于浏览器中的区块链交互。           |
