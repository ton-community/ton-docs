...
1. New colors.
   新颜色。
   此外，还增加了许多新颜色。它们中的大多数在Android上最为明显。因此，下面的示例将基于Android展示，但适用于所有平台。 The examples focus on Android but apply to all platforms.
...
3. 已经提供了 token accent_text_color，它适用于应用程序中的任何突出元素。以前，大家都使用了不太合适的 dark link_color。 Previously, everyone used the less suitable dark link_color.
...
Use `hint_color` for hint sections under different sections. 我建议将它们用作部分下方的提示部分的颜色，以及 `secondary_bg_color` 这类背景的链接颜色。
