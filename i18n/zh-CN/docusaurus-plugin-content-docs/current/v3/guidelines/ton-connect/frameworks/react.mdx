...
推荐用于React应用程序的SDK是[UI React SDK](/v3/guidelines/ton-connect/guidelines/developers#ton-connect-react)。它是一个React组件，提供了与TON Connect交互的高级方式。
...
安装包之后，您应该为您的应用程序创建一个`manifest.json`文件。有关如何创建manifest.json文件的更多信息，请参阅[此处](/v3/guidelines/ton-connect/creating-manifest)。
...
- 在此获取更多示例：[准备信息](/v3/guidelines/ton-connect/cookbook/cells)
...
:::tip
了解如何签署和验证信息：[签名和验证](/v3/guidelines/ton-connect/verifying-signed-in-users)
:::
