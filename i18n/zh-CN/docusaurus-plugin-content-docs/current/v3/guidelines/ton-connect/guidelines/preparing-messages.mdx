...
要从 cell 中读取或解析数据，需要调用 `beginParse()` 函数。使用类似的`load...()` 函数，读取数据的顺序与存储数据的顺序相同： You read data in the same order it was stored using similar `load...()` functions:
...
每个 cell 都有 1023 位的限制。如果超过上限，就会发生错误： If you exceed this, an error occurs:
...
Cells can store optional values, which may be null. cell 可以存储可选值（可能为空）。这些值使用 `storeMaybe...()` 函数存储：
...
您可以使用相应的 loadMaybe...() 函数解析可选值。返回值可以为空，因此不要忘记检查它们是否为空！ Returned values are nullable so do not forget to check them for null!
...
The `body` for jetton transfers is based on the ([TEP-74](https://github.com/ton-blockchain/TEPs/blob/master/text/0074-jettons-standard.md#1-transfer)) standard. jetton 转帐信息函数示例。请注意，不同代币的小数位数可能不同：例如，USDT 使用 6 个小数位 (1 USDT = 1 _ 10\*\*6)，而 TON 使用 9 个小数位(1 TON = 1 _ 10\*\*9)。
...
Jetton 转账（[TEP-74](https://github.com/ton-blockchain/TEPs/blob/master/text/0074-jettons-standard.md#1-transfer)）的 `messageBody` 带有注释，我们应在常规转账的 `body` 之外将注释序列化，并将其打包到 `forwardPayload` 中。请注意，不同代币的小数位数可能不同：例如，USDT 使用 6 个小数位 (1 USDT = 1 _ 10\*\*9)，而 TON 使用 9 个小数位 (1 TON = 1 _ 10\*\*9)。  Please note that the number of decimals can vary between different tokens:
...
Jetton Burn 的 `body` 以 ([TEP-74](https://github.com/ton-blockchain/TEPs/blob/master/text/0074-jettons-standard.md#1-transfer)) 标准为基础。请注意，不同代币的小数位数可能不同：例如，USDT 使用 6 个小数位 (1 USDT = 1 _ 10\*\*9)，而 TON 使用 9 个小数位 (1 TON = 1 _ 10\*\*9)。 Please note that the number of decimals can vary between different tokens:
...
`buyAmount = Nftprice TON + 1.0 TON`。 The only important thing is the accurate TON amount, which is calculated as follows:
