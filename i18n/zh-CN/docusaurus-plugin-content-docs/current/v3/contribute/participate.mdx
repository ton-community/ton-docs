...
1. 阅读 [优秀教程原则](/v3/contribute/style-guide#concepts-and-guidelines) 来编写出色的教程 :)
2. 可以从源代码中以 [铸造你的第一个 Jetton](/v3/guidelines/dapps/tutorials/mint-your-first-token) 作为示例来找到灵感。
3. **设置环境**。[检查教程](/v3/contribute#online-one-click-contribution-setup) 是在本地或使用 Gitpod 运行你的 fork。
4. **编写教程**。用到相应的环境，查看教程在你的 fork 上的样子。
5. **发起 Pull Request**。打开 PR 以获取维护者的反馈。
6. 合并！
