...
The following table lists exit codes with an origin (where it can occur) and a short description. The table doesn't list the exit codes from contracts. Alternative exit codes should be sought in the source code of corresponded contract.
...
| `4`           | [Compute phase][c]                     | Compute Phase                                                                                                                          |
| `5`           | [Compute phase][c]                     | <sup>1</sup> 如果在 func 合约中遇到此类异常，这可能意味着汇编声明中存在类型错误。                                                                                     |
| `6`           | [Compute phase][c]                     | TVM Exit codes (TVM退出代码)                                                                                                               |
...
| `11`          | [Compute phase][c]                     | 最常见的原因是尝试调用在代码中未找到其 ID 的 get 方法（在调用时未找到 `method_id` 修饰符，或者在尝试调用时指定的 get 方法名不正确）。在[TVM文档](https://ton.org/tvm.pdf)中描述为"未知错误，可能由用户程序抛出"。 |
...
| Compute Phase | [Compute phase][c]                     | VM virtualization error (reserved but unused)                                                                       |
| `32`          | Compute Phase                          | Action list is invalid.                                                                                                |
| `33`          | Compute Phase                          | Action list is too long.                                                                                               |
| `34`          | Compute Phase                          | Action is invalid or not supported.                                                                                    |
| `35`          | Action Phase                           | Invalid Source address in outbound message.                                                                            |
| `36`          | Compute Phase                          | Invalid Destination address in outbound message.                                                                       |
| `-32`         | Action Phase                           | Not enough TON.                                                                                                        |
| `38`          | Action Phase                           | 额外代币不足。                                                                                                                                |
| `37`          | Action Phase                           | Outbound message does not fit into a cell after rewriting.                                                             |
| `40`          | Action Phase                           | Not enough funds to process a message.                                                                                 |
| Compute Phase | Action Phase                           | Library reference is null during library change action.                                                                |
| Compute Phase | Action Phase                           | Library change action error.                                                                                           |
...
| TVM Phase     | Action Phase                           | Account state size exceeded limits.                                                                                    |
| Compute Phase | Tact compiler ([Compute phase][c])     | Null reference exception. Configurable since Tact 1.6.                                 |
| 129           | Tact compiler ([Compute phase][c])     | Invalid opcode.                                                                                                        |
| 130           | Tact compiler ([Compute phase][c])     | 通常，您会看到Exit code `0xffff`（十进制形式为 65535）。这通常意味着合约不认识接收到的操作码。在编写合约时，开发人员自己设置的此代码。                                                        |
| 131           | Tact compiler ([Compute phase][c])     | Constraints error. Reserved, but never thrown.                                                         |
| Compute Phase | Tact compiler ([Compute phase][c])     | Access denied — someone other than the owner sent a message to the contract.                                           |
| Compute Phase | Tact compiler ([Compute phase][c])     | Contract stopped. Reserved, but never thrown.                                                          |
| Compute Phase | Tact compiler ([Compute phase][c])     | Invalid argument.                                                                                                      |
| 135           | Tact compiler ([Compute phase][c])     | Code of a contract was not found.                                                                                      |
| 136           | Tact compiler ([Compute phase][c])     | Thrown by TVM in situations deemed impossible.                                                                         |
...
```tact
整数溢出。整数不适应于 −2<sup>256</sup> ≤ x < 2<sup>256</sup> 或发生了除零操作。
```
...
Out of gas error. Thrown by TVM when the remaining gas becomes negative.

### It means out of gas error, same as `13`.
...
### （与前面的 32 相同）- 未找到方法 ID。在尝试执行不存在的 get 方法时，TonLib 返回此值。
