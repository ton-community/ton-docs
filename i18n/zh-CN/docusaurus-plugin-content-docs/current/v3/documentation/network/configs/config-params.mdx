...
我们假设读者已经熟悉[Fift](/develop/fift/overview)和[轻客户端](/v3/guidelines/nodes/running-nodes/liteserver-node)，如[FullNode-HOWTO (低级)](/v3/guidelines/nodes/running-nodes/full-node)和[Validator-HOWTO (低级)](/v3/guidelines/nodes/running-nodes/validator-node)中所述，其中描述了验证者为配置提案投票的部分。
