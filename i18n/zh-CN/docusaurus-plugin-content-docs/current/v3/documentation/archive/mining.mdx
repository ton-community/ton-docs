...
挖矿过程在很大程度上由`mytonctrl`自动化。关于挖矿过程的详细信息可以在PoW givers文档（已归档）中找到。

## <a id="advanced"></a>高级

如果您认真对待挖矿并希望操作多台机器/矿场，那么您真的需要了解TON以及挖矿的工作原理；请参阅[TON文档](/v3/documentation/ton-documentation)以获取深入信息。以下是一些通用建议：
...
- **应该**尝试了解挖矿过程的工作原理；大多数大型矿工使用自己的脚本，在多个挖矿机器的环境中提供比`mytonctrl`更多的优势。
...
这超出了本指南的范围，请参阅[运行完整节点](/v3/guidelines/nodes/running-nodes/full-node)和/或[mytonctrl说明](https://github.com/igroman787/mytonctrl)。

#### <a id="faq-software-build"></a>你能帮我在我的操作系统上构建软件吗？

这超出了本指南的范围，请参阅[运行完整节点](/v3/guidelines/nodes/running-nodes/full-node)以及[Mytonctrl安装脚本](https://github.com/igroman787/mytonctrl/blob/master/scripts/toninstaller.sh#L44)，以获取有关依赖项和过程的信息。
