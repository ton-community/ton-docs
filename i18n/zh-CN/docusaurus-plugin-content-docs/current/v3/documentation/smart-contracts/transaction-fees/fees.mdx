...
TON上的费用难以提前计算，因为它们的数量取决于交易运行时间、账户状态、消息内容和大小、区块链网络设置以及无法在交易发送之前计算的其他许多变量。阅读关于[计算费用](/v3/documentation/smart-contracts/transaction-fees/fees-low-level#computation-fees)的低层级文章概述。
...
根据[低层级费用概述](/v3/documentation/smart-contracts/transaction-fees/fees-low-level)，TON上的费用按照以下公式计算：
