...
如[TVM概述](/v3/documentation/data-formats/tlb/overview)中所述，交易执行包括几个阶段(phase)。在这些阶段期间，可能会扣除相应的费用。

## 交易及其阶段

如[TVM 概述](/v3/documentation/data-formats/tlb/overview)所述，交易执行由几个阶段组成。在这些阶段中，可能会扣除相应的费用。有一个 [高级费用概述](/v3/documentation/smart-contracts/transaction-fees/fees)。
...
`cell_price` 和 `bit_price` 均可从网络配置[参数 18](/v3/documentation/network/config-params/overview#param-18)中获取。
...
所有计算成本均以 gas 单位计算。 gas 单位的价格由[链配置](/v3/documentation/network/config-params/overview#param-20-and-21)决定（配置 20 用于主链，配置 21 用于基链），只有在验证者达成共识后才能更改。需要注意的是，与其他系统不同，用户不能设定自己的 gas 价格，也没有收费市场。
...
[`first_frac`](/v3/documentation/network/config-params/overview#param-24-and-25)是TON区块链的24和25参数（主链和工作链）的一部分。目前，这两个参数的值都设置为 21845，这意味着 `action_fee` 约为 `msg_fwd_fees` 的三分之一。如果是外部消息操作 `SENDRAWMSG`，`action_fee` 等于 `msg_fwd_fees`。
...
阅读有关 [重复数据删除](/v3/documentation/data-formats/cells/library) 的更多信息。
:::
...
- [事务和阶段](/v3/documentation/data-formats/tlb/overview#transactions-and-phases)
