...
The same happens in the reverse case, when two contracts, *B* and *C*, send a message to one contract, *A*. Even if message `B -> A` was sent before `C -> A`, we can't know which one will be delivered first. The `B -> A` route may require more shard chain hops.
