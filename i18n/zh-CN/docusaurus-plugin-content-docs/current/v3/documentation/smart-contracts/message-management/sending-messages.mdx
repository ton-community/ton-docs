...
消息的组成、解析和发送位于[TL-B schemas](/v3/documentation/data-formats/tlb/overview)、[交易阶段和TVM](/v3/documentation/tvm/overview)的交汇处。

事实上，FunC有[send_raw_message](/v3/documentation/smart-contracts/func/docs/stdlib#send_raw_message)函数，该函数期望一个序列化消息作为参数。
...
:::info cell大小
请注意，任何[Cell](/v3/documentation/data-formats/cells/overview)最多可包含`1023`位。如果您需要存储更多数据，您应该将其分割成块并存储在引用cell中。
:::
...
更多配置参数及其值可在 [这里](/v3/documentation/network/config-params/overview#param-43) 找到。
:::
