...
但如在全局变量[章节](/v3/documentation/smart-contracts/func/docs/global_variables)中提到的，不允许重新声明全局变量。
...
请注意，目前所有的一元和二元运算符都是整数运算符。逻辑运算符表示为位整数运算符（参见[没有布尔类型](/v3/documentation/smart-contracts/func/docs/types#absence-of-boolean-type)）。
...
请注意，条件 `x < 100` 的真值是类型为 `int` 的（参见[没有布尔类型](/v3/documentation/smart-contracts/func/docs/types#absence-of-boolean-type)）。
