...
本节描述了一些比之前文章中描述的语言结构更不基础的构造。它们可以在 [stdlib.fc](/v3/documentation/smart-contracts/func/docs/stdlib) 中定义，但这样会减少 FunC 优化器的操作空间。
...
- `null?` 检查参数是否为 `null`。对于 TVM 类型的 `null` 值，FunC 的 `Null` 表示某些原子类型值的缺失；参见 [null 值](/v3/documentation/smart-contracts/func/docs/types#null-values)。
