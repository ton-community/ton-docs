...
函数名可以是任何[标识符](/v3/documentation/smart-contracts/func/docs/literals_identifiers#identifiers)，也可以以 `.` 或 `~` 符号开头。这些符号的含义在[声明](/v3/documentation/smart-contracts/func/docs/statements#method-calls)部分解释。

例如，`udict_add_builder?`、`dict_set` 和 `~dict_set` 都是有效且不同的函数名。（它们在 [stdlib.fc](/v3/documentation/smart-contracts/func/docs/stdlib) 中定义。）

#### 特殊函数名

FunC（实际上是 Fift 汇编器）有几个预定义的保留函数名，具有预定义的[id](/v3/documentation/smart-contracts/func/docs/functions)。
...
当 [TVM 初始化](/v3/documentation/tvm/initialization) 时，栈上有一些变量，通过在 `recv_internal` 中设置参数，我们使智能合约代码能够了解其中的一些变量。那些代码不知道的变量将永远躺在栈底，从未被触及。
...
返回类型可以是[类型](/v3/documentation/smart-contracts/func/docs/types)部分中描述的任何原子或复合类型。例如，
...
请注意，尽管函数可能看起来像是多个参数的函数，实际上它是一个单一[张量类型](/v3/documentation/smart-contracts/func/docs/types#tensor-types)参数的函数。要了解差异，请参阅[函数应用](/v3/documentation/smart-contracts/func/docs/statements#function-application)。然而，参数张量的组成部分通常被称为函数参数。
...
例如，在 [stdlib.fc](/v3/documentation/smart-contracts/func/docs/stdlib) 函数中
...
:::info
注意，在手动重新排列的情况下，参数将按重新排列的顺序计算。要覆盖这种行为，请使用 `#pragma compute-asm-ltr`：[compute-asm-ltr](/v3/documentation/smart-contracts/func/docs/compiler_directives#pragma-compute-asm-ltr)
:::
