...
FunC 程序被编译成 Fift 汇编代码，生成对应的 [TON 虚拟机](/v3/documentation/tvm/overview) 字节码。

进一步地，这个字节码（实际上是 [cell树](/v3/documentation/data-formats/cells/overview)，就像 TON 区块链中的任何其他数据一样）可以用于在区块链中创建智能合约，或者可以在 TVM 的本地实例上运行。
...
开始开发和编译智能合约最方便快捷的方式是使用 Blueprint 框架。更多信息请参阅 [Blueprint](/v3/documentation/smart-contracts/getting-started/javascript) 部分。
...
- [环境设置页面](/v3/documentation/smart-contracts/getting-started/javascript)

:::info
同时，您始终可以从源代码构建二进制文件，如：
[FunC 编译器源代码](https://github.com/ton-blockchain/ton/tree/master/crypto/func)（阅读[如何从源代码编译](/v3/guidelines/smart-contracts/howto/compile/compilation-instructions#func) FunC 编译器）。
:::
...
:::tip 新手提示
开始使用 FunC 进行开发的最佳起点：[入门介绍](/v3/documentation/smart-contracts/overview)
:::
