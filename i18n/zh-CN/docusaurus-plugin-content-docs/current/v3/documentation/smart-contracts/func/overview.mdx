...
FunC is a domain-specific, C-like, statically typed language. FunC 是一种领域特定的、类 C 语言的、静态类型语言。
...
标准的基本智能合约，如钱包、选举人（在 TON 上管理验证）、多签名钱包等，都可以作为学习时的参考。 These can serve as helpful learning references.
