...
一个名为 "提名者池 "的智能合约可以让一个或多个提名者借出通币作为验证者的股份，并确保验证者只能将通币用于验证。此外，智能合约还保证了奖励的分配。
...
为了让提名人存款，他需要向提名人池智能合约发送带有通币的信息，并发送文本注释 "d"。
...
要提取资金，提名人需要向提名人池智能合约发送信息，并附上文本注释 "w "和一些用于支付网络费用的通币（1 通币即可）。除极少数情况外，信息中未使用的 TON 币将被退回。

如果提名者奖池余额中有足够的通币，将立即提款。如果提名人池已完成一轮验证，但尚未提交新一轮验证申请，则所有资金都将计入提名人池余额。

如果提名人池余额中没有足够的通币，则会向提名人发出 "提款请求"，通币将在本轮验证结束后被提取。
