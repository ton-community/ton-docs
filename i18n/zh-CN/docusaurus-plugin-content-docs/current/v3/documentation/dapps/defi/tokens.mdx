...
>
> 特定集合中的每个 NFT 都是一个独立的智能合约。每个用户的代币余额都存储在单独的用户钱包中。 The token balance for each user is stored in a separate user wallet.
>
...
Jettons are custom fungible tokens on TON Blockchain. Jettons 是 TON 区块链上的自定义可替代代币。您可以使用下面的 Jetton Deployer 示例在 TON 区块链上创建自己的代币：
