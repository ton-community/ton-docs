...
- 用
  FunC 编写的消费 RedStone Oracle数据的 Oracle 合约示例 [price_manager.fc](https://github.com/redstone-finance/redstone-oracles-monorepo/blob/main/packages/ton-connector/contracts/price_manager.fc)。它需要 [TVM Upgrade 2023.07](/v3/documentation/tvm/changelog/tvm-upgrade-2023-07)。 It requires [TVM Upgrade 2023.07](/v3/documentation/tvm/changelog/tvm-upgrade-2023-07).
...
传递 `signer_count_threshold` 。 There is also needed `signer_count_threshold` to be
passed.
...
。例如：
`'ETH'` 为 `int` ，十六进制为`0x455448`，十进制为`4543560`，即`256*256*ord('E')+256*ord('T')+ord('H')`。 For example:
...
saving/writing the aggregated values to the contract's storage. 无论采用哪种即时处理方式，也存在一种在链上处理 `payload` 的方法，但
会将汇总值保存/写入合约的存储空间。这些值会保存在合约的存储空间中，然后可以使用 `read_prices` 函数读取。使用 `read_timestamp` 函数可以读取上次保存/写入合约的数据的时间戳。 The timestamp of data last saved/written to the contract is able to read by using the `read_timestamp` function.

该方法必须以 TON 内部报文的形式调用。信息参数如下 The arguments of the message are:
...
初始化时已对其进行了配置。同时也会返回传入数据包的最小时间戳。 Returns also the min timestamp of the passed data packages.
...
需要传递要调用的 `price_feed`。 Works also with `single_feed_man`.
The `price_feed` to be called needs to be passed.

#### 初始化数据

Similar to the `price_feed` initial data. 与 `price_feed` 初始数据类似。由于 TON 合约的架构设计，初始数据需要与合约的存储结构相匹配，其构造如下：
...
，以便它们携带剩余的交易余额。 The message can be then fetched in the sender and processed or saved in the
sender's storage.
The initial payload's `ref` (`initial_payload`) is added as a ref - containing for example the first message's sender,
to allow they carry the remaining transaction balance.
