...
要安装和管理自己的节点，请使用 TON 基金会开发的**MyTonCtrl**开源工具。大多数 TON 节点都是可靠的，并通过 **MyTonCtrl** 进行了测试。 **MyTonCtrl** is reliable and tested on most TON nodes.
...
获取当前的 mytonctrl 和节点状态。参数组合： 以下参数的任意组合

| Format name | Format        | Example       | Description                                                                                                                     |
| ----------- | ------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| No args     | `status`      | `status`      | 完整的状态报告，包括验证器效率和在线验证器。                                                                                                          |
| Fast        | `status fast` | `status fast` | This must be used on Testnet. 必须在 TestNet 上使用。状态报告不含验证器效率和在线验证器计数。                                                              |
...
Sets the specified value for a given setting. 设置指定设置的指定值。如果启用 "强制"，则跳过设置存在性检查
...
无参数，退回到 mytonctrl 1.0。无论如何，你都不应该使用 mytonctrl 1.0。 **You should avoid using MyTonCtrl 1.0 under any circumstances**.
...
2. **Validator index is in the range [0, `max_main_validators`)**:
   整轮的 "验证器效率 "必须高于 90%（今后可根据统计数据更改这一数字）；否则，可能会受到处罚（罚款）。 Currently, no penalties will be applied; however, this may change in future updates.

3. **The user is not a validator**:
   不适用处罚，但用户也没有资格获得奖励。无 "验证器效率 "可查。这可能是由于质押过低或节点配置不正确造成的。此外，请确保 `mytonctrl` 持续运行。 In this case, there is no `validator efficiency` to monitor. This situation can arise from having a low stake or an incorrect node configuration. Additionally, ensure that `MyTonCtrl` is running continuously.

阅读更多关于 `max_validators` 和 `max_main_validators` [配置参数页面](/v3/documentation/network/configs/blockchain-configs#configuration-parameters-for-the-number-of-validators-for-elections)。获取 [mainnet](https://tonviewer.com/config#16) 和 [testnet](https://testnet.tonviewer.com/config#16) 的实际值。 Get the current values [for mainnet](https://tonviewer.com/config#16) and [for testnet](https://testnet.tonviewer.com/config#16).
...
3. 运行以下命令，将 `word1`, `word2`... 替换为您的记忆短语，将 `address` 替换为您的钱包合约地址：
   ...
   ```
4. 脚本将生成 `wallet.pk` 和 `wallet.addr`。将它们重命名为 `imported_wallet.pk` 和 `imported_wallet.addr`。 Rename them to `imported_wallet.pk` and `imported_wallet.addr`.
5. 将这两个文件复制到 `~/.local/share/mytoncore/wallets/` 目录。
6. 打开 mytonctrl 控制台，使用 `wl` 命令列出钱包。
7. 确认钱包已导入并显示正确余额。
8. 现在可以使用 `mg` 命令发送资金。输入 `mg` 查看帮助文档。
   运行命令时，请记住用实际值替换占位符（`< >`内的单词）。 Enter `mg` to view the help documentation.
...
| JSON with full hash | `ol --json hash` | 打印 `data` 的 JSON 表示形式。在这种情况下，"hash "参数不起作用。 The `"hash"` argument has no effect in this case. |
...
| Any combination of following args | `vl --json adnl pubkey wallet past` | 以 JSON 表示的完整 ADNL、Pubkey、钱包和过去的验证器条目。                                         |
...
| Any combination of following args | `cl --json adnl past` | 完整的 ADNL，包括以 JSON 表示的以往投诉。                             |
...
| Format name        | Format                                      | Description                                                                                       |
| ------------------ | ------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| 添加或替换参数            | `set_node_argument [-ARG_NAME] [ARG_VALUE]` | Add argument or replace it value if it exists. 添加参数或替换已存在的参数值。-ARG_NAME "必须以"-"或"--"开头            |
...
| `archive-ttl`      | ttl of blocks node stores. 节点存储块的 ttl。_要强制节点不存储归档块，请使用值 `86400`_                             | 如果在安装过程中启用了 liteserver 模式，则为 2592000，否则为 86400                                      |
...
No args. 无参数。为网络管理界面设置密码，运行 `python3 /usr/src/mtc-jsonrpc/mtc-jsonrpc.py -p`。
