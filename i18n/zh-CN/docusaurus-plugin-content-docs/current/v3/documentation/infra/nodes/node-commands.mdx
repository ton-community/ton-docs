...
本文档介绍运行 TON 节点时可用的各种标志和选项。每个标记都有一个短名称、一个长名称、一个默认值（如适用）以及对其功能的描述。 Each flag has a short name, a long name, a default value (if applicable), and a description of its functionality.
...
| `-I`, `--ip`            | 指定实例的 IP 地址和端口。首次运行时使用。 Used during the first run                                                       | N/A                            | `-I <ip:port>`              |
...
| `--max-archive-fd`                | 设置归档管理器中打开文件描述符的数量限制。`0` 表示无限制。 `0` for unlimited                                                                                                  | `0` (unlimited)        | `--max-archive-fd <number>`                  |
...
| `--celldb-in-memory`              | Stores the whole celldb in memory. 在内存中存储整个 celldb。对于使用默认设置的验证器，cellldb 的大小约为 80-100GB，因此 128GB 是绝对必要的，最好是 256GB。                                  | Disabled               | `--celldb-in-memory`                         |
