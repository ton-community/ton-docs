...
Let's say we want to check if any event is relevant. To do this, we use the flag variable. 何かのイベントが関連しているかどうかを確認したいとしましょう。これを行うには、flag 変数を使用します。 FunC の `true` は `-1` で、`false` は `0` であることを忘れないでください。
