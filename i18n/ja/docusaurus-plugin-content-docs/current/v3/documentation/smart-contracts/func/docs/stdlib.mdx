...
Lists in FunC are represented as nested two-element tuples. リストはネストされた 2 要素のタプルとして表現できます。空のリストは TVM の `null` 値として表されます(`null()` を呼び出すことで取得できます)。 例えば、タプル `(1, 2, (3, null))` はリスト `[1, 2, 3]` を表します。
...
計算されます。 More precisely, an unsigned random value `x` is generated as in `random`; then `z := x * range / 2^256` is
computed.
...
- `addr_std`は`t = (2, u, xs)`で表されます。 `u`は`null`（`anycast`が存在しない場合）または`rewrite_pfx`（`anycast`が存在する場合）を含むスライス`s'のどちらかです。 次に、整数`x`は`workchain_id`で、スライス`s\`はアドレスを含んでいます。 Next, integer `x`is the`workchain_id`, and slice `s\` contains the address
...
- `addr_extern` は`t = (1, s)`で表されます。スライス`s`は`external_address`の
  フィールドを持っています。 言い換えれば、`t`はペア（2つのエントリからなるタプル）で、1とスライスに等しい整数を含みます。 In other words, `t` is a pair with the first element being `1` and the second being `s`.
...
Checks whether a dictionary is empty. 辞書が空かどうかをチェックします。`cell_null?` と同等です。
