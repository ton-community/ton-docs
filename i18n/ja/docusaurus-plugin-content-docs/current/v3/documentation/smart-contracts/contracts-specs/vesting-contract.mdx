...
- `contracts` - プロジェクトのすべてのスマートコントラクトとその依存関係のソースコード。
- `wrappers` - [de]シリアライズプリミティブとコンパイル関数を含む、契約のラッパークラス (`Contract` をton-coreから実装) 。
- `tests` - コントラクトのテスト。
- `scripts` - プロジェクトで使用されるスクリプト、主にデプロイスクリプト。
...
- [シングルノミネーター](/v3/documentation/smart-contracts/contracts-specs/single-nominator-pool)
- [ベスティングコントラクト](https://github.com/ton-blockchain/vesting-contract)
