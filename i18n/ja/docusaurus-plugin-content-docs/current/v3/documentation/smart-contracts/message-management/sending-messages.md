...
TON は幅広い機能を備えた包括的なシステムであるため、この機能をすべてサポートする必要があるメッセージは非常に複雑に見える場合があります。ただし、その機能のほとんどは一般的なシナリオでは使用されず、ほとんどの場合、メッセージのシリアル化は次のように簡略化できます。 However, most of that functionality is not used in common scenarios, and message serialization, in most cases, can be simplified to:
...
したがって、開発者は心配する必要はありません。このドキュメントの内容が最初に読んだだけでは理解できないように思えても、大丈夫です。一般的な考え方だけを把握してください。 Just grasp the general idea.
