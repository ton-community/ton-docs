...
- `msg` - 暗号化される任意のバイト文字列。 `len(msg) <= 960` 。 `len(msg) <= 960`.
...
5. Calculate `x = hmac_sha512(shared_secret, msg_key)`. `x = hmac_sha512(shared_secret, msg_key)` を計算します。`key=x[0:32]` と `iv=x[32:48]` をみましょう。
...
`0xffffff`と`0xfffffe`に等しい`op`を持ついくつかの「標準」の応答メッセージがあります。 一般に、`0xfffffff0`から`0xffffffff`までの`op`の値は、そのような標準的な応答のために予約されています。 In general, the values of `op` from `0xfffffff0` to `0xffffffff` are reserved for such standard responses.
...
Notice that unknown "responses" (with an `op` in the range `2^31 .. 未知の「応答」（`op`で`2^31の範囲にある。） 2^32-1`) は無視されるべきです (特に `op`に対する応答は`0xffffffff\` に等しいものではありません) 予期しないバウンスされたメッセージと同じように(「バウンスされた」フラグがセットされている)。
