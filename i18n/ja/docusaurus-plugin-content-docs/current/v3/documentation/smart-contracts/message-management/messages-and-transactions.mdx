...
この種の通信を扱うことは、衛星を宇宙に打ち上げることを思い出させます。作成したメッセージはわかっていますが、結果を判断するには打ち上げ後の観察が必要です。 While we know the message we've created, observation after launch is required to determine the outcome.
...
1. 2 番目の可能な順序は `tx2'_lt < tx1'_lt`です:
...
2. 最初の可能な順序は `tx1'_lt < tx2'_lt`です:
