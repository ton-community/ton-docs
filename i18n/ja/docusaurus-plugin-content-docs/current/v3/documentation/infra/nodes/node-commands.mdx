...
このドキュメントでは、TON ノードを実行する際に利用できる様々なフラグとオプションについて説明します。 各フラグには、短い名前、長い名前、デフォルト値(該当する場合)とその機能の説明があります。 Each flag has a short name, a long name, a default value (if applicable), and a description of its functionality.
