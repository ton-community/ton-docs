...
Pingパケットは5秒に1回送信するのが最適です。これは、データが送信されていない間も接続を維持するために必要なことで、そうしないとサーバーが接続を終了してしまう可能性があります。 This is necessary to maintain the connection while no data is being transmitted, otherwise the server may terminate the connection.
...
Result`の中には `b5ee9c7201010501001b00020800000203010202030400080ccffcc1000000080aabbcc8\\` があり、これはデータを含む[BoC](/v3/documentation/data-formats/tlb/cell-boc#bag-of-cells)です。これをデシリアライズすると、セルが得られます： When we deserialize it, we will get a cell:
...
Note that they are in reverse order. 逆順であることに注意してください。同じように、関数を呼び出すときには引数を渡す必要があります。
...
In our case, n will be equal to 7. In len we will have `(#< 7)` which means the number of bits that can hold a number up to 7. 7-1=6を2進数-110`に変換すると3ビットになるので、長さlen=3ビットとなります。そして値は `(uint (len \* 8))\`となります。つまり、VarUIntegerの値を得るために読み込む必要があるビット数です。 And value is`(uint (len \* 8))`. To determine it, we need to read 3 bits of the length, get a number and multiply by 8, this will be the size of `value\`, that is, the number of bits that need to be read to get the value of VarUInteger.

cells:(VarUInteger7)`を読み、ルートセルから次のビットを取り、次の16ビットを見て理解する、これは`00100110101000`です。lenの最初の3ビットを読むと、これは`001`、つまり1です。サイズ（uint (1 * 8)）を求めると、uint 8となり、8ビットを読むと`cells`、`00110101`、つまり10進数で53となります。同じことを `bits`と`public_cells\`にも行います。 We read the first 3 bits of len, this is`001`, i.e. 1, we get the size (uint (1 * 8)), we get uint 8, we read 8 bits, it will be `cells`, `00110101`, i.e. 53 in decimal form. We do the same for `bits`and`public_cells\`.
...
`last_trans_lt:uint64`を読み込みます。これは64ビットで、最後の口座取引のltを格納します。最後に、スキーマで表される残高です： And finally, the balance represented by the schema:
...
Read first 4 bits - `1000`, this is 8. 8\*8=64、次の64ビットを読みます= `00000111011101001000111001110111001110110000001100000`、余分なゼロビットを取り除くと `111011101110100100011110111001110110000001100000`となり、これは`531223439883591776`に等しく、ナノからトンに変換すると`531223439.883591776`となります。
...
DHの本質は、秘密情報を開示することなく、共有秘密鍵を得ることです。これがどのように行われるのか、最も単純化した形で例を挙げます。我々とサーバーの間で共有鍵を生成する必要があるとすると、そのプロセスは次のようになります： I will give an example of how this happens, in the most simplified form. Suppose we need to generate a shared key between us and the server, the process will look like this:
