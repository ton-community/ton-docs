...
お勧めの React Apps は [UI React SDK](/v3/guidelines/ton-connect/guidelines/developers#ton-connect-react) です。 これは、TON Connect とやり取りする高レベルの方法を提供する React コンポーネントです。 It is a React component that provides a high-level way to interact with TON Connect.
...
モーダルウィンドウを開閉するための関数にアクセスするには、このフックを使用します。 フックは、モーダルを開閉するための現在のモーダル状態とメソッドを持つオブジェクトを返します。 The hook returns an object with the current modal state and methods to open and close the modal.
...
`tonConnectUI.setConnectRequestParameters` 関数を使用して、接続リクエストパラメータを設定します。 You can use it for:
