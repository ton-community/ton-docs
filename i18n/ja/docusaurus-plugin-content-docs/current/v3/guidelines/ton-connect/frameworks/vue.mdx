...
アニメーションが動作しない場合に問題が発生している場合 ウェブアニメーション API のサポートがないためかもしれません。 この問題を解決するには、 `web-animations-js` polyfill を使用します。 To resolve this issue, you can use the `web-animations-js` polyfill.
