...
前日または前々日の統計を表示したい場合は、オプションとして数値を渡します。
...
ネットワークインターフェース名で結果をフィルタリングしたい場合

列 `%ifutil` の結果を確認してください。これは、最大リンク速度を考慮したインターフェイスの使用状況を示しています。

以下のコマンドを実行すると、NIC でサポートされている速度を確認できます。
