...
まず、スワップするJettonの**vault**アドレス、またはToncoinの**vault**アドレスを知る必要があります。これはコントラクト[**Factory**](https://docs.dedust.io/reference/factory)の`get_vault_address`取得メソッドを使用して行うことができます。引数として、スキームに従ってスライスを渡す必要があります： This can be done using the `get_vault_address` get method of the contract [**Factory**](https://docs.dedust.io/reference/factory). As an argument you need to pass a slice according to the scheme:
...
This was swapping TON with jetton SCALE. これはTONとjetton SCALEを入れ替えたものです。jettonとjettonを交換するプロセスは同じです。唯一の違いは、TL-Bスキーマに記述されているペイロードを提供しなければならないことです。
...
アカウントのトランザクションのリストは `getTransactions` API メソッドで取得することができます。これは `Transaction` オブジェクトの配列を返し、各項目は多くの属性を持ちます。しかし、最もよく使われるフィールドは以下のとおりです： It returns an array of `Transaction` objects, with each item having lots of attributes. However, the fields that are the most commonly used are:
