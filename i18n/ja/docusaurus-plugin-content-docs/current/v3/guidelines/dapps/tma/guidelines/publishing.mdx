...
デフォルトでは、ボットとのチャットには、リストされたすべての[コマンド](https://core.telegram.org/bots#commands)にすばやくアクセスできる便利な**メニューボタン**が常に表示されます。Bot API 6.0](https://core.telegram.org/bots/api-changelog#april-16-2022)では、このボタンを **ミニアプリ** の起動に使用できます。 With [Bot API 6.0](https://core.telegram.org/bots/api-changelog#april-16-2022), this button can be used to **launch a Mini App** instead.
...
インラインミニアプリはチャットに**アクセス**できません - ユーザーの代わりにメッセージを読んだり、新しいメッセージを送信することはできません。メッセージを送信するには、ユーザーが**インラインモード**にリダイレクトされ、アクティブに結果を選択する必要があります。 Users must switch to inline mode to send messages.
