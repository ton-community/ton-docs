...
- デバイスの[USB-Debuggingを有効にします](https://developer.chrome.com/docs/devtools/remote-debugging/)。
...
- Ios Telegramクライアント内のミニアプリを開きます。
...
- _設定 > 詳細設定 > 実験的設定 > ウェブビュー検査_を有効にします。
- windowsとLinuxでWebViewを右クリックし、_インスペクション_を選択します。
