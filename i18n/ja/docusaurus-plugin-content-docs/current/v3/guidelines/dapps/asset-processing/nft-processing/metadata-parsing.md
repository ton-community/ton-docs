...
| `image_data`  | binary\* | 任意   | Either a binary representation of the image for the on-chain layout or base64 for the off-chain layout. |

## Jettonのメタデータ属性

1. `uri` - Optional. Used by "Semi-chain content layout". ASCII string. `uri` - 任意。「セミチェーンコンテンツレイアウト」で使用されるASCII文字列で、メタデータを含むJSONドキュメントを指すURIです。
2. `image` - Optional. ASCII string. A URI pointing to a resource with a mime type image.
...
6. `symbol` - Optional. UTF8 string. `symbol` - 任意。UTF8文字列。トークンのシンボル - 例えば"XMPL"。"You received 99 XMPL"という形式で使用されます。 Used in the form "You received 99 XMPL".
...
| `description`  | UTF8文字列  | 任意   | Describes the asset                                                                                                                                                                                                                                                                                                                                                   |
| `image`        | ASCII文字列 | 任意   | A URI pointing to a resource with a mime type image                                                                                                                                                                                                                                                                                                                   |
| `image_data`   | binary\* | 任意   | Either a binary representation of the image of an on-chain layout or base64 for off-chain layout                                                                                                                                                                                                                                                                      |
...
| `amount_style` |          | 任意   | Required by external applications to understand the format of displaying the number of tokens. Defined using _n_, _n-of-total_, _%_.                                                                                                                                                                                                  |
...
After on-chain NFT data is retrieved, it must be parsed. オンチェーンのNFT データを取得した後は、それを解析する必要があります。この処理を行うには、NFT の内部構造を構成する最初のバイトを読み取ることにより、NFT のコンテントタイプを判定する必要があります。
