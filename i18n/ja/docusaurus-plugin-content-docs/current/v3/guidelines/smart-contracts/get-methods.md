...
Finally, we can use this method in our tests. 最後に、テストでこのメソッドを使用できます。`tests/Counter.spec.ts`に移動して、新しいテストを追加します。
...
簡単のため、操作コードには単純な小さな数字1、2、3だけを使用しました。しかし、実際のプロジェクトでは、標準に従って設定してください： But for real projects, consider setting them according to the standard:
