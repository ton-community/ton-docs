...
`gas_used` を計算するには、以下のようにコントラクトをテストする必要があります。
...
メッセージ構造が決定論的な場合は、以下のパラメータを含むオペコードを使用します。
...
- [手数料計算付きステーブルコインコントラクト](https://github.com/ton-blockchain/stablecoin-contract)
