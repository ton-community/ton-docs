...
This article describes wallets from a developmental perspective. この記事では、開発の観点からウォレットについて説明します。最終的な目標は、TONの大量採用をサポートするウォレットアプリケーションを作成することです。
...
ハードウェア財布は、インターネットから離れてあなたの暗号通貨資金への秘密鍵を保存する物理的なデバイスです。 たとえ取引を行ったとしても、ウォレットはオフライン環境でトランザクションを確認します。 このプロセスは、秘密鍵を常にインターネットのリスクから遠ざけるのに役立ちます。 Even if you make transactions with it, the wallet confirms the transactions in an offline environment. This process helps keep your private keys away from the risks of the internet at all times.
...
[@wallet](https://t.me/wallet) — TelegramでP2Pを使ってTNを送受信またはトレードするボットです。Telegram MiniアプリUIをサポートしています。 Supports Telegram Mini App UI.
