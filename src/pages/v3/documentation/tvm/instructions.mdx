...
You can skip this section and return later.
:::
...
- [**TVM.pdf**](https://docs.ton.org/tvm.pdf) concept document for TON Virtual Machine (may include outdated information).
...
A description of Fift, including an introduction to the Fift assembler, can be found in the [Fift overview](/v3/documentation/smart-contracts/fift/overview).

This document specifies the corresponding mnemonic for each instruction.

Note the following:

1. Fift is a stack-based language, therefore all the arguments of any instruction are written before it (e.g. [`5 PUSHINT`](/v3/documentation/tvm/instructions), [`s0 s4 XCHG`](/v3/documentation/tvm/instructions)).
...
1. _Parsing cells_: Transforming a cell into a slice costs **100 gas units** when a cell is being loaded for the first time and **25** for subsequent loads during the same transaction. For such instructions, two gas prices are specified (e.g. [`CTOS`](/v3/documentation/tvm/instructions): `118/43`).
2. _Cell creation_: **500 gas units**.
3. _Throwing exceptions_: **50 gas units**. In this document, the exception fee is only specified for an instruction if its primary purpose is to throw (e.g., [`THROWIF`](/v3/documentation/tvm/instructions)). If the instruction only throws in some cases, two gas prices are specified (e.g. [`FITS`](/v3/documentation/tvm/instructions): `26/76`).
...
:::info
A full machine-readable list of TVM instructions is available [here](https://raw.githubusercontent.com/ton-community/tvm-spec/master/match-report.json).
:::

Each section includes a built-in search for its opcodes; on this page, the search covers all existing opcodes across the full set.
...
- [Back to TVM overview](/v3/documentation/tvm/tvm-overview)
