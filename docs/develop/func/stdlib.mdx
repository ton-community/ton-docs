...
```
Loads a serialized amount of Toncoins (any unsigned integer up to `2^128 - 1`).
...
```
Computes the minimum key `k` in dictionary `dict`, removes it, and returns `(dict', x, k, -1)`, where `dict'` is the modified version of `dict` and `x` is the value associated with `k`. If the dict is empty, returns `(dict, null, null, 0)`.
...
```
Computes the maximum key `k` in dictionary `dict`, removes it, and returns `(dict', x, k, -1)`, where `dict'` is the modified version of `dict` and `x` is the value associated with `k`. If the dict is empty, returns `(dict, null, null, 0)`.
