...
```js
    ...
    const messageBody = beginCell()
        ...
        .storeUint(quiryId, 64)                   // query_id:uint64
        .storeCoins(toNano("1"))                  //amount:(VarUInteger 16)
        .storeAddress(Jetton_Wallet_DST)          //destination:MsgAddress
        .storeAddress(Wallet_OWNER)               //response_destination:MsgAddress
