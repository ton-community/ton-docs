...
If load on the shard exceeds [certain level](https://docs.ton.org/develop/blockchain/sharding-lifecycle#algorithm-for-deciding-whether-to-split-or-merge) shard splits. And the goal is to provide dedicated computational resources to the contract under load and isolate it's impact on the whole network.
...
In order to understand how this works, one should understand how contract address [depends](https://docs.ton.org/learn/overviews/addresses#account-id) on it's code and data.
...
It has `salt` data field, the only purpose of which is to *bruteforce* the value of it, that results in the desired address pattern.
