...
```func   
get (cell) get_prices_v2(cell data_feed_ids, cell payload);
```
...
That are just `get` functions - they don't modify the contract's storage and don't consume TONs.
...
```func   
get (tuple) read_prices(tuple data_feed_ids);
```
...
That's just a `get` function - it doesn't modify the contract's storage and don't consume TONs.

#### read_timestamp

```func
get (int) read_timestamp();
```
...
```func
get (int, int) get_price_and_timestamp();
```
...
```func
get (int, int) get_price(cell payload);
```
...
```func
get (int, int) read_price_and_timestamp();
```
