...
You shouldn't run any type of node on your local machine for long, even if it is satisfy requirements. Nodes actively use disks and can destroy them fast.
:::
...
If you don't have **non-root** user, you can create this with the following steps steps (otherwise skip first two steps and go to the third).
