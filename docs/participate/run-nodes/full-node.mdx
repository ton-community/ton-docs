...
Sometimes we get `block is not applied` or `block is not ready` for various requests - is this normal?

This is normal; typically, this means you tried to retrieve the block that did not reach the node you asked for.
