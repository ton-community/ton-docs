...
Read more about `max_validators` and `max_main_validators` [in config parameters page](https://docs.ton.org/develop/howto/blockchain-configs#configuration-parameters-for-the-number-of-validators-for-elections). Get actual values [for mainnet](https://tonviewer.com/config#16) and [for testnet](https://testnet.tonviewer.com/config#16).
