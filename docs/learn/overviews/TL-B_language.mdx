...
In this example, `transfer#4034a3c0` will be serialized as a 32-bit unsigned integer from the hex value after the hash sign(`#`). This meets the standard requirements for an `op` in the [smart contract guidelines](https://ton.org/#/howto/smart-contract-guidelines).

To meet the standard described in paragraph 5 of the [smart contract guidelines](https://ton.org/#/howto/smart-contract-guidelines), it is not enough for us to calculate the CRC32. You can utilise the following examples to define an `op` in requests or responses from smart contracts in a TL-B scheme:
