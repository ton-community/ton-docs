...
:::info
For instance, if you store the `jetton-wallet` code as a library cell (approximately 1 cell with 256 + 8 bits, instead of approximately 20 cells and approximately 6000 bits), the forwarding fees for a message that includes `init_code` can be reduced from approximately 0.011 TON to approximately 0.003 TON (see [fee calculation](/v3/guidelines/smart-contracts/fee-calculation.mdx)).
:::
...
A library cell is an [exotic cell](/v3/documentation/data-formats/cells/exotic#library-reference) that references another static cell by storing its 256-bit hash.
...
To be usable within the MasterChain library context—i.e., to be found and loaded by a library cell—a source cell must be published. This is done by storing the cell within a MasterChain smart contract with the public flag (2). The opcode used for this is `SETLIBCODE`.
...
In particular, the forwarding fee for an `internal_transfer` message containing `init_code` drops from 0.011 TON to 0.003 TON—a roughly 3–4× reduction.
...
You can manually create a library cell in Fift assembly by writing its tag and hash to a builder and closing it as an exotic cell.

For example, see a Fift assembly construction in [order_code.func](https://github.com/ton-blockchain/multisig-contract-v2/blob/master/contracts/auto/order_code.func) and an example of compiling a contract directly to a library cell in [Order.compile.ts](https://github.com/ton-blockchain/multisig-contract-v2/blob/master/wrappers/Order.compile.ts).
...
### Using in TypeScript (@ton/core)

You can construct a library cell entirely in TypeScript using the `@ton/core` library without Fift. Here’s how to do it in a Blueprint project:

Here, `jwallet_code_raw` is a `Cell` containing the compiled `jetton-wallet` code.
...
The core of this contract is the line: `set_lib_code(lib_to_publish, 2);`. This function call publishes an ordinary cell with the public flag (2), which indicates that the library is public and can be used by anyone.

**Note:** the contract that publishes the cell is responsible for paying its own storage fees and the MasterChain’s storage fees. Storage costs in the MasterChain are approximately 1000 times higher than in the BaseChain. Therefore, using a library cell is only cost-effective for contracts that thousands of users utilize.
...
With the **liteserver** (LS), all these processes happen automatically behind the scenes, and users needn’t be concerned with the specific method of code storage.
...
You can review regular contracts' available get methods, the contract interface, to understand how they work. Alternatively, you may "steal" the account state to your local pseudo-net and execute methods there.
...
When running get methods with LiteServer, the correct library context is automatically set. If you need to detect the contract type using get methods or run them locally, download the corresponding cells via the LiteServer method [liteServer.getLibraries](https://github.com/ton-blockchain/ton/blob/4cfe1d1a96acf956e28e2bbc696a143489e23631/tl/generate/scheme/lite_api.tl#L96).
...
as well as a list of libraries for a specific MasterChain block:
...
- [Exotic cells](/v3/documentation/data-formats/cells/exotic)
- [TVM instructions](/v3/documentation/tvm/overview)
