...
A library cell is an [exotic cell](/v3/documentation/data-formats/cells/exotic) that references another static cell by storing its 256-bit hash.
...
During intraction with **liteserver (LS)** all these processes happen automatically behind the scenes, and users needn’t be concerned with the specific method of code storage.
...
This approach is not feasible for a library cell because it does not store data on its own. Instead, you must manually detect and retrieve the necessary cells from the context. This can be done using toncenter, LS or via DTon.
#### Retrieving library cell with TON Center
[getLibraries method](https://toncenter.com/api/v2/#/blocks/get_libraries_getLibraries_get) - accepts list of base64 encoded libraries hashes and returns cell bocs.
