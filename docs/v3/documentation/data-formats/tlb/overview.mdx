...
**Type Language - Binary (TL-B)**,  a version of [Type Language](/v3/documentation/data-formats/tl), that serves to describe serialization of objects in TON Blockchain.

## Overview

A set of TL‑B constructs is referred to as a TL‑B document. A typical TL‑B document consists of type declarations (constructors), and functional combinators. Each combinator declaration ends with a semicolon `;`.
...
When parsing the binary string `1000000000000000000000000000000001`—a `1` followed by 32 zeros and ending with another `1`—using TL‑B type `A`, we first extract the initial two bits to determine the tag.
...
- `_ a:^Owner = NFT;` - `NFT.a` is a cell reference to type `Owner` — i.e., the `Owner` is stored in the next referenced cell.
...
- `uint X` / `int X` / `bits X` - same as `uintX` but can use a parameterized `X` in this types
...
Allowed constraints: `E` | `E = E` | `E <= E` | `E < E` | `E >= E` | `E > E` | `E + E` | `E * E` | `E ? E`
...
This means that `A` is parameterized by `x`, where `x` is a `Nat`. During the deserialization process, we will fetch an `x`-bit unsigned integer. For example:
...
You can also use partial applications with such parameterized types:
...
You can even apply partial application to parameterized types themselves:
...
### NAT fields usage for parameterized types
...
### Expression in parameterized types
