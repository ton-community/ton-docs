...
<ThemedImage
  alt="Diagram of a TON cell: bits and references"
  sources={{
...
<ThemedImage
  alt="Diagram of Bag of Cells (BoC) serialization"
  sources={{
...
<ThemedImage
  alt="Overview diagram: code, data, and blocks built from cells"
  sources={{
...
<ThemedImage
  alt="Diagram: basic BoC serialization example"
  sources={{
...
This byte equals the number of 4‑bit groups (nibbles) in the cell’s bitstring: `ceil(b / 4)`.
...
```text
0 1[8_]      -> 02 01 -> references 2 cells with these indexes
1 7[FE]      -> 02 -> references the cell with index 2
2 24[0AAAAA] -> no refs
```
...
```
b5ee9c72                      -> TL‑B identifier of the BagOfCells structure
...
Where i indexes references of c, and e is the number of references of c (reference count).
...
- [Special (exotic) cells](/v3/documentation/data-formats/tlb/exotic-cells)
- [Verifying Merkle proofs](/v3/documentation/data-formats/tlb/proofs)
