...
This example is explained clearly in the [TL-B types](/v3/documentation/data-formats/tlb/tl-b-types#unary) article. The main idea here is that `UnaryChain` recursively deserializes until it reaches `unary_zero$0` because we know the last element of the `Unary X` type by the definition `unary_zero$0 = Unary ~0`. The value of `X` is calculated at runtime due to the definition `Unary ~(n + 1)`.
