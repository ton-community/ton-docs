...
We recommend reading [this documentation](/v3/documentation/data-formats/tlb/overviewe) to familiarize yourself with the topic and get started.
...
To determine the value of `l`, we need to load the `label:(HmLabel ~l 8)` sequence. Below, we outline the 3 basic structural options for `HmLabel`:
