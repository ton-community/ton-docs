...
It requires syncing key blocks and/or validating block signatures. This topic is out of scope for this article.
...
```
We should deserialize the cell according to the block [TL‑B scheme](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/block/block.tlb#L442):
...
Next, we need to verify that the `seqno` in the deserialized block matches the `seqno` of the block we know. After that, compute the representation hash (`get_hash(0)`) of the referenced cell from the Merkle proof and compare it to the block hash.

```python
h_proof = Cell.from_boc(header_proof)[0]
...
The first root is a MasterChain block Merkle proof, which must be verified using the `check_block_header_proof` function:
...
This is a Merkle update of the [ShardState](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/block/block.tlb#L412-L413) TL‑B schema. The resulting new hash should be stored.
...
The check_block_header_proof function returns the new hash of the ShardState Merkle Update.
"""

mc_state_hash = mc_state_root[0].get_hash(0)
state_hash = check_block_header_proof(mc_block_cell[0], block_id.root_hash, True)
...
Since this cell is trusted, we can also trust the shard block data (`ShardStateUnsplit` -> `custom` -> `shard_hashes` -> `0 (workchain 0)` -> `leaf`).
...
As in the `shard_proof` verification, the `check_block_header_proof` function must validate the block cell and record the new `StateUpdate` hash.
...
state_hash = check_block_header_proof(proof_cells[0][0], block_id.root_hash, True)
...
Compare the `Hash_1` from the Merkle proof (reference only) with the hash obtained from the `check_block_header_proof` function above. If they match, the cell can be trusted:
