...
- Assigns new indices for empty references
...
- Iterate over the root cells again. For each root:
  ...
  - If the new sum exceeds the root cell’s weight, the node is considered a special (exotic) node with a weight of 0. Set the weight to 0 accordingly. At this point, increment the Internal hashes count by the hash count of the node.

- Iterate over the root cells once more. For each root:
  - If the node is not special (exotic) (i.e., weight > 0), increment the Top hashes count by the node’s hash count.

- Recursively reindex the tree:
  ...
  - Root nodes are added at the end of the list, thus receiving the highest indices.

- The result is a sorted list in which deeper nodes receive lower indices.
...
<<<<<<< HEAD
- A special (exotic) cell has weight `0`.
...
- A special (exotic) cell has weight 0.
- On import, ensure that the weight fits within 8 bits (`weight <= 255`).
- Internal hashes count is the sum of hash counts of all cells with final weight 0.
- The top hashes count is the sum of hash counts of root cells with weight > 0.
>>>>>>> pitaya/doc-fix/run_20250901_144329_2bdedc55/kf6cb4b59
