...
| `-l`, `--logname`       | Specifies the log output file.                                                                           | N/A                            | `-l <file>`                 |
...
| `-K`, `--key-proof-ttl` | Sets the TTL for key proofs in seconds.                                                                 | `315360000` seconds (10 years) | `-K <seconds>`              |
...
| Option                            | Description                                                                                                                                                 | Default                | Usage                                        |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- | -------------------------------------------- |
| `--shutdown-at`                   | Schedules the node to shut down at the given Unix timestamp.                                                                                                | N/A                    | `--shutdown-at <timestamp>`                  |
...
| `--disable-rocksdb-stats`         | Disables the gathering of RocksDB statistics.                                                                                                               | Enabled (stats gathering) | `--disable-rocksdb-stats`                    |
...
| `--celldb-direct-io`              | Enables direct I/O mode for RocksDB in CellDB (only applies when cache size is ≥ 30 GB).                                                                    | Disabled               | `--celldb-direct-io`                         |
| `--celldb-preload-all`            | Preloads all cells from CellDB on startup.                                                                                                                  | Disabled               | `--celldb-preload-all`                       |
| `--celldb-in-memory`              | Stores the entire CellDB in memory. For validators with default settings, the size of `celldb` is ~80–100 GB, so 128 GB is required and 256 GB is preferred. | Disabled               | `--celldb-in-memory`                         |
