...
Each ShardChain and MasterChain has a dedicated set of validators. Sets of MasterChain validators consist of up to 100 validators exhibiting the highest voting weight (defined as Network Parameter `ConfigParam 16: max_main_validators`).

Each ShardChain is validated by 23 validators, as defined by Network Parameter `ConfigParam 28: shard_validators_num`. These validators are rotated randomly every 1000 seconds according to Network Parameter `ConfigParam 28: shard_validators_lifetime`.
...
1. **Initial selection**: Elector considers all candidates who have staked more than a set minimum amount (300k TON, as specified in the [configuration](https://tonviewer.com/config#17)).
...
- The Elector then evaluates each potential group of candidates, starting from the largest group and moving to smaller ones:
  ...
  - For each candidate, Elector calculates their **effective stake**. If a candidate's stake is significantly higher than the minimum, it's adjusted down (e.g., if someone staked 310k TON and the minimum is 100k TON, but there's a rule capping at three times the minimum, their effective stake is considered as 300k TON).
...
1. Elector takes all applicants who have a stake higher than the minimum ([300k TON in config](https://tonviewer.com/config#17)).
...
3. If there are more participants than the [maximum number](https://tonviewer.com/config#16) of validators, Elector discards the tail of the list. Then Elector does the following:
   ...
   - It calculates the effective stake, considering the `max_factor`. That is, if a person has put in 310k TON, but with a `max_factor` of 3, and the minimum stake in the list is 100k TON, then the effective stake will be min(310k TON, 3*100k TON) = 300k TON. One validator node may use up to 600k TON (in this example) in two rounds (half in odd rounds, half in even rounds). To increase the stake, it is necessary to set up multiple validator nodes.
...
Similarly to all blockchain networks, each transaction on TON requires a computation fee called [gas](https://blog.ton.org/what-is-blockchain) to process transactions and store data on-chain. On TON, these fees are accumulated within the Elector contract in a reward pool.
...
After a validation cycle lasting 65536 seconds, or approximately 18 hours (as determined by the network parameter `ConfigParam 15: validators_elected_for`), staked TON is not immediately released by each validator. Instead, it is held for an additional 32768 seconds, or about 9 hours (as specified by the network parameter `ConfigParam 15: stake_held_for`). During this period, slashing penalties can be imposed on the validator as a consequence for any misbehavior. Once the funds are released, validators can withdraw their staked amount along with a share of the rewards accrued during the validation round, proportional to their voting **weight**.

 

The total supply of Toncoin (5 billion TON) has an inflation rate that can vary over time.

This inflation rate, however, is not always constant and may deviate depending on the network’s current state. Eventually, it will tend to deflate after the deflationary burn mechanism is activated and network utilization grows.
...
On the TON blockchain, there are generally two ways validators can be penalized for misbehaving: **idle** or **malicious** misbehavior. Both are prohibited and may result in fines (in a process called slashing) for their actions.

If a validator fails to participate in block creation and transaction signing for a significant period during a validation round, they may incur a fine based on the **Standard fine** parameter. As of April 2023, the Standard fine that can be accrued is 101 TON (Network Parameter `ConfigParam 40: MisbehaviourPunishmentConfig`).
...
TON provides the [lite-client](https://github.com/ton-blockchain/ton/tree/master/lite-client) utility. In lite-client, there is a `checkloadall` command.

This command analyzes the number of blocks the validator should have processed and the number it actually processed in a given period of time.
...
- Validation of complaints is maintained by validators and is fully decentralized.
...
During this time, anyone can send a complaint against a validator who performed poorly in that round. This happens on-chain on the Elector smart contract.
...
The amount of the fine is defined by Network Parameter `ConfigParam 40: MisbehaviourPunishmentConfig` and is currently 101 TON (approximately equal to a validator’s income per round).
...
The fine is distributed among the validators minus network costs, and a small reward is given to the first complainant who sends the correct complaint to the Elector.
...
Please ensure you comply with the [validator guidelines](/v3/guidelines/nodes/running-nodes/validator-node#important-measures).
