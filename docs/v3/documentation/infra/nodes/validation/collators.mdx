...
:::caution
This section provides instructions and guidance for interacting with TON node at a low level.
...
**Accelerator** is update designed to improve blockchain scalability. Its main features are:
...
:::note
Current value of `monitor_min_split` in Mainnet is `0`, which means that all shards are in one group. That actual value can be always checked in [Config Param 12](/v3/documentation/network/configs/blockchain-configs#param-12).
:::
...
1. **DO NOT** add these flags to a node that is participating in validation. Currently, validators are required to monitor all shards; this will be improved in future updates so that they will be able to monitor only the MasterChain.
...
In the current `master` node branch, validators must still monitor all shards. However, you can launch collator nodes and configuring your validators to collate through them.
...
By default, validators proposing new block in validator set do not attach data that proves "prior to block" state. This data should be obtained by other validators from locally stored state. That way old and new nodes may reach consensus, but new validators should keep eye on all network state.
