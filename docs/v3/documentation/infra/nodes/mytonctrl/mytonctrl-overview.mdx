...
1. **Validator index in the range [0, `max_main_validators`)**:
   The `validator efficiency` must remain above 90% (this percentage may be adjusted in the future based on statistics) for the entire round. If it falls below this threshold, a penalty (fine) may be imposed.
