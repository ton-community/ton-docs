...
This document specifies the two standard address formats used on the TON Blockchain:
- The raw format
- The user-friendly format

The choice of format has significant implications for security and message handling.

## Raw address

The raw address is the canonical, on-chain representation of a smart contract's location.<br/>
It is designed for system-level use where precision is critical and UX features are unnecessary.
...
For example, `-1` for the Masterchain and `0` for the Basechain.
...
The raw address format lacks key safety features, making it unsuitable for general use:

-   **No error detection**: The format includes no checksum.<br/>
A single-character transcription error will result in an irrecoverable loss of funds.
-   **No metadata**: It does not support flags, such as `isBounceable`, which prevents nuanced control over message processing.

Due to these limitations, raw addresses should not be exposed to end-users.

## User-friendly address

The user-friendly address format is a secure, Base64-encoded wrapper around the raw address. It incorporates flags and a checksum to prevent common errors and provide greater control over message routing.
...
Learn more about [non-bounceable messages](/v3/documentation/smart-contracts/message-management/non-bounceable-messages) and their use cases in TON.
...
When developing custom solutions on the TON blockchain, it is critical to implement proper address handling logic. First, always verify whether the recipient address is initialized before sending funds to prevent unintended losses.
...
-   **SDKs**: See `Address.spec.ts` in the [ton-core](https://github.com/ton-org/ton/tree/master/ton-core) library for conversion examples.<br/>
It's also possible to make use of similar mechanisms using other [SDKs](/v3/guidelines/dapps/apis-sdks/sdk).
