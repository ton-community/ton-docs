...
After each validation round, the pool recovers its stake plus rewards from the elector contract. Rewards are split as:
...
Nominators receive shares proportional to their stakes. For example, if there are two nominators in the pool with stakes of 100k and 300k TON, then the first one will take 25% and the second 75% of the `nominators_reward`.
...
3. Pool design ensures validator funds always cover maximum possible fines
...
Send message with comment "d" and amount ≥ `min_nominator_stake + 1 TON`.
...
Send message with comment "w" and 1 TON for fees:

- Executes immediately if funds available
...
:::info
The loss of one participant's private key has no impact on other participants in the pool.
:::

## Emergency operations

If validator becomes inactive, any party can send operational messages such as `process withdraw requests`, `update current validator set`, `new_stake`, `recover_stake` to enable nominator withdrawals. The validator software `mytonctrl` does this automatically.
...
3. `stake_amount_sent` (nanotons)  
   Current validation stake amount
...
12. `stake_at` (uint)  
    Next validation round ID supposed to start at `utime_since`
...
Returns detailed nominator information for specified address.
>Address must be in raw format without `0:` prefix.  
...
Returns all active proposals with:
...
For deposits, withdrawals and votes, send simple messages to the pool with the desired text comments as described above. When sending a deposit, you can store the amount that was sent in the local storage. When sending a re-deposit, add it to this amount too.
...
To obtain the complete state of the pool, you need to query both `get_pool_data` and `list_nominators`.
