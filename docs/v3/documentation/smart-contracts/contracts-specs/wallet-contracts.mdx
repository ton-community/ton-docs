...
:::info
Note that [TVM](/v3/documentation/tvm/overview) has [standard exit codes](/v3/documentation/tvm/exit-codes) (`0x0` - is one of them), so you can get one of them too, if you run out of [gas](/v3/documentation/smart-contracts/transaction-fees/fees), for example, you will get `0xD` code.
:::
...
- [addresses-in-ton-blockchain](/v3/documentation/smart-contracts/addresses/address)
...
- [Account ID](/v3/documentation/smart-contracts/addresses/address#account-id)
...
<br></br>
<ThemedImage
alt="Wallet V5 contract structure diagram"
sources={{
...
:::caution
In contrast to the specifications of previous wallet versions, we will rely on the [TL-B](/v3/documentation/data-formats/tlb/overview) scheme, due to the relative complexity of this wallet version's interface implementation. We will provide a brief description for each. Nevertheless, a basic understanding is still required; in combination with the wallet source code, it should be enough.
:::
...
:::info
Note that the maximum number of actions is 255; this is a consequence of the realization through the [c5](/v3/documentation/tvm/overview#results-of-tvm-execution) TVM register. Technically, you can make a request with empty `OutAction` and `ExtendedAction`, but in that case, it will be similar to just receiving funds.
:::
...
![Gasless transaction flow diagram](/img/gasless.jpg)

:::tip
Consequently, there will be services (such as [Tonkeeper's Battery](https://blog.ton.org/tonkeeper-releases-huge-update#tonkeeper-battery)) that provide this functionality: they pay the transaction fees in Toncoin on behalf of the user, but charge a fee in tokens.
:::
