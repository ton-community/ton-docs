...
- Past election data in the `past_elections` hashmap (including complaints and `frozen` stakes held for `stake_held_for` periods, defined in [**ConfigParam 15**](/v3/documentation/network/configs/blockchain-configs#param-15)).
...
- Take applications with stake ≥ `min_stake` ([**ConfigParam 17**](/v3/documentation/network/configs/blockchain-configs#param-17)).
- Arrange candidates by stake in descending order.
- If applicants exceed `max_validators` ([**ConfigParam 16**](/v3/documentation/network/configs/blockchain-configs#param-16)), discard the lowest-staked candidates.
...
- `min_validators` ≤ participants ≤ `max_validators` ([**ConfigParam 16**](/v3/documentation/network/configs/blockchain-configs#param-16)).
- Stakes must satisfy:
- `min_stake` ≤ stake ≤ `max_stake`
  - `min_total_stake` ≤ total stake
  - Stake ratios ≤ `max_stake_factor` ([**ConfigParam 17**](/v3/documentation/network/configs/blockchain-configs#param-17)).
...
The Elector registers the complaint in the `past_elections` hashmap. Current round validators then verify the complaint. If the proof is valid, validators vote on the complaint. Approval requires agreement from more than **three-quarters of the total validator weight** and follows the wins/losses limits defined in [**ConfigParam 11**](/v3/documentation/network/configs/blockchain-configs#param-11) (not just a majority of participants).

If approved, the fine (as determined by [**ConfigParam 40**](/v3/documentation/network/configs/blockchain-configs#param-40) — MisbehaviourPunishmentConfig) is deducted from the validator's `frozen` stake in the relevant `past_elections` record. These funds stay locked for the period defined by [**ConfigParam 15**](/v3/documentation/network/configs/blockchain-configs#param-15) (`stake_held_for`).
...
2. **Config** stores it in [**ConfigParam 36**](/v3/documentation/network/configs/blockchain-configs#param-32-34-and-36) (_next validators_).
3. At the scheduled time (`utime_since`), **Config**:
   - Moves the old set to **ConfigParam 32** (_previous validators_).
   - Promotes [**ConfigParam 36**](/v3/documentation/network/configs/blockchain-configs#param-32-34-and-36) to [**ConfigParam 34**](/v3/documentation/network/configs/blockchain-configs#param-32-34-and-36) (_current validators_).

### Proposal/voting mechanism

1. **Submit a proposal**: Pay storage fees to propose parameter changes.
2. **Vote**: Validators (from [**ConfigParam 34**](/v3/documentation/network/configs/blockchain-configs#param-32-34-and-36)) sign approval messages.
3. **Outcome**:
   - **Approved**: After `min_wins` rounds ([**ConfigParam 11**](/v3/documentation/network/configs/blockchain-configs#param-11)) with ≥3/4 weighted votes.
   - **Rejected**: After `max_losses` rounds.
   - _Critical parameters_ ([**ConfigParam 10**](/v3/documentation/network/configs/blockchain-configs#param-10)) require more rounds.
...
- [Contract examples](https://docs.ton.org/v3/documentation/smart-contracts/contracts-specs/examples/#contracts-used-in-production)
