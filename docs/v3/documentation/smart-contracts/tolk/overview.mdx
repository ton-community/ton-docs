...
* Language features exposing TON nature: auto-serialized structures, pattern matching, and convenient message management.
* Advanced static type system: null safety, type aliases, union types, generics — everything you need to write safe and flexible code.
* Maximum efficiency: smart contracts in Tolk are cheaper than smart contracts in FunC — benchmarks show up to 50% gas savings thanks to compiler optimizations and unique "lazy" functionality.
* Deep control: when necessary, you're able to manipulate the stack or custom TVM instructions.
...
2. Create a "Tolk counter" with blueprint: `npm create ton@latest`. Learn more in [Setting up the environment](/v3/documentation/smart-contracts/tolk/environment-setup.mdx).
3. Check step by step explanation of Counter contract in [Counter contract implementation in Tolk](/v3/documentation/smart-contracts/tolk/counter-smart-contract.mdx).
4. Check out the [Tolk language guide](/v3/documentation/smart-contracts/tolk/language-guide.mdx) for a high-level overview of the language.
