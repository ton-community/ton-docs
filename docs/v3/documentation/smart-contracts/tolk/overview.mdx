...
Tolk is a modern programming language for creating smart contracts on the TON Blockchain. Designed to be more accessible than its predecessors FunC, Tolk features a syntax similar to TypeScript and Kotlin, making it approachable for developers familiar with popular programming languages. Tolk retains the power and flexibility needed to work with the TON Virtual Machine (TVM) while simplifying the process of writing and reading code.

This guide is for those starting smart contract development from scratch. No prior knowledge of FunC or other TON languages is required—we’ll begin with the basics and gradually explore key concepts.

If you are already familiar with FunC, please refer to the documentation on migrating from FunC to Tolk.
