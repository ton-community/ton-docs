...
-   **frozen**: occurs when `storage_fees_due` exceeds 0.1 TON. The account balance becomes zero; unpaid storage fees are tracked as debt (see `StorageInfo.due_payment` and transaction `storage_fees_due`). Only the hashes of the previous code and data cells are preserved. While frozen, the contract cannot execute. To unfreeze, send a message with the valid `state_init` and sufficient funds for storage fees. See the [TON unfreezer](https://unfreezer.ton.org/).

:::note
Thresholds for storage‑fee debt:
- If unpaid storage fees (StorageInfo.due_payment) exceed 0.1 TON, the account becomes frozen ([source](https://github.com/ton-blockchain/ton/blob/72056a2261cbb11f7cf0f20b389bcbffe018b1a8/crypto/block/transaction.cpp#L949C11-L953C12)).
- If the debt (StorageInfo.due_payment) exceeds 1 TON, the account is deleted (status `nonexist`) ([source](https://github.com/ton-blockchain/ton/blob/72056a2261cbb11f7cf0f20b389bcbffe018b1a8/crypto/block/transaction.cpp#L936C11-L946C12)).
:::
