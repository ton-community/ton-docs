...
-   **frozen**: occurs when `storage_fees_due` exceeds 0.1 TON. Only the combined state hash of the previous code and data is preserved. While frozen, the contract cannot execute. To unfreeze, send a message with the valid `state_init` and sufficient funds for storage fees. Recovery is complex; avoid reaching this state. See the [TON unfreezer](https://unfreezer.ton.org/).
