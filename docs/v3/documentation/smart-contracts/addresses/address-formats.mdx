...
-   **SDKs**: see `Address.spec.ts` in the [ton-core](https://github.com/ton-org/ton/tree/master/ton-core) library for the conversion examples.
...
Additional binary data related to TON Blockchain employs similar armored base64 representations, inspired by the user-friendly address format. These differentiate from one another depending on the first 4 characters of their byte tag. For example, 256-bit Ed25519 public keys are represented by first creating a 36-byte sequence using the process below in order:
