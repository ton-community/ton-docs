...
In fact, an Account ID (**account_id**) is defined as the result of applying a hash function (specifically SHA-256) to a initial state of the contract. Initial state of a contract consists of two main parts:
...
- `nonexist` - contract doesn't have any data (or the contract was deleted) and the balance of an account is 0. We can say that initially all 2<sup>256</sup> address are in this state.
