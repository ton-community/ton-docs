...
Developed by [TON Studio](https://tonstudio.io), powered by the community — by the end of 2024, the number of _unique code_[^1] contracts deployed on the Mainnet reached almost 28,000, of which about 33% were written in Tact. You can view some selected projects here: [Tact in production](#tact-in-production).
...
- [Security audit of Tact by Trail of Bits (2025, PDF)](https://github.com/trailofbits/publications/blob/master/reviews/2025-01-ton-studio-tact-compiler-securityreview.pdf)
...
It will make the `tact` compiler available in your `PATH`, as well as:

- a convenient `unboc` disassembler for contract code compiled into the [Bag of Cells (BoC)](/v3/documentation/data-formats/cells/overview/#bag-of-cells) (`.boc`) format,
...
- BoC Disassembler (`unboc`) — Disassembler for [`.boc`](/v3/documentation/data-formats/cells/overview#bag-of-cells) files. Ships as a standalone binary with the compiler. You can invoke it by running `npx unboc` in your Tact projects.
