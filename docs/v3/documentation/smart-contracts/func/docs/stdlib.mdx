...
For detailed descriptions of these commands, refer to the [TVM documentation](/v3/documentation/tvm/overview).
Some explanations in this document are adapted from there.
...
Returns the tail and then the head of a lisp-style list (in that order). It can be used as a [(non-)modifying method](/v3/documentation/smart-contracts/func/docs/statements#method-calls).
...
Creates a tuple with a single element—a singleton.
...
The [c7 special register](/v3/documentation/tvm/overview#control-registers) holds useful data about smart contract execution.
...
- `int`: The remaining balance in nanotons.
...
Returns the value of the global configuration parameter with integer index `x` as `cell` or `null` value.
...
You need to manually add it to your contract, with the `asm` word specified as shown above.
:::
...
If the total number of visited cells `x` exceeds `max_cells`, the process stops before visiting the `(max_cells + 1)`-th cell, and the function returns `(null, null, null, 0)` to indicate failure. If `c` is `null`, the function returns `(0, 0, 0, -1)`.
...
A strict version of `compute_data_size?` that throws an exception if the computation fails.
...
A strict version of `slice_compute_data_size?` that throws an exception if the computation fails.
...
Converts a `slice s` into a basic continuation `c`, where `c.code = s`, with an empty stack and a savelist.
...
This function calculates how much gas can be purchased for the amount `gram` (in nanotons) and updates the gas limit `gl` accordingly,
...
that reserves a specific amount of nanotons from the account’s remaining balance.
The behavior depends on the `mode` parameter:

- If `mode = 0`, exactly the `amount` of nanotons is reserved.
- If `mode = 2`, up to the `amount` of nanotons is reserved.
...
The pseudo-random number generator relies on a random seed, a 256-bit unsigned integer, and sometimes additional data stored in [c7](/v3/documentation/tvm/overview#control-registers).
...
- `x` is also converted into a 32-byte big-endian array.
  The new seed is the SHA-256 hash of their concatenation.
...
Call `randomize_lt` before using the pseudo‑random number generator for the first time.
...
A deserialized `MsgAddress` is represented as a tuple `t` with the following structure:
...
Loads a serialized amount of Toncoin, which is an unsigned integer up to `2^120 - 1`.
...
Learn more in the [TVM overview](/v3/documentation/tvm/overview) section.
