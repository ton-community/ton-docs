...
- `^a.1` matches any `a.x.y` where `x >= 1` (e.g., `a.1.3`, `a.2.3`, `a.1.0`).

The `#pragma` version directive can be used multiple times, and the compiler must satisfy all specified conditions.

### #pragma not-version

The syntax of `#pragma not-version` is identical to `#pragma version`. Compilation fails if the specified condition is met.
...
Arguments are evaluated left to right; any `asm` reordering is applied after argument evaluation.
