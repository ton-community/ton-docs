...
This section provides a brief overview of FunC statements, which form the core of function bodies.
...
```
### Lambda expressions
Lambda expressions are not supported in FunC.

### Method calls

#### Non-modifying methods

In FunC, a function with at least one argument can be invoked as a non‑modifying method using the `.` syntax.
...
These operators must be separated from the operands:
- `- x` - Negates x.
...
They also should be separated from the operands:
...
A range check exception is thrown if the repetition count is less than `-2^31` or greater than `2^31 - 1`.
...
Negated condition (equivalent to `if (~flag)`):
```func
ifnot (flag) {
  do_something();
}
