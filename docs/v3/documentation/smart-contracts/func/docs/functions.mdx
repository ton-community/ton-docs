...
Every program must include a function with `id = 0`, meaning it must define either `main` or `recv_internal`. The `run_ticktock` function is used in tick-tock transactions of special smart contracts.

#### Receive internal

The `recv_internal` function is invoked when a smart contract receives **an inbound internal message**. When the [TVM initializes](/v3/documentation/tvm/overview#initializing-tvm), certain variables are automatically placed on the stack. By specifying arguments in `recv_internal`, the smart contract can access some of these values. Any values not explicitly referenced in the function parameters will remain unused at the bottom of the stack.
...
Function arguments are separated by commas. The following types of argument declarations are valid:
...
A function with at least one argument can be called using the non-modifying method syntax. For example, the function `store_uint` has the type `(builder, int, int) → builder`, where:
- The first argument is the builder.
...
**Negative IDs**: Negative IDs are reserved for special entrypoints (e.g., `recv_external` = -1, `run_ticktock` = -2) and user-defined `method_id(...)` must be non-negative within the 19-bit range (0–524,287).
...
When called, this function is translated into the two assembler commands `INC` and `NEGATE`. Alternatively, it can be written as:
...
:::info
The list of assembler commands can be found here: [TVM instructions](/v3/documentation/tvm/overview#tvm-instructions).
:::
