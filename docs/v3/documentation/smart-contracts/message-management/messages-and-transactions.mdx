...
There is a [TVM exit code](/v3/documentation/tvm/exit-codes) for a transaction that had a compute phase. Success requires a compute‑phase exit code of 0 or 1 and, if an action phase executed, an action result code of 0. Also, TVM [compute phase may be skipped](/v3/documentation/tvm/overview#when-the-compute-phase-is-skipped) for reasons like lack of funds or state.
