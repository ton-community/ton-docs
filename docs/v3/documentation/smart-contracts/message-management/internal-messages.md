...
3. Select byte string `prefix` of length between 16 and 31 such that `len(prefix+msg)` is divisible by 16. The first byte of `prefix` is equal to `len(prefix)`, and the other bytes are random. Let `data = prefix + msg`.
...
The same format is used for comments for NFT and jetton transfers. Note that the public key of the sender and receiver addresses (not jetton-wallet addresses) should be used.
