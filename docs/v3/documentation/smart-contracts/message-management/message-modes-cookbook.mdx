...
The key difference is that `flag 16` creates bounces for [action phase errors](/v3/documentation/tvm/exit-codes). In contrast, the message's [built‐in bounce flag](/v3/documentation/data-formats/layout/messages#commonmsginfo) handles protocol‐level failures like:
