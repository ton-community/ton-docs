...
The key difference is that `flag 16` creates bounces for [action phase errors](/v3/documentation/tvm/exit-codes#standard-exit-codes). In contrast, the message's [built‐in bounce flag](/v3/guidelines/smart-contracts/howto/wallet#internal-message-creation) handles protocol‐level failures like:
