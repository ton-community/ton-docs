...
The contract reflects all TON tokens it holds in its `balance`. Some of these tokens are also assigned to the currently processed incoming `message`. Transactions process one incoming message and up to 255 outgoing messages (see [overview on messages and transactions](/v3/documentation/dapps/assets/overview#overview-on-messages-and-transactions)).
...
- From the incoming message's `value`. See [accept_message effects](/v3/documentation/smart-contracts/transaction-fees/accept-message-effects).
- From the contract's `balance`. Storage fees are paid from the contract balance.

#### Fees

Actual transaction fees will vary depending on the blockchain configuration, the smart contract code, and other factors. Storage fees are deducted from the contract balance; for internal messages, gas is covered by the incoming message value by default, and if funds are insufficient the compute phase may be skipped.
...
| Fwd. fee                                                                                              | 0.001       | action_fee + fwd_fee + ihr_fee  |
...
For internal messages using `+2` (ignore action-phase errors), the inbound value remains credited in the credit phase even if the action phase is ignored. See this [example transaction](https://testnet.tonviewer.com/transaction/8a388731812c80ab9b0ea9531108425488af5def854e4bd6f0ed47362b56d557).
...
The key difference is that `flag 16` creates bounces for [action phase errors](/v3/documentation/tvm/tvm-exit-codes#standard-exit-codes). In contrast, the message's [built‐in bounce flag](/v3/documentation/data-formats/tlb/msg-tlb#commonmsginfo) handles protocol‐level failures like:
...
**A** sent 0.1 TON to **B**; after that, **B** sent 0.5 TON to **C** with `mode` = 64, `msg_fwd_fees` are 0.004 TON, actual received `value` will be 0.6 TON. The sum `action_fee` + `fwd_fee` (and `ihr_fee` if applicable) is deducted from the outbound message value; `storage_fee` (part of `total_fee`) is paid from the contract balance; the recipient’s net credit equals the message value minus outbound fees.

State after the transaction: Account A has 0.896 TON, Account B has 0.5 TON, Account C has 1.6 - (`total_fee` + `fwd_fee`) TON

:::info
You can check [this example](https://testnet.tonviewer.com/transaction/f63ab35f34e342cdd249f13018d5034ce3d80c488628d5a4db0a43163fa50adb).
...
**A** sent 0.1 TON to **B**; after that, **B** sent 0.5 TON to **C** with `mode` = 65, `msg_fwd_fees` are 0.004 TON, actual received value will be 0.6 TON. The sum `action_fee` + `fwd_fee` (and `ihr_fee` if applicable) is deducted from the contract balance; `storage_fee` (part of `total_fee`) is paid from the contract balance; the recipient’s net credit equals the full message value.

State after the transaction: Account A has 0.896 TON, Account B has 0.5 - (`action_fee` + `fwd_fee` + `ihr_fee`, if applicable) TON, Account C has 1.6 TON

:::info
You can check [this example](https://testnet.tonviewer.com/transaction/ad93e784453b573d737d9d928b4377ff3779177753e05629e54f6629556568ad).
Please note that `storage_fee` is included in `total_fee` but is always paid from the contract `balance`.
:::
...
**A** sent 0.1 TON to **B**; after that, **B** sent 0.5 TON to **C** with `mode` = 80, `msg_fwd_fees` are 0.004 TON, actual received value will be 0.6 TON. The sum `action_fee` + `fwd_fee` (and `ihr_fee` if applicable) is deducted from the outbound message value; `storage_fee` (part of `total_fee`) is paid from the contract balance; the recipient’s net credit equals the message value minus outbound fees.
...
:::tip
If no errors occur, the result is the same as [mode = 64, flag 0](#mode64). You can [check it in the C5 action list](https://retracer.ton.org/?tx=6489d60d9197c0be7ee64b0812139d82221e8f94c6e378c356acc10f9067310c) of the jetton wallet.
:::

<br></br>
<div class="text--center">
  <ThemedImage
    alt="Diagram: mode 80 carry remaining value (no error)"
    sources={{
      light:
...
**A** sent 0.1 TON to **B**; after that, **B** sent 0.5 TON to **C** with `mode` = 81, `msg_fwd_fees` are 0.004 TON, actual received value will be 0.6 TON. The sum `action_fee` + `fwd_fee` (and `ihr_fee` if applicable) is deducted from the contract balance; `storage_fee` (part of `total_fee`) is paid from the contract balance; the recipient’s net credit equals the full message value.
If an error occurs during the action phase, the message will bounce, and `total_fee` + `fwd_fee` will be deducted from the `value`.
...
:::tip
If no errors occur, the result is the same as [mode = 128](#mode128). You can [check it in the C5 action list](https://retracer.ton.org/?tx=e4f31e37eec74a8cfcecdad9246a6bbf3da20c4edb3635150cb0fa54b9def558) of the jetton wallet.
:::
