...
Please check how [get_balance](/v3/documentation/smart-contracts/func/docs/stdlib/#get_balance) works to understand transaction state better. Balance in
...
Actual transaction fees will vary depending on blockchain configuration, code of the smart contract and other factors. When message is received, part of the contract `balance` will be consumed by [storage fees](/v3/documentation/smart-contracts/transaction-fees/fees-low-level#storage-fee) and gas_fees if message `value` [is above certain amount](/v3/documentation/tvm/tvm-overview#compute-phase-skipped).
