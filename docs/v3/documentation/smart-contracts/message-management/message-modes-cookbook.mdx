...
Actual transaction fees will vary depending on the blockchain configuration, the smart contract code, and other factors. When a message is received, storage and gas fees will consume part of the contract `balance` if the message `value` [is above a certain amount](/v3/documentation/tvm/overview#when-the-compute-phase-is-skipped).

According to the [transaction flow](/v3/documentation/tvm/overview#transactions-and-phases), each transaction consists of five distinct phases:
...
In case of an error during [action phase](/v3/documentation/smart-contracts/message-management/sending-messages#message-modes), the message will be skipped instead of throwing an [exit code](/v3/documentation/tvm/exit-codes#standard-exit-codes).
...
:::info tip
If no errors occur, the result is the same as [mode = 64, flag 0](#mode64). This mode is widely used for jetton transfers. You can [check it in the C5 action list](https://retracer.ton.org/?tx=6489d60d9197c0be7ee64b0812139d82221e8f94c6e378c356acc10f9067310c) of the jetton wallet.
:::
...
:::info tip
If no errors occur, the result is the same as [mode = 128](#mode128). This mode is widely used for jetton transfers. You can [check it in the C5 action list](https://retracer.ton.org/?tx=e4f31e37eec74a8cfcecdad9246a6bbf3da20c4edb3635150cb0fa54b9def558) of the jetton wallet.
:::
