...
This update is enabled with [Config8 `version >= 9`](../../network/configs/blockchain-configs.mdx#param-8).
...
| :--------------------------------- | :---------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SECP256K1_XONLY_PUBKEY_TWEAK_ADD` | _`key tweak - 0 or f x y -1`_ | Performs `secp256k1_xonly_pubkey_tweak_add`. Inputs are two 256-bit unsigned integers (`key`, `tweak`). Returns a 65-byte public key as `uint8 f`, `uint256 x`, `uint256 y` (similar to `ECRECOVER`). Gas cost: 1276 gas. |
| `mask SETCONTCTRMANY`              | _`cont - cont'`_              | For each bit set in `mask` (range 0..255), sets the corresponding continuation control register for index `i` in the continuation. Efficiently updates multiple continuations.                                           |
...
- The `RAWRESERVEX` action in mode = 2 supports reserving extra currencies in addition to TON.
...
| `GETGASFEESIMPLE`     | _`gas_used is_mc - price`_           | Computes additional gas cost without flat price, using `(gas_used * price) / 2^16`.                                                                                                                                                                                                                                   |
| `GETFORWARDFEESIMPLE` | _`cells bits is_mc - price`_         | Computes additional forwarding fee without lump price, using `(bits * bit_price + cells * cell_price) / 2^16`.                                                                                                                                                                                                        |
| `HASHEXT`             | _`cell i - hash`_                    | Computes an extended hash of a cell at the given index.                                                                                                                                                                                                                                                               |
| `PFXDICTSET`          | _`x k D n - D' -1 or D 0`_           | Sets a value in a prefix code dictionary. Fails if prefix constraints are violated.                                                                                                                                                                                                                                   |
| `PFXDICTREPLACE`      | _`x k D n - D' -1 or D 0`_           | Replaces a value in a prefix code dictionary if constraints are satisfied.                                                                                                                                                                                                                                            |
| `PFXDICTADD`          | _`x k D n - D' -1 or D 0`_           | Adds an entry to a prefix code dictionary. Fails if entry already exists.                                                                                                                                                                                                                                             |
| `PFXDICTDEL`          | _`k D n - D' -1 or D 0`_             | Deletes an entry from a prefix code dictionary.                                                                                                                                                                                                                                                                       |
...
In the emulator, handling of library cells and extra currency operations has been improved to provide more accurate behavior during local testing. Support for extra currencies has been refined across both `run_get_method` and reserve modes, ensuring compatibility for contracts that rely on multi-currency interactions.

> Note: The `HASHEXT` opcode described here is distinct from the `HASHEXT_(HASH)`/`HASHEXTR_(HASH)` family introduced in 2023.
