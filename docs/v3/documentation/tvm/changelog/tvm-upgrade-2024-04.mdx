...
:::tip
This upgrade has been active on Mainnet since March 16, 2024. See the details [here](https://t.me/tonstatus/101). A preview of the update for blueprints was available in tvmbeta builds of the ecosystem packages.
:::

This update is enabled with ConfigParam 8 (GlobalVersion) ≥ 6.

## `c7`

The `c7` tuple has been extended from 14 to 17 elements as follows:
...
| `SENDMSG`             | _`msg mode - i`_ | Retrieves `ConfigParam 24/25` (message prices) and `ConfigParam 43` (`max_msg_cells`) from `c7` rather than from the config dictionary. Returns the fee for generating the message (in nanotons). |
...
| `GETPRECOMPILEDGAS`   | _`- i or null`_                      | Returns the cost of contract execution in gas units if the contract is _precompiled_.                                                                                                                                                                                                                                                                                                                                                                     |
...
**Note:** `gas_used`, `cells`, `bits`, and `seconds` are integers in the range `0..2^63-1`.
...
| `CHASHIX`    | _`cell i - hash`_     | Returns the `i`-th hash of the cell.  |
