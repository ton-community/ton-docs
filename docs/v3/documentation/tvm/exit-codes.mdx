...
| 1         | [Compute phase][c]                  | Alternative successful execution exit code (rarely used).                                              |
...
| -14       | [Compute phase][c]                  | Equivalent to code 13; see [exit code 13](#13).                                                        |
...
| 39        | [Action phase][a]                   | Outbound message does not fit into a cell.                                                             |
...
This is an alternative exit code for successfully executing the [compute phase](#compute); rarely used. Can be produced by `RETALT`.
...
Currently, only four actions are supported: changing the contract code, sending a message, reserving a specific amount of nanotons, and changing the library cell. If there's any issue with the specified action (invalid message, unsupported action, etc.), an error with exit code 34 is thrown: `Invalid or unsupported action`.
...
:::note
[Extra currencies](/v3/documentation/dapps/assets/extra-currencies) in TON Docs.
...
- `max_vm_data_depth` equals `2^9` — maximum cell depth in messages and account state.
...
- `defer_out_queue_size_limit` equals `2^8` — maximum number of outbound messages to be queued (applies to validators and collators).
...
[tlb]: /v3/documentation/data-formats/tlb/overview
...
[exotic]: /v3/documentation/data-formats/cells/exotic
