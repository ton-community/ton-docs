...
2. **Credit phase**: credits the incoming message value to the account balance; storage fees are handled in the storage phase.
3. **Compute phase**: executes the contract code on TVM. The result includes `exit_code` and may update `c4` (persistent data) and `c5` (output actions).
...
:::tip
- TON Blockchain paper (may include outdated information): https://ton.org/tvm.pdf
:::
...
- `c3`: initialized with the smart contract code cell. Continuation.
...
For a complete list of TVM instructions, visit [TVM instructions](/v3/documentation/tvm/specification/runvm).
...
- [TVM instructions](/v3/documentation/tvm/specification/runvm)
