...
Exit codes 0 and 1 indicate standard (successful) execution of the [compute phase](#compute). Exit (or [result](#action)) code 0 indicates the standard (successful) execution of the [action phase](#action). Non‑zero codes indicate failure of that phase: compute‑phase failures skip the [action phase](#action) and may bounce, while action‑phase failures may be ignored or revert depending on send modes.
...
| 11        | [Compute phase][c]                     | Unknown error (may be thrown by user programs).                                                                  |
...
| ~~137~~   | ~~Tact compiler ([Compute phase][c])~~ | ~~Masterchain support is not enabled for this contract.~~ Removed since Tact 1.6                       |
| 138       | Tact compiler ([Compute phase][c])     | Not a BaseChain address.                                                                               |
>>>>>>> pitaya/doc-fix/run_20250901_144329_2bdedc55/k8eb204f4
...
it('tests something, you name it', async () => {
  ...
  const res = await your_contract_name.send(/* args */);
...
[exotic]: /v3/documentation/data-formats/tlb/exotic-cells
