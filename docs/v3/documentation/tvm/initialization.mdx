...
it is highly recommended to familiarize yourself with the [TL-B language](/v3/documentation/data-formats/tlb/overview).
- [TVM retracer](https://retracer.ton.org/)
:::

TVM is invoked during the compute phase of ordinary and other transactions.
...
After that, the contract code (current continuation) is executed. It selects the correct function based on _s_, which is expected to process the remaining arguments and terminate.
...
The smart contract must terminate with either _g_<sub>c</sub> = 0 or _g_<sub>r</sub> ≥ _g_<sub>c</sub>.
...
- The 256-bit address of the current account inside the MasterChain is passed as an unsigned _integer_.
...
- A _StateInit_ represents the state of the sibling account.
- A _slice_ containing _SplitMergeInfo_.
- The 256-bit address of the current account.
- The 256-bit address of the sibling account.
- An integer 0 ≤ _d_ ≤ 63, equal to the position of the only bit in which the addresses of the current and sibling accounts differ.
...
- `action_set_code` — for updating the contract code
...
| `balance_remaining` | [CurrencyCollection](/v3/documentation/data-formats/layout/messages#currencycollection) | Remaining balance of the smart contract                                                                                                                                                             |
| `myself`            | [MsgAddressInt](/v3/documentation/data-formats/layout/messages#msgaddressint-tl-b)      | Address of this smart contract                                                                                                                                                                      |
| `global_config`     | (Maybe Cell)                                                                        | Contains information about the global config                                                                                                                                                        |

In the July 2023 TVM upgrade, the **c7** tuple was extended from 10 to 14 elements. Read more about it [here](/v3/documentation/tvm/changelog/tvm-upgrade-2023-07).

## See also

- Original description of [the TVM initialization section](https://docs.ton.org/tblkch.pdf#page=89&zoom=100) in the TVM whitepaper.
