...
[MyTonCtrl](https://github.com/ton-blockchain/mytonctrl) is a console application that is a convenient wrapper for fift, lite-client, and validator-engine-console. It is specifically designed to streamline wallet, domain, and validator management tasks on Linux.
...
- `-i` - Ignore minimum requirements; use this only if you test the compilation process without running a real node.
 - `-m` - Set the mode to either `validator` or `liteserver`.
- `-t` - Disable telemetry.

To use testnet, pass `-c https://ton.org/testnet-global.config.json`.

The default for `-c` is `https://ton-blockchain.github.io/global.config.json` (mainnet).
...
| Fast        | `status fast` | `status fast` | Use in testnet; excludes validator efficiency and online validator count. |

[See more about status output](/v3/documentation/nodes/mytonctrl/status)
...
Read [Custom overlays](/v3/guidelines/nodes/custom-overlays).
...
```bash
MyTonCtrl> add_custom_overlay custom /config.json
...
This also works, but you should prefer values from your current MyTonCtrl state:
...
Learn more about `max_validators` and `max_main_validators` [on config parameters page](/v3/documentation/network/config-params/overview#configuration-parameters-for-the-number-of-validators-for-elections). Get the current values [for mainnet](https://tonviewer.com/config#16) and [for testnet](https://testnet.tonviewer.com/config#16).

## Pool commands

See [Nominator Pool](/v3/documentation/smart-contracts/contracts-specs/nominator-pool).
...
See [Single Nominator Pool](/v3/documentation/smart-contracts/contracts-specs/single-nominator-pool).
...
```bash
MyTonCtrl> nw 0 name v3 # By default, the subwallet is 0x29A9A317 + workchain
```
...
:::caution
Wallet version 'v4' is not supported for transferring
:::
...
| Any combination of the following arguments | `el --json adnl pubkey wallet past` | Full ADNL, Pubkey, Wallet, and past election entries in JSON representation. |
...
| Any combination of the following arguments | `vl --json adnl pubkey wallet past` | Full ADNL, Pubkey, Wallet, and past validator entries in JSON representation. |
...
| Any combination of the following arguments | `cl --json adnl past` | Full ADNL with past complaints in JSON representation. |
...
Exits from the `mytoninstaller` terminal.

### status

Displays the status of services — Full Node, MyTonCore, Validator Console, Lite Server — and node arguments.

### set_node_argument

| Format name        | Format                         | Description                                                                                       |
| ------------------ | ------------------------------ | ------------------------------------------------------------------------------------------------- |
| Add or replace arg | `set_node_argument -ARG_NAME ARG_VALUE` | Add argument or replace its value if it exists. `-ARG_NAME` must have `-` or `--` at the beginning |
...
| ------------------ | -------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| `threads`          | count of threads                                                                             | `CPU count - 1`                                                                     |
| `daemonize`        |                                                                                              | No value                                                                            |
...
| `state-ttl`        | TTL of blockchain states that the node keeps                                                 | 3600                                                                                |
| `archive-ttl`      | TTL of blocks that the node stores. _To force node not to store archive blocks use the value `86400`_ | 2592000 if liteserver mode was enabled during installation process, otherwise 86400 |
...
Modes can have the following names:
...
Performs the same action as `enable` in `mytoninstaller`.
