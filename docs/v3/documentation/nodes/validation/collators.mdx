...
:::note
Current value of `monitor_min_split` in Mainnet is `0`, which means that all shards are in one group. The actual value can always be checked in [Config param 12](/v3/documentation/network/config-params/overview#param-12).
:::
