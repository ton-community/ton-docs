...
:::note
Current value of `monitor_min_split` in Mainnet is `0`, which means that all shards are in one group. The actual value can always be checked in [ConfigParam 12](/v3/documentation/network/config-params/overview#param-12).
:::
...
The command `--add-shard 0:0800000000000000` will add the entire shard group associated with the prefix `0:2000000000000000` due to the `monitor_min_split` configuration.
