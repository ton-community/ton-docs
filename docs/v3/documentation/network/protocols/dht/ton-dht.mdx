...
Key-value pairs can be added and updated in the TON DHT. The rules for these updates can vary. In some cases, they allow for the old value to be replaced with a new one if the update satisfies the key’s update rule (for example, a valid signature for dht.updateRule.signature by the owner of the associated private key). The signature must be retained alongside the value so it can be verified by other nodes.
