...
Let's analyze **runSmcMethod** - this is a method that calls a function from a smart contract and returns a result. Here we need to understand some new data types such as [TL-B](/v3/documentation/data-formats/tlb/overviewe), [Cell](/v3/documentation/data-formats/cells/overview#cell) and [BoC](/v3/documentation/data-formats/cells/overview#bag-of-cells).
...
We will stop here, since we have already analyzed all the main cases, the rest can be obtained in a similar way with what we have analyzed. Also, additional information on parsing TL-B can be found in [official documentation](/v3/documentation/data-formats/tlb/overviewe).
