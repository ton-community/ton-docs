...
- 4-byte packet size in little-endian (N)
- 32-byte nonce (random bytes to protect against checksum attacks)
- (N - 64) payload bytes
- 32-byte SHA-256 checksum of nonce and payload
...
- \[32 bytes] **Our Ed25519 public key**
- \[32 bytes] **SHA-256 hash of our 160 bytes**
...
After establishing a connection, you can start receiving information. Data is serialized using TL.
...
- 4-byte packet size in little-endian -> 64 + (4+8) = **76**
- 32-byte nonce -> random 32 bytes
- 4-byte schema ID -> **9a2b084d**
- 8-byte request ID -> random uint64 number
- 32-byte SHA-256 checksum of nonce and payload

Send the packet and wait for [tcp.pong](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/tl/generate/scheme/ton_api.tl#L23), where `random_id` equals the one sent in the ping packet.
...
[Learn more about encoding byte arrays in TL here](/v3/documentation/data-formats/tl/#encoding-bytes-array).
...
- 4-byte packet size in little endian -> 64 + (4+32+(1+4+(1+4+3)+3)) = **116**
- 32-byte nonce -> random 32 bytes
- 4-byte ADNLQuery schema ID -> **7af98bb4**
- 32 bytes `query_id:int256` -> random 32 bytes
  - 1 byte array size -> **12**
  - 4 bytes of ID of the LiteQuery schema -> **df068c79**
    - 1 byte array size -> **4**
    - 4 bytes of ID of the getMasterchainInfo schema -> **2ee6b589**
...
The received packet is deserialized using the same algorithm as the request, but in the opposite direction, except that the response is wrapped only in [ADNLAnswer](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/tl/generate/scheme/lite_api.tl#L23).
...
Let's analyze **runSmcMethod** - this is a method that calls a function from a smart contract and returns a result. Here we need to understand some new data types such as [TL-B](/v3/documentation/data-formats/tlb/overview), [Cell](/v3/documentation/data-formats/cells/overview) and [BoC](/v3/documentation/data-formats/cells/overview#bag-of-cells).
...
- params:bytes - [Stack](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/crypto/block/block.tlb#L783) serialized in [BoC](/v3/documentation/data-formats/cells/overview#bag-of-cells), containing arguments to call the method. [\[Implementation example\]](https://github.com/xssnick/tonutils-go/blob/88f83bc3554ca78453dd1a42e9e9ea82554e3dd2/tlb/stack.go)
...
- result:mode.2?bytes -> [Stack](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/crypto/block/block.tlb#L783) serialized in [BoC](/v3/documentation/data-formats/cells/overview#bag-of-cells), containing the values returned by the method.

Let's analyze the call and get the result from the `a2` method of the contract `EQBL2_3lMiyywU17g-or8N7v9hDmPCpttzBPE2isF2GTzpK4`:
...
- `account` = WorkChain 0 (4 bytes `00000000`), and int256 [obtained from our contract address](/v3/documentation/data-formats/tlb/types#addresses), i.e. 32 bytes `4bdbfde5322cb2c14d7b83ea2bf0deeff610e63c2a6db7304f1368ac176193ce`
...
- `params:bytes` = Our method does not accept input parameters, so we need to pass it an empty stack (`000000`, cell 3 bytes - stack depth 0) serialized in [BoC](/v3/documentation/data-formats/cells/overview#bag-of-cells) -> `b5ee9c72010101010005000006000000` -> serialize in bytes and get `10b5ee9c72410101010005000006000000000000` 0x10 - size, 3 bytes in the end - padding.
...
- `result:mode.2?bytes` -> [Stack](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/crypto/block/block.tlb#L783) containing the data returned by the method in [BoC](/v3/documentation/data-formats/cells/overview#bag-of-cells) format, we will unpack it.

Inside `result` we received `b5ee9c7201010501001b000208000002030102020203030400080ccffcc1000000080aabbcc8`, this is [BoC](/v3/documentation/data-formats/cells/overview#bag-of-cells) containing the data. When we deserialize it, we will get a cell:
...
- `state` - [BoC](/v3/documentation/data-formats/cells/overview#bag-of-cells) TL-B [account state scheme](https://github.com/ton-blockchain/ton/blob/ad736c6bc3c06ad54dc6e40d62acbaf5dae41584/crypto/block/block.tlb#L232).
...
[Parse this BoC](/v3/documentation/data-formats/cells/overview#bag-of-cells) and get:
...
From here we will read `grams:Grams` which will be the account balance in nanotons. `grams:Grams` is `VarUInteger 16`, to store 16 (in binary form `10000`, subtracting 1 we get `1111`), then we read the first 4 bits, and multiply the resulting value by 8, then we read the received number of bits, it is our balance.
...
We will stop here, since we have already analyzed all the main cases, the rest can be obtained in a similar way with what we have analyzed. Also, additional information on parsing TL-B can be found in [official documentation](/v3/documentation/data-formats/tlb/overview).
...
The key ID is the SHA-256 hash of the serialized TL schema.
...
As an example, for keys of type ed25519 that are used for handshake, the key ID is the SHA-256 hash of [0xC6, 0xB4, 0x13, 0x48] and the public key (36-byte array, prefix + key).
...
To encrypt them, we need an AES-CTR cipher, to get it we need a SHA-256 hash of 160 bytes and [ECDH shared key](#getting-a-shared-key-using-ecdh).
