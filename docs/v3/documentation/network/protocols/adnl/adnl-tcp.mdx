...
- 32 bytes `query_id:int256` -> random 32 bytes
  ...
  - 4 bytes of ID of the LiteQuery schema -> **df068c79**
    ...
    - 3 zero bytes of padding (padded to a 4-byte boundary) ([details](/v3/documentation/data-formats/tl/#encoding-bytes-array))
  - 3 zero bytes of padding (padded to a 4-byte boundary) ([details](/v3/documentation/data-formats/tl/#encoding-bytes-array))
- 32-byte SHA-256 checksum of nonce and payload
...
We repeat the same process. The first 8 bits = `03` - that is, again cell. The second reference is the value `32[0AABBCC8]` and since our stack depth is 2, we complete the pass. IIn total, we have 2 values returned by the contract - `32[0CCFFCC1]` and `32[0AABBCC8]`.
...
<<<<<<< HEAD
As an example, for keys of type ed25519 that are used for handshake, the key ID will be the SHA-256 hash of **[0xC6, 0xB4, 0x13, 0x48]** and the **public key** (36-byte array, prefix + key).
=======
As an example, for keys of type ed25519 that are used for handshake, the key ID will be the SHA-256 hash from **\[0xC6, 0xB4, 0x13, 0x48]** and **public key**, (36 byte array, prefix + key).
>>>>>>> pitaya/doc-fix/run_20250901_144329_2bdedc55/kcc0cbf5d
