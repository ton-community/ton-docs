...
You can view the parameter values in the [current configuration](https://tonviewer.com/config), and the method of writing them into [cells](/v3/concepts/dive-into-ton/ton-blockchain/cells-as-data-storage) is outlined in the [block.tlb](https://github.com/ton-blockchain/ton/blob/master/crypto/block/block.tlb) file in [TL-B](/v3/documentation/data-formats/tlb/tl-b-language) format.

:::info
Configuration values are TL‑B‑typed cells serialized into Bags of Cells (BoC); see [canonical serialization](/v3/documentation/data-formats/tlb/canonical-cell-serialization) and [Bag of Cells](/v3/documentation/data-formats/tlb/cell-boc).
:::
...
:::info
In the configuration parameter, only the hash portion of the address is recorded, as the contract always resides in the [MasterChain](/v3/concepts/dive-into-ton/ton-blockchain/blockchain-of-blockchains#masterchain) (WorkChain -1). Therefore, the full address of the contract will be written as `-1:<value of the configuration parameter>`.
:::
...
:::info
For details, see [TON DNS & Domains](/v3/guidelines/web3/ton-dns/dns) and the [original specification](https://github.com/ton-blockchain/TEPs/blob/master/text/0081-dns-standard.md).
...
:::info
For background and process details, see the [Minter flow documentation](/v3/documentation/infra/minter-flow).
:::

## Param 7

This parameter stores the volume of each additional currency in circulation. The data is organized as a dictionary (also referred to as a **hashmap**). The structure uses the format `currency_id -> amount`, where the amount is represented as a `VarUInteger 32`, which is an integer ranging from `0` to `2^248-1`.
...
:::info
Validators enforce TL-B validity only for configuration parameters with non-negative indices. Values with negative indices are not validated against a specific `ConfigParam i` type. See the explanation in Configuration parameters: /v3/documentation/network/configs/config-params#configuration-parameters.
:::
