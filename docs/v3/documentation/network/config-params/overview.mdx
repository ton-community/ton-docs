...
You can view the parameter values in the [current configuration](https://tonviewer.com/config), and the method of writing them into [cells](/v3/concepts/dive-into-ton/ton-blockchain/cells) is outlined in the [block.tlb](https://github.com/ton-blockchain/ton/blob/master/crypto/block/block.tlb) file in [TL-B](/v3/documentation/data-formats/tlb/overviewe) format.

:::info
Configuration values are TL‑B‑typed cells serialized into Bags of Cells (BoC); see [canonical serialization](v3/documentation/data-formats/cells/serialization) and [Bag of Cells](/v3/documentation/data-formats/cells/overview).
:::
...
:::info
For background and process details, see the [Extra currency minter flow documentation](/v3/documentation/network/config-params/extra-currency).
:::
...
:::info
Validators enforce TL-B validity only for configuration parameters with non-negative indices. Values with negative indices are not validated against a specific `ConfigParam i` type. See the explanation in [Configuration parameters](/v3/documentation/network/config-params/update#configuration-parameters) section.
:::
...
On this [page](/v3/documentation/network/global-config), you can find active network configurations of the TON Blockchain.
