...
The site https://ton-connect.github.io/open-tc redirects the user to the link passed in the `connect` query parameter, so it's a workaround to open a `tc://` link in Telegram.
=======
...
https://ton-connect.github.io/open-tc redirects the user to the link passed in the `connect` query parameter; it is a workaround to open a `tc://` link in Telegram.
>>>>>>> pitaya/doc-fix/run_20250901_144329_2bdedc55/kba7299d2
...
<<<<<<< HEAD
Note that this button contains an empty deep link. It means that the send transaction request data goes through the HTTP bridge, and the transaction will appear in the user's wallet even if they just open the wallet app without clicking this button.
=======
Note that this button contains an empty deep link. It means that the send-transaction request data goes through the http-bridge, and the transaction will appear in the user's wallet, even if they simply open the wallet app without clicking this button.
>>>>>>> pitaya/doc-fix/run_20250901_144329_2bdedc55/kba7299d2
...
<<<<<<< HEAD
Starting from v3, TON Connect supports connection to TWA wallets like @wallet. At the moment in the tutorial, the bot could be connected to @wallet.
=======
