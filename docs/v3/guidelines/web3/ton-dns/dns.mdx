...
The [TON DNS standard](https://github.com/ton-blockchain/TEPs/blob/master/text/0081-dns-standard.md) outlines the domain name format, the domain resolution process, the interface for DNS smart contracts, and the structure of DNS records.

## SDK

Support for working with TON DNS is available through the JavaScript SDK [TonWeb](https://github.com/toncenter/tonweb) and [TonLib](/v3/guidelines/dapps/apis-sdks/ton-adnl-apis/).

Assumes `tonweb` is an initialized TonWeb instance.
...
Also, lite-client and tonlib-cli support DNS queries.

## Top-level domains (TLD)

TON DNS currently supports `.ton` and `.t.me` domains.
...
This may change in the future. Adding a new top-level domain would require deploying a new root DNS smart contract and a validator vote to update the [network config #4](/v3/documentation/network/configs/blockchain-configs#param-4) (see also [tonviewer](https://tonviewer.com/config#4)).
...
The `.ton` DNS collection contract acts as an NFT collection, while each `.ton` domain is an NFT item.
