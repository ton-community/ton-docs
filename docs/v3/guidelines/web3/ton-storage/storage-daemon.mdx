...
See source code in the [TON repository](https://github.com/ton-blockchain/ton/).

## Hardware requirements

- At least 1 GHz dual-core CPU
- At least 2 GB RAM
- At least 2 GB free SSD (excluding space for bag data)
- 10 Mbps network bandwidth with a static IP
...
You can compile `storage-daemon` and `storage-daemon-cli` from source using these [instructions](/v3/guidelines/smart-contracts/howto/compile/compilation-instructions#storage-daemon).
...
- TON Storage uses torrent-like technology, so terms like _Torrent_, _bag of files_, and _bag_ are used interchangeably. However, there are some important differences:
  ...
  - Each bag is distributed via its own [overlay network](/v3/documentation/network/protocols/overlay).
  ...
  - The [TON DHT](/v3/documentation/network/protocols/dht/overview) network is used to discover peers.

- A **bag of files** includes:
  - Torrent info.
  - The data block starts with a torrent header including file names and sizes, followed by the files themselves.

The data block is divided into chunks, with a default size of 128 KB. A Merkle tree built from TVM cells is constructed on the SHA-256 hashes of these chunks. This structure enables the creation and verification of _Merkle proofs_ for individual chunks and allows efficient reconstruction of the _bag_ by exchanging only the proof of the changed chunk.

- **Torrent info** includes:
  ...
  It also includes a Merkle root computed over the data block.

- Torrent info is serialized into a TVM cell. The hash of this cell is called the **bagID**, and it uniquely identifies the **bag**.

- The **bag metafile** is a file that includes the _torrent info_ and _header_. This file serves the same purpose as a `.torrent` file.
...
- `-I` - IP address and port of the daemon (`<host>:<port>`, where `<port>` equals the daemon’s `-p` TCP console port)
- `-k` and `-p` - client private key and server public key paths (in `storage-daemon-cli`, `-p` is a key path, not a port; similar to `validator-engine-console`). These are auto-generated on first daemon startup and stored in `<db>/cli-keys/`.
...
```bash
storage-daemon-cli ... -c "add-by-meta <metafile>" -c "list --hashes"
```
...
add-by-meta <metafile> -d directory
```

If -d is omitted, the bag is saved to the storage daemon’s default directory.
...
In the following commands, `<BagID>` can be either a bag's hexadecimal hash or its ordinal number in the current session, which is visible in the list output using the list command. Note that ordinal numbers of bags are not persistent and are unavailable in non-interactive mode.
...
- `remove --remove-files <BagID>` also removes the bag and its files. Note that if the bag is saved in the internal storage daemon directory, the files will be deleted in any case.
...
add-by-meta <metafile> -d dir --partial file1 file2 file3
```
...
After creation, detailed information, including the hash `bagID`, is shown in the console, and the daemon begins sharing the bag.
...
To download the bag, other users need to know its hash. You can also save the bag metafile:

```bash
get-meta <BagID> <metafile>
```
