...
- An HTTP → RLDP proxy running locally on the client's machine to access TON Sites. 
...
The proxy will continue running in your terminal if everything is set up correctly. Configure your tools or browser to use `http://localhost:8080` as an HTTP proxy (for example, `curl -x 127.0.0.1:8080`). See [Accessing TON Sites](#accessing-ton-sites) for details.
...
4. Generate a persistent ADNL address for your entry proxy.
   ...
   This is your newly generated persistent ADNL address in hexadecimal and user-friendly form. The corresponding private key is saved into file `45061...E2BD` in the current directory. Move the key into the keyring directory.
...
5. Run **rldp-http-proxy**.
   ...
   **Example**
   ```bash
   rldp-http-proxy/rldp-http-proxy -p 8080 -a <your_public_ip>:3333 -C global.config.json -A vcqmha5j3ceve35ammfrhqty46rkhi455otydstv66pk2tmf7rl25f3
   ```

   - `8080`: TCP port to listen for incoming HTTP requests on localhost. 
   ...
   - `-A <your_adnl_address>`: specifies the persistent ADNL address generated earlier.

The proxy will stay running in the terminal if everything is configured correctly. Configure your tools or browser to use `http://<your_public_ip>:8080` as an HTTP proxy (for example, `curl -x <your_public_ip>:8080`). See [Accessing TON Sites](#accessing-ton-sites) for setup details.
...
Once your RLDP-HTTP proxy is running on `localhost:8080`, as described in [Running an entry proxy](#running-an-entry-proxy), you can verify your setup with `curl`.
...
You can also use the ADNL address with the `.adnl` suffix. For example:
 ...
 **Example**
 ```bash
 rldp-http-proxy/rldp-http-proxy -a <your_public_ip>:3333 -L '*' -C global.config.json -A vcqmha5j3ceve35ammfrhqty46rkhi455otydstv66pk2tmf7rl25f3 -d -l tonsite.log
 ```

If everything is configured correctly, the RLDP-HTTP proxy will:
- Accept incoming HTTP requests from the TON Network via RLDP/ADNL. 
- Run on UDP port 3333 of your server’s IPv4 address `<your-server-ip>`. 
- Forward these HTTP requests to `127.0.0.1:80`, which is your local web server.
...
**Example**
```bash
rldp-http-proxy/rldp-http-proxy -a <your_public_ip>:3333 -R '*'@<YOUR_WEB_SERVER_HTTP_IP>:80 -C global.config.json -A vcqmha5j3ceve35ammfrhqty46rkhi455otydstv66pk2tmf7rl25f3 -d -l tonsite.log
```

In this case, your regular web server must be accessible at `http://<YOUR_WEB_SERVER_HTTP_IP>:80`, although this IP address will remain hidden from external access.


### Recommendations
TON Proxy currently lacks anonymity features, so if you'd prefer to keep your web server's IP address private, you have two options:
