...
To become a validator, you must meet two requirements: have a high-performance server and obtain at least 300,000 Toncoins, in order to make a stake. The number of active validators per round is determined by the network configuration parameters.
See [blockchain configuration parameters](/v3/documentation/network/configs/blockchain-configs) for current validator limits.
