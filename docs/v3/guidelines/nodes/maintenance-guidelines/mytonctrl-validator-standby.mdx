...
MyTonCtrl offers functionality to [create backups](mytonctrl-backup-restore.mdx) that include all necessary data. If you operate a validator node, we highly recommend setting up [automated backups](mytonctrl-backup-restore.mdx#automated-backup-creation) and regularly downloading backup archives from your machine.
...
The restore process is outlined in [the section](mytonctrl-backup-restore.mdx).
