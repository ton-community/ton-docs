...
Seed was brought from logical time of the transaction, and a hacker can win by brute-forcing the logical time in the current block (cause `lt` is sequential in the borders of one block).
...
There is no way to safe execute a third-party code in the contract, because [`out of gas`](/v3/documentation/tvm/exit-codes#standard-exit-codes) exception cannot be handled by `CATCH`. The attacker simply can [`COMMIT`](/v3/documentation/tvm/instructions#F80F) any state of contract and raise `out of gas`.
