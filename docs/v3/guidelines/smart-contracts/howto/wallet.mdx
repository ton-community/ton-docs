...
![Console output: Hello, TON!](/img/docs/how-to-wallet/wallet_1.png)
...
:::info
This guide provides basic information. For further details, please refer to our TL-B [documentation](/v3/documentation/data-formats/tlb/overview) to learn more about TL-B.
:::
...
|    Option    |                                                                                                                                                                                                                           Explanation                                                                                                                                                                                                                           |
| :----------: | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: |
| IHR Disabled |                Currently, this option is disabled (meaning we store `1`) because Instant Hypercube Routing (IHR) is not yet fully implemented. This option will become relevant once many [Shardchains](/v3/guidelines/dapps/transactions/foundations-of-blockchain#shardchain) are active on the network. For more details about the IHR Disabled option, refer to [tblkch.pdf](https://ton.org/tblkch.pdf) (chapter 2).                |
...
:::tip Wallet V4
In addition to the basic verification process we learned above for the Wallet V3, Wallet V4 smart contracts [extract the opcode to determine whether a simple transaction or a message associated with the plugin](https://github.com/ton-blockchain/wallet-contract/blob/4111fd9e3313ec17d99ca9b5b1656445b5b49d8f/func/wallet-v4-code.fc#L94-L100) is required. To match this version, it is necessary to add the `storeUint(0, 8)` (JS/TS), `MustStoreUInt(0, 8)` (Go) functions after writing the **sequence number (seqno)** and before specifying the transaction mode.
:::
...
Now that we have a completed message ready to send to our contract, the next step is to serialize it into a `BoC` ([bag of cells](/v3/documentation/data-formats/cells/overview#bag-of-cells)). Once serialized, we can send it using the following code:
...
> 💡 Useful link:
>
> More about [Bag of cells](/v3/documentation/data-formats/cells/overview#bag-of-cells)
...
|   Option    |                                                                                                                                                                                                                Explanation                                                                                                                                                                                                                 |
| :---------: |:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:|
| split_depth |        This option is designed for highly loaded smart contracts that can be split and distributed across multiple [ShardChains](/v3/guidelines/dapps/transactions/foundations-of-blockchain#shardchain). For more details on how this works, refer to the [tblkch.pdf](https://ton.org/tblkch.pdf) (section 4.1.6). Since this feature is not needed for wallet smart contracts, only a `0` bit is stored.         |
|   special   | This option is used for tick-tock smart contracts that are automatically triggered for each block. Regular smart contracts, such as wallets, do not require this functionality. For more details, refer to [this section](/v3/documentation/data-formats/layout/transactions#tick-tock) or the [tblkch.pdf](https://ton.org/tblkch.pdf) (section 4.1.6). Since this feature is unnecessary for our use case, only a `0` bit is stored. |
...
|   library   |                     This option refers to a library that operates on the [MasterChain](/v3/guidelines/dapps/transactions/foundations-of-blockchain#masterchain) and can be shared across multiple smart contracts. Since wallets do not require this functionality, its bit is set to `0`. For more information, refer to [tblkch.pdf](https://ton.org/tblkch.pdf) (section 1.8.4).                      |
...
const internalMessage = beginCell()
  ...
  .storeAddress(
    Address.parse("put your first wallet address from where you sent 0.1 TON")
  )
...
As you already know, [a single cell can store up to 1023 bits of data and up to 4 references](/v3/documentation/data-formats/cells/overview#cell) to other cells. In the first section of this tutorial, we explained how internal messages are delivered in a ‘whole’ loop as a link and sent. This means it’s possible to **store up to 4 internal messages within an external message**, allowing four messages to be sent simultaneously.
...
Now, let's complete the message as laid out in this tutorial's previous sections. The correct code to complete the message is in the [Source code section](/v3/guidelines/smart-contracts/howto/wallet#-source-code).
...
|                     Method                     |                                                                                                                            Explanation                                                                                                                            |
| :--------------------------------------------: |:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:|
|             int get_subwallet_id()             |                                                                                   Earlier in the tutorial, we considered this. This method allows you to retrieve subwallet_id.                                                                                    |
| int is_plugin_installed(int wc, int addr_hash) |         Let us know if the plugin has been installed. To call this method, you need to pass the [WorkChain](/v3/guidelines/dapps/transactions/foundations-of-blockchain#workchain-and-basechain) and the plugin address hash.          |
...
:::tip TL-B
You can read more about TL-B [here](/v3/documentation/data-formats/tlb/overview).
:::
...
In some situations, sending a large number of messages per transaction may be necessary. As previously mentioned, ordinary wallets support sending up to 4 messages simultaneously by storing [a maximum of 4 references](/v3/documentation/data-formats/cells/overview#cell) in a single cell. High-load wallets only allow 255 messages to be sent at once. This restriction exists because the maximum number of outgoing messages (actions) in the blockchain’s config settings is set to 255.
...
It is necessary to interact with the `f` variable several times. Since the [TVM is a stack machine](/v3/documentation/tvm/overview), during each interaction with the `f` variable, it is necessary to pop all values to get the desired variable. The `f~touch()` operation places the f variable at the top of the stack to optimize code execution.
...
|     int get_public_key()     |                                                                                                   Retrieve a public key. We have considered this method before.                                                                                                   |
...
- [TL-B](/v3/documentation/data-formats/tlb/overview)
- [Blockchain of blockchains](/v3/concepts/dive-into-ton/ton-blockchain/overview)
