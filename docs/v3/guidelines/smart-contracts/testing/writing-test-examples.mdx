...
The Fireworks contract redeploys itself, creating three entities, each with its ID (stored in the contract) and, as a result, a different smart contract address.
...
- **0**: An external request to the treasury (the launcher) sends an outbound message `op::set_first` with 2.5 TON to Fireworks.  
...
from Fireworks Launcher-1. This message is sent with `send mode = 0` in the transaction[ID:2].{" "}
...
This issue can arise when working directly with opcodes, as [stdlib.fc](https://github.com/ton-blockchain/ton/blob/master/crypto/smartcont/stdlib.fc) (a library for FunC) assumes this problem will not occur. 
...
If you attempt to read more data from a slice (when converting a cell to a slice data type for reading) than it contains, the system throws exit code 9. For example, this occurs if you try to read 11 bits from a slice containing only 10 bits or attempt to load a reference when no references exist.
