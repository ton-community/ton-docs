...
The Fireworks contract demonstrates different ways of sending messages in TON Blockchain.
...
it automatically executes with primary and common combinations of send modes.
...
For clarity, define Fireworks instances with different `state_init` by ID as follows:
...
Each **firework** is an outbound message with a unique message body, appearing in transactions with IDs 2 and 3.
...
The transaction [ID:3] occurs in Fireworks Launcher-2, is invoked with `op::launch_first`,
and executes one outbound message to the launcher.
...
it("should exist a transaction[ID:7] with a comment send mode = 128 + 32", async () => {
  ...
  expect(launchResult.transactions).toHaveTransaction({
    ...
    body: beginCell()
 .storeUint(0, 32)
 .storeStringTail("send mode = 128 + 32")
 .endCell(), // 0x00000000 comment opcode and encoded comment
 });
});
```
...
![Fireworks transaction fee trace](/img/docs/writing-test-examples/fireworks_trace_tonviewer.png?raw=true)

**Index** refers to the ID of a transaction in the `launchResult` array.

- **0**: External request to the treasury that results in a message `op::set_first` to Fireworks.
- **1**: The Fireworks transaction results in two messages to the launcher.
...
it("should execute with expected fees", async () => {
  ...
  if (computeFee == null || actionFee == null) {
...
This section provides test cases for TVM exit [codes](/v3/documentation/tvm/exit-codes) that can occur during transaction processing. These exit codes are part of the blockchain code itself. It is necessary to distinguish between exit codes during the [Compute Phase](/v3/documentation/tvm/overview#compute-phase) and the Action Phase.
...
An alternative exit code `1` is reserved in the TVM specification and does not occur in practice.
In early examples you may encounter [`RETALT`](https://github.com/ton-community/fireworks-func/blob/ef49b4da12d287a8f6c2b6f0c19d65814c1578fc/contracts/fireworks.fc#L20) being used to demonstrate non-zero exits,
but the only code denoting successful execution remains `0`.

### Compute Phase | exit code = 2

TVM is a [stack machine](/v3/documentation/tvm/overview#tvm-as-a-stack-machine). When interacting with different values, the system places them on the stack. If an opcode requires elements from the stack but finds it empty, the system throws this error.
...
If an `integer` value exceeds the expected range, exit code 5 is thrown. For example, if a negative value is used in the `storeUint(...)` function.
...
All data in TON is stored in [cells](/v3/documentation/data-formats/cells/overview#cell). A cell can store up to 1023 bits of data and four references to other cells. If you attempt to write more than 1023 bits or more than four references, exit code 8 is thrown.
...
The Action Phase starts after the Compute Phase and processes actions stored in [register c5](/v3/documentation/tvm/initialization#control-register-c5) during the Compute Phase. If the system detects incorrect data in this register, it throws exit code 32.
...
You must specify the correct source address when building a message's [CommonMsgInfo](/v3/guidelines/smart-contracts/howto/wallet#commonmsginfo) part. It must be either [addr_none](/v3/documentation/data-formats/layout/messages#addr_none00) or the address of the account sending the message.
...
This is similar to exit code `37` but refers to insufficient [ExtraCurrency](/v3/documentation/network/config-params/extra-currency#extracurrency) balance.
...
A library is a cell stored in [MasterChain](/v3/guidelines/dapps/transactions/foundations-of-blockchain#masterchain) and can be used by all smart contracts if it is [public](https://github.com/ton-blockchain/ton/blob/9728bc65b75defe4f9dcaaea0f62a22f198abe96/crypto/block/transaction.cpp#L1844).
