...
:::info
It is expected, that you learn basics on the creating TON Connect connection. Learn more with the [integration manual](/v3/guidelines/ton-connect/guidelines/integration-with-javascript-sdk).
:::

## Cells and Message Serialization

Before diving into building messages lets introduce concept cells what message bodies are made from.
...
### Using assets sdk for simpler serialization/deserialization

Manually handling cells can be tedious, so the `@ton-community/assets-sdk` provides convenient methods for serializing/deserializing messages.

Using `@ton-community/assets-sdk` and is more readable and less error-prone.
...
The simplest example is adding a payload with a comment. See more details on [this page](/v3/documentation/smart-contracts/message-management/internal-messages#simple-message-with-comment).
