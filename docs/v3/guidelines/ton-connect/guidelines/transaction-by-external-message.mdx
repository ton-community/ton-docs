...
API requests may fail due to rate limits or network issues. Use the `retry` function to handle such failures:
...
If a message has just been sent, it may take a few seconds before it appears on-chain. The `waitForTransaction` function polls the blockchain and waits for the corresponding transaction:
