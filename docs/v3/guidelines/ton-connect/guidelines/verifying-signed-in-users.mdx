...
* `AppDomain` is Length ++ EncodedDomainName:
    - `Length` is number of bytes in UTF-8 encoded app domain name, 32-bit little endian;
    - `EncodedDomainName` is `Length`-byte UTF-8 string encoding the app domain name;
* `Timestamp` holds 64-bit Unix epoch time of the signing operation, little endian
* `Payload` is a variable-length binary string.

Note: payload is variable-length data controlled by server. TON Connect 2 put it last to avoid using unnecessary length prefixes.
