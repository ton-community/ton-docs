...
This page explains the mandatory `ton_proof` flow that every dApp must implement to confirm that the connected wallet address really belongs to the user.

Without this server-side verification step your application cannot safely bind on-chain addresses to personal data or grant privileged actions. Always validate `ton_proof` before trusting any address-specific request.
