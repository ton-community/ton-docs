...
After authenticating users with the mandatory `ton_proof` verification, TON Connect also allows sending outgoing messages via connected wallets.
...
:::tip
Before sending any transactions you must establish the connection using `ton_proof` (see [Verifying signed-in users](/v3/guidelines/ton-connect/guidelines/verifying-signed-in-users)).
