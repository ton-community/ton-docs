...
<br></br>
<ThemedImage
    alt="TON Connect communication flow"
    sources={{
...
1. **Wallets** provide user interfaces for approving transactions and securely store cryptographic keys on users’ devices.
2. **DApps** built on TON offer rich functionality and secure user funds through the use of smart contracts.
...
3. **TON Connect** bridges wallets and DApps, providing users with a seamless and secure connection experience.
...
- **Developer-friendly tool**: provides APIs and features for secure, consistent integration.
...
- **Enable payments**: process payments using [TON API](https://tonapi.io/) or a custom indexer (e.g., [gobicycle](https://github.com/gobicycle/bicycle)). See the [Payments processing guide](/v3/guidelines/dapps/asset-processing/payments-processing) for more details.
