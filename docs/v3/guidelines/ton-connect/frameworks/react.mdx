...
:::tip
Get more examples on the [Sending messages](/v3/guidelines/ton-connect/guidelines/sending-messages) page.
:::
...
### Mandatory backend verification with ton_proof
...
To ensure that the user truly owns the declared address, you must use `ton_proof`.
...
- Ready State: After receiving the `tonProof` payload, set the state to 'ready' and include the payload.
- Error State: If obtaining `tonProof` fails, display an error and do not proceed with the connection.
...
if (tonProofPayload) {
  ...
  // Handle error: tonProof is required for authentication
  throw new Error('Unable to obtain tonProof payload. Connection aborted.');
}
```
