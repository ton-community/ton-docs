...
Additionally, the `urlSafe` parameter is used to make the base64 at compatible with URLs. The standard base64 encoding can cause issues with characters like + and /. When urlSafe = true, + is replaced by - and / by _.

You can obtain these address ats using the following code:
