...
o obtain a TON address object from a string in your SDK, you can use the following code:
...
Additionally, the `urlSafe` parameter is used to make the base64 format compatible with URLs. The standard base64 encoding can cause issues with characters like + and /. When urlSafe = true, + is replaced by - and / by _.
...
## Standard wallets in the TON Ecosystem
...
- AForm messages you want to send and initiate the transaction. You can send up to 4 messages per request, as detailed in the [advanced manual](/v3/guidelines/smart-contracts/howto/wallet#sending-multiple-messages-simultaneously).
...
If the `forward_amount` is nonzero, a notification about the Jetton reception will be sent to the destination contract. Additionally, if the `response_destination` address is non-null, excess TON coins (called "excesses") will be sent to that address.
...
:::tip
Be careful when calculating fees and amounts for outgoing Jetton messages. For instance, transferring 0.2 TON with no excess may prevent you from receiving 0.1 TON in a response.
:::
...
To deploy a batch, the contract stores information about new NFTs in a dictionary. The process involves: ыetting up storage fees (e.g., `0.05`), obtaining arrays with the new NFT owners and content, retrieving the `next_item_index` using the `get_collection_data method`.
...
DeDust provides two primary paths for exchanging assets: jetton \<-> jetton or TON \<-> jetton. Each has a different scheme. The process involves sending either jettons or TON coins to specific vaults and including a special payload. Below is the process for both types of swaps:
