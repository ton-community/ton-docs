...
<div class="text--center">
  <ThemedImage
    alt="Wallet message flow diagram"
    sources={{
...
- Create a wallet wrapper (object) of the correct version, typically v3r2 (see also [wallet versions](/v3/documentation/smart-contracts/contracts-specs/wallet-contracts)), your secret key and workchain (usually 0 for the [BaseChain](/v3/guidelines/dapps/transactions/foundations-of-blockchain#workchain-and-basechain)).
...
### How to calculate the user's jetton wallet address (off-chain)?
...
<Tabs groupId="code-examples">
  <TabItem value="js-ton" label="JS (@ton)">
    ...
    async function main() {
        ...
        const collectionMetaCell = beginCell()
            .storeUint(1, 8) // we have off-chain metadata
  ...
  <TabItem value="js-tonweb" label="JS (tonweb)">
    ...
    async function main() {
        ...
        nftCommonMetaCell.bits.writeUint(1, 8); // we have off-chain metadata
...
<Tabs groupId="code-examples">
  <TabItem value="js-ton" label="JS (@ton)">
    ...
    we also need to bring the DeDust SDK.
...
swap#e3a0d482 _:SwapStep swap_params:^SwapParams = ForwardPayload;
  ...
  <TabItem value="ton-kotlin" label="ton-kotlin">
    ...
    runBlocking {
        wallet.transfer(pk, WalletTransfer {
            destination = JETTON_WALLET_A // your existing jetton wallet
            bounceable = true
  ...
  <TabItem value="py" label="Python">
    ...
    async def main():
        ...
        swap_params = (begin_cell()
                      ...
                      .store_address(None) # Referral address
...
<Tabs>
  <TabItem value="ts" label="@ton/ton">
    ...
    import { Cell, Address, beginCell, storeMessage } from "@ton/core";
    import { TonClient } from "@ton/ton";
    ...
    const client = new TonClient({
            endpoint: 'https://toncenter.com/api/v2/jsonRPC',
            apiKey: '<YOUR_API_KEY>', // https://t.me/tonapibot
        });
