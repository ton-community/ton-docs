...
2. ADNL connection - Connects to a [liteserver](/v3/guidelines/nodes/running-nodes/liteserver-node). You can host your personal liteserver or rent one and use it directly. Public liteservers are often inaccessible because they are public and have finite resources. However, liteservers cannot provide false data due to built-in validation.
3. Tonlib binary - Also connects to a liteserver, inheriting the same advantages and limitations. However, your application includes a dynamically loaded library compiled externally. Tonlib has to sync for the first time, which takes some time.
