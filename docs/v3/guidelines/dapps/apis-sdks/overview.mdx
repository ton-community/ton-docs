...
- To interact with TON Blockchain and process its data, choose one of the listed [SDKs](/v3/guidelines/dapps/apis-sdks/sdk). The most popular languages for this purpose include:
...
- To integrate user authentication and payment processing with users’ TON wallets, use [TON Connect](/v3/guidelines/ton-connect/overview).
...
- [Dune analytics](https://dune.com/queries?category=canonical&namespace=ton) provides tables with TON data, including raw transactions and messages, jetton events, and DEX trades. Dune allows you to build custom charts and dashboards, fetch query results via API, and set up alerts. Before writing queries, check [this guide](https://dune.com/ton_foundation/ton-quick-start) for best practices, tips, and tricks.
