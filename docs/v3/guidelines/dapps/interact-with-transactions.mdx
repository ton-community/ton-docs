...
## Blockchain HTTP API
### Core services in TON


This section covers services that form the core of TON's native infrastructure. Most third-party API providers are typically built on top of these standard components.
We recommend evaluating the quality of each provider independently, based on their historical performance and how well it aligns with your specific use case.
...
#### TONX API
An API service originally based on TON Center. It has since continued in closed development, expanding its interface to meet client-specific requirements.

#### Tonhub API
An HTTP API service with a unique structure, different from both TON API and TON Center. It is better suited for specific use cases.
