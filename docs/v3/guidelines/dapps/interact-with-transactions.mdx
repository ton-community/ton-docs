...
- Native computed – the basis of this object exists on-chain, but the data is extended on the API side.
- Postprocessed – the data calculated additionally by the API provider based on indexed information.
...
Review the content of transaction body:
* '@fields' - internal maintainence fields in TON Center API/v2.
* Header data - transaction metadata lt, now(unix time)
* Fees - high level information information
* Incoming message data - internal or external incoming message, that initiated transaction
* Outgoind message data - the array, that optionally contents from 0 to 255 messages sent during this transaction
...
| `in_msg`                  | Message         | Incoming message body object. May be internal or external.                                  | `"in_msg": {...}`                                     |
| `out_msgs`                | Message[]       | Array of outgoing messages                                                                  |  `"out_msgs": [{...}, {...}, ...,  {...}]` |
