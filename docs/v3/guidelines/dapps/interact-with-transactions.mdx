...
You can run this script in blueprint to get visual result of difference between bounceable and non-bounceable messages:
...
//
// Option 1: With environment variables in the command line.
// Use a backslash `\` to wrap the command on multiple lines for readability.
//
// WALLET_MNEMONIC="unfold sugar water ... other words" \
// WALLET_VERSION="v4r2" \
// npx blueprint run sendBounceableMessages --testnet --mnemonic
//
//
// Option 2: Using a .env file.
// Create a `.env` file in the `examples` directory with your mnemonic:
//
// WALLET_MNEMONIC="unfold sugar water ... other words"
// WALLET_VERSION="v4r2"
//
// Then run the script without any variables:
// npx blueprint run sendBounceableMessages --testnet --mnemonic
//
// NOTE: We can't use TonConnect (e.g. with the `--tonconnect` flag) for this script
// because wallet extensions (like Tonkeeper) do not allow dApps to control the `bounce`
// flag for security reasons. They always send bounceable messages.
// To have full control, we must create and sign the message manually using a mnemonic.
...
export async function run(provider: NetworkProvider): Promise<void> {
    ...
    if (versionStr === 'v3r1' || versionStr === 'v3r2') {
        ...
        throw new Error('Unsupported wallet version: ' + versionStr + '. Supported versions are v3r1, v3r2, v4r1, v4r2.');
    }
...
Imagine you have to compose a message for some contract, we will use jetton wallet and wallet v3 as example.
There are two types of receiving functions:
...
To serialize an internal message which will be correct for this smart-contract we should check how it deserializes the message:
...
Now we must use it as a guidance to compose the correct message:
...
// How to run this script:
// 1. https://t.me/testgiver_ton_bot
// 2. https://t.me/testnet_usdt_giver_bot
// 3. replace jettonWalletAddress with your own jetton wallet
// 4. npx blueprint run sendInternalMessage --testnet --tonconnect
...
export async function run(provider: NetworkProvider): Promise<void> {
    ...
    const jettonWalletAddress = Address.parse('kQCQm4DAW2QXcwPeo54K1UWn5LVi_v5qTuB9akoTEhpH8y0o');
...
Although you can use `blueprint --tonconnect` to send internal message, you can't use it for external message.

It doesn't expose low-level signing capabilities because:

1. **Security by design**: Wallets (like Tonkeeper) should not expose raw private keys to dApps
2. **Ton Connect abstraction**: The provider abstracts away the underlying wallet implementation
3. **Limited API surface**: Blueprint focuses on contract deployment and testing, not raw message crafting

That is why we will use mnemonic method of the blueprint instead of tonconnect:
...
export async function run(provider: NetworkProvider): Promise<void> {
    ...
    toSign.storeUint(3, 8)            // var mode from func
  ...
  messages: [
{
  ...
  */
  ...
  if (message.info.type !== 'external-in') throw new Error(`Expected an external-in message, got ${message.info.type}.`);
...
This section consider how developer may access set of transaction from blockchain.

### TON HTTP API services

_HTTP API service_ is a software solution that retrive RAW data from Liteserver node and encode this for other service to standard formats.
The one how provide solution calls a HTTP API _provider_.

We will review services that build native infrastructure; regarding API providers most likely rely on one of these standard solutions.
We recommend evaluating a provider's quality independently based on [historical performance](https://tonstat.us/) and suitability for your specific use case.
...
Consider response for API/v2 transaction.
Method https://toncenter.com/api/v2/getTransactions returns raw message data according to transaction TL-B.
...
* @fields - maintainence fields in TON Center API/v2
...
|------------------------|-------------------------------|--------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------|
| `@type`                | Postprocessing                | type of object in API v2                                                                                     | `"raw.transaction"`               |
| `address.@type`        | Postprocessing                | Postprocessed type of address by smartcontract                                                               | `accountAddress`               |
| `account_address`      | Native                        | Account address where the transaction occurred; returned in userfriendly format                              | `"EQDcN89ChMLQe4G-p5Tqk5qhH_PpY9lbhRjXNagmCGsg_9l1"` |
...
| `fee`                  | Native                        | Total amount of fees associated with the transaction. TL-B field: `total_fees`                               | `1207600`                                                   |
...
Method https://toncenter.com/api/v3/transactions returns a huge structure extended with postprocessed data. 
...
Once developer fetched data set of transactions related to point of his interest: filtered by account or set of accounts, he may consider raw data to process events. Read in detail how transaction works and how parce this for various TON in Transaction section.
...
`storage_fees_due` has type `Maybe` because if the contract has a balance, this value is not present. It's only needed to represent debt. `AccStatusChange`, in turn, indicates whether the account status changed after this phase. For example, if the debt exceeds 0.1 TON, the account state will , and after 1 TON — it will be .
