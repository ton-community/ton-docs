...
This extension enables decentralized, [Merkle proof](/v3/documentation/data-formats/cells/exotic#merkle-proof) airdrops without requiring traditional minting processes.
...
4. Update metadata:
  - Add `mintless_merkle_dump_uri` and `custom_payload_api_uri` to the token's metadata according the [metadata standard](https://github.com/ton-blockchain/TEPs/blob/master/text/0064-token-data-standard.md).
...
```
This utility will print the mintless merkle dump cell hash and store the list in `<output-file>` in `<address> <amount> <start_from> <expired_at>` format (one airdrop per line).

You can additionally generate all Merkle proofs (needed for `custom_payload_api_uri`) via the `build/crypto/mintless-proof-generator make_all_proofs <input-boc> <output-file>` command.
...
```
- This utility reads an airdrop file and prints it to the console in the format: `address,amount,start_from,expired_at`.
