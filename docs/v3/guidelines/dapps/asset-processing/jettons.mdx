...
[Batched withdrawals](https://github.com/toncenter/examples/blob/main/withdrawals-jettons-highload-batch.js): allows multiple withdrawals to be processed in batches, ensuring fast and cost-effective transactions.
...
When a centralized service performs a user withdrawal (i.e., sends jettons from its hot wallet to a user wallet), set `forward_ton_amount` > 0 if you want the `transfer_notification` (with optional comment) to be emitted. We recommend 0.001 TON as a practical default so the notification is reliably delivered and easily processed by downstream services. This is a recommendation for interoperability, not a TEP‑74 requirement: any value (including 0) is standard‑compliant; with 0 no `transfer_notification` is sent.
:::

- Refer to the JS library [tonweb](https://github.com/toncenter/tonweb) for the examples.
...
If they are non-mintable, no additional tokens can be minted. To mint jettons for the first time, refer to the [Mint first jetton](/v3/guidelines/dapps/tutorials/mint-your-first-token) page.
...
| `amount`               | coins      | The total jetton amount to transfer.                                                                                                                         |
...
| `forward_ton_amount`   | coins      | Toncoin to forward to the recipient; must be > 0 to send a transfer notification with `forward_payload` and must be less than the Toncoin attached to this message.       |
| `forward_payload`      | maybe cell | Always at least 1 bit in size. If the first 32 bits equal 0x00000000, it is a text comment.                                                                                 |
...
:::info
Recommended `forward_ton_amount` for a jetton transfer with a comment: 0.001 TON.
:::

Finally, to retrieve the Excess (`0xd53276db`) message you must set up `response_destination`.

Sometimes you may get a `709` error when sending a jetton. This error indicates that the amount of Toncoin attached to the message is not enough to send it. Make sure `Toncoin > to_nano(TRANSFER_CONSUMPTION) + forward_ton_amount`, which is usually <0.05 unless the payload being forwarded is very large. The fee depends on various factors, including the jetton code data and whether the recipient needs to deploy a new jetton wallet.

It is recommended to attach extra TON to the transfer message (msg.value) and set your address as the `response_destination` to receive `Excess 0xd53276db` messages. For example, attach 0.05 TON to the transfer message and set `forward_ton_amount` to 0.001 TON (this 0.001 TON is forwarded with the `jetton notify 0x7362d09c` to the recipient and is not returned; only the unused part of the original attached TON is sent back as Excess, minus fees).
...
It is not always possible to determine the exact amount of jettons received from a message, as `transfer_notification`, `excesses`, and `internal_transfer` messages are defined in [TEP-74](https://github.com/ton-blockchain/TEPs/blob/master/text/0074-jettons-standard.md), but their support is optional. This means that some implementations may not send them, so there is no guarantee that such a message can be relied on or decoded.
...
The jetton wallet then sends the jettons to the recipient. It is important to attach some TON (at least 0.001 TON)
...
11. Get a list of the latest unprocessed transactions in the hot wallet account and retry it.
    Learn more here: [Check contract's transactions](/v3/guidelines/dapps/asset-processing/payments-processing#check-contracts-transactions),
    ...
    <details>
      ...
      async def main():
          ...
          transfer_cell = (begin_cell()
                          ...
                          .store_coins(1000000)               # Ton forward amount (0.001 TON)
...
<Tabs groupId="parse-code-examples">
  <TabItem value="@ton/ton" label="JS (@ton/ton)">
    <details>
      <summary>
        Source code
      </summary>
