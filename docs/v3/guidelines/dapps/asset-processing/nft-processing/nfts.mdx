...
5. `forwardAmount` - `VarUInteger 16` - The amount of TON forwarded with the message (typically 0.01 TON). This funds an internal notification message to the `newOwnerAddress` upon successful receipt of the NFT.
6. `forwardPayload` - `Either Cell ^Cell` - Optional data included in the ownership_assigned notification message.
...
- Verify all new transactions and ignore those with a body length less than 363 bits (OP - 32, QueryID - 64, Address - 267; internal addresses are 267 bits).
