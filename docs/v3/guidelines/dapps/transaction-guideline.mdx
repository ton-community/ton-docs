...
#### Ожидаемый результат
Последние 10 транзакций по указанному адресу, включая хэш транзакции
и если есть входящее сообщение — также выводит хэш этого сообщения:
...
1. уточнить и прописать зачем нам нужен ненормализованный хэш. Получается просто, что обычно так вычисляем, а надо как выше показано?
...
Этот алгоритм может быть любым отличным от нормализованного. Если отсутствие нормализации стало проблемой проекта который был ранее разработан, то правильно сделать миграцию на нормолизованные хеши во всем проекте.
...
console.log('Length:', stateInitBase64.length);
...
Вычисленный адрес: UQA_B407fiLIlE5VYZCaI2rki0in6kLyjdhhwitvZNfpe-pd
❌ StateInit НЕ соответствует адресу!
```
...
| `gas_credit` | `Maybe (VarUInteger 3)` | Используется в external сообщениях. Поскольку они не могут нести при себе TON, то в этом случае выделяется немного газа для того, чтобы код смарт контракта мог запуститься и решить, готов ли он продолжать выполнение, используя уже свой собственный баланс. |
| `exit_code` | `int32` | Код выхода виртуальной машины. Если код равен 0 или 1 (alternative success), то код смарт-контракта был успешно выполнен. Если код не равен 0, то это означает, что код смарт-контракта завершился с ошибкой (исключением является применение `commit`). <br></br> Часто для удобства люди используют выражение "код выхода смарт контракта", хотя на самом деле это не совсем верно. |
...
Эти actions выполняются в том порядке, в котором они были созданы в рамках выполнения кода. Всего максимум может быть создано [255 actions](https://github.com/ton-blockchain/ton/blob/72056a2261cbb11f7cf0f20b389bcbffe018b1a8/crypto/block/transaction.h#L165).
...
После ознакомления с APIv2 и APIv3, может возникнуть вопрос, какой из них использовать. Ответ на этот вопрос зависит целиком от решаемых задач. В качестве рекомендации можно использовать `JSON-RPC` протокол из APIv2, так как в таком случае можно будет использовать готовый SDK, который уже содержит в себе все необходимые методы и типы.

В случае, если этот функционал не покрывает все задачи, следует рассматривать полный или частичный переход на APIv3, либо другие API в экосистеме, которые могут предоставлять больше информации.
...
Ранее мы говорили об actions с технической точки зрения. Однако часто в различных API сервисах этот термин используется для обозначения пользовательских операций. Тут важно вспомнить, что TON является асинхронным блокчейном, поэтому одна операция может состоять из цепочки транзакций. В данном контексте важна совокупная картина, а не отдельные транзакции.

Например, один перевод Jetton с одного кошелька на другой осуществляется минимум за 3 разных транзакции. Каждый сервис называет эти цепочки по-своему: action, event, operation. Во избежание путаницы с другими терминами, которые уже ранее встречались выше, мы будем использовать термин `Event`.

:::tip
Следует учитывать, что не каждый Jetton Transfer может быть назван `Jetton Transfer Event`. Например, если вы отправляете жетоны на DEX для получения других жетонов, то это уже будет считаться `Swap Event`. 

Таким образом контекст может сильно меняться в зависимости от того, что вы делаете с жетонами. API сервисы заранее парсят `forward_payload` и проверяет большое количество других параметров, чтобы определить точную операцию в удобном виде отобразить пользователю. Это относится не только к жетонам, но и к NFT и другим операциям.
:::
...
В качестве простого примера рассмотрим обычный перевод TON с одного кошелька на другой. Это совокупность двух транзакций:

1. Кошелёк отправителя получает внешнее сообщение с подписью для верификации. После обработки этого сообщения, смарт контракт отправителя создаёт выходные сообщения, указанные в теле внешнего сообщения.
2. Кошелёк получателя получает входящее сообщение с нужным количеством TON.

Для того, чтобы понять, что перевод был успешным, логично предположить, что мы должны проверить вторую транзакцию. Если на аккаунте получателя есть транзакция, которую инициировало входящее сообщение с нашего кошелька и при этом эти средства не вернулись обратно (ниже это будет рассмотрено), то мы можем считать, что перевод был успешным.
...
NFT представляет собою смарт контракт, который хранит в себе контент. Это может быть [как ссылка на контент, так и сам контент](https://github.com/ton-blockchain/TEPs/blob/master/text/0064-token-data-standard.md). Так же, данный контракт хранит в себе адрес владельца. Передача NFT означает смену значения этого поля.

Для этого владельцу требуется отправить специальное сообщение, собранное согласно [TL-B схеме](https://github.com/ton-blockchain/TEPs/blob/master/text/0062-nft-standard.md#tl-b-schema), которая описана в [TEP-0062]:
...
Здесь всё схоже с Jetton Transfer, за исключением того, что данный event считается успешным, если поле, хранящее адрес владельца, изменился на нужный. В случае, если был указан `response_destination`, то остатки возвращаются на этот адрес:

```tlb
excesses query_id:uint64 = InternalMsgBody;
```

Также, если был указан `forward_amount`, то указанное количество TON вместе с `forward_payload` будет отправлено новому владельцу NFT:

```tlb
ownership_assigned query_id:uint64 prev_owner:MsgAddress forward_payload:(Either Cell ^Cell) = InternalMsgBody;
```

Поле `custom_payload` не нужен в обычных NFT и заложено лишь для того, чтобы при необходимости в каких-то более сложных NFT контрактах можно было передавать данные, не теряя соответствие со стандартом.

Для получения владельца NFT, мы будем использовать GET метод `get_nft_data`:
...
Это означает, что нам не нужно будет разбирать транзакции и проводить сложные проверки. Мы просто получим данные о NFT и посмотрим на адрес владельца:
...
:::note
Как и в двух предыдущих примерах, мы должны учитывать, что это лишь облегчённый пример и в реальных условиях вся система будет сложнее. Для получения более подробной информации, вы можете ознакомиться с [отдельной статьёй](/v3/guidelines/dapps/asset-processing/nft-processing/nfts).
:::

### Как исследовать контракт и его схему в TL-B, чтобы построить логику для анализа успешной операции

В этом разделе мы на основе [исходного кода Jetton Wallet контракта](https://github.com/ton-blockchain/token-contract/blob/main/ft/jetton-wallet.fc) и [TL-B схемы](https://github.com/ton-blockchain/TEPs/blob/master/text/0074-jettons-standard.md#tl-b-schema) рассмотрим, как нам понять, что необходимо проверить для того, чтобы убедиться, что операция была целиком успешно завершена. Для этого нам нам сперва необходимо проанализировать в общих чертах доступные операции в контракте:
...
В этом коде мы видим, что есть две операции, связанные с отправкой и получением жетонов:

1. `op::transfer` - это операция, которая осуществляется при отправке жетонов. Она вызывает функцию `send_tokens`, которая отвечает за отправку жетонов на другой кошелёк.
2. `op::internal_transfer` - это операция, которая осуществляется при получении жетонов. Она вызывает функцию `receive_tokens`, которая отвечает за получение жетонов на кошелёк.

Разберём отправку. Для начала посмотрим, что представляет собою входящее сообщение:
...
Здесь мы видим, что данное сообщение довольно мало отличается от `internal_transfer`, который мы уже рассматривали ранее. Здесь указывается, кому и сколько жетонов отправить, кому вернуть остаток средств и при необходимости, отправить дополнительный payload получателю жетонов.

Теперь посмотрим на функцию `send_tokens`, которая отвечает за отправку жетонов:
...
Хоть и код может показаться сложным, он довольно прост:

1. Для начала считываются необходимые поля из `in_msg_body`, что представляет собой тело входящего сообщения.
2. На основе этих полей проверяется, что отправитель является владельцем жетонов и что на балансе достаточно жетонов для отправки.
3. На основе адреса получателя высчитывается адрес жетон кошелька получателя.
4. Проверяется, что ко входящему сообщению было приложено достаточно TON для оплаты комиссии за отправку жетонов (и даже для их возврата в случае какой-либо ошибки), а так же, если необходимо, то для доставки `forward_payload`.
5. Создаётся сообщение, которое будет отправлено на кошелёк получателя жетонов. Это сообщение содержит в себе уже знакомое нам `internal_transfer` сообщение.
6. Контракт обновляет своё хранилище.
...
В этой функции мы видим, что:

1. Контракт считывает необходимые поля из тела входящего сообщения.
2. Контракт пополняет счётчик баланса жетонов на указанное количество.
3. Проверяется, что отправителем действительно является валидный кошелёк, либо жетон мастер.
4. Производятся некоторые вычисления для газ мененджмента, чтобы обрабатывать комиссию за хранилище контракта.
5. Если в сообщении указано `forward_ton_amount`, то создаётся сообщение, которое отправляется уже самому получателю жетонов.
6. Если указано поле `response_address`, то создаётся сообщение, которое отправляется на этот адрес с остатком средств.

Теперь мы знаем, что баланс получателя пополняется только после успешного выполнения транзакции, которая была инициирована сообщением `internal_transfer`. В случае, если произошла ошибка, то сгенерируется bounce сообщение на жетон кошелёк получателя. Рассмотрев код контракта, мы можем заметить, что обработка этого сообщения предусмотрена:
...
При получении bounce сообщения, контракт вызывает функцию `on_bounce`:
...
Здесь мы видим, что контракт просто возвращает жетоны на баланс. В данном случае нам не нужно проверять то, от кого пришло сообщение, поскольку bounce сообщение невозможно подделать. Если мы получили такое сообщение, то это означает, что когда-то мы отправляли сообщение с указанным содержимым, поэтому мы спокойно можем прочитать количество жетонов и вернуть на баланс.