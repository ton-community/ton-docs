...
В отличие от большинства синхронных моделей, в TON контракты не вызывают друг друга напрямую. Вместо этого акторы отправляют сообщения друг другу, и каждый актор обрабатывает сообщение внутри себя независимо от других акторов. Это значит, что один контракт не может получить прямой доступ к результату исполнения другого. Он узнает результат только через отдельное входящее сообщение, если оно будет.

> В отличие от синхронных блокчейнов, где можно "сделать вызов → получить ответ → принять решение", в TON каждая обработка сообщения — это изолированный внутри актора процесс. Результат этого процесса, в зависимости от наличия отправленного сообщения, может прийти позже или не прийти вовсе, если контракт не отправит ответное сообщение.
...
Смарт-контракт в TON — это аккаунт, находящийся в активном состоянии `active`, у которого:
...
<BlockMath math="{Address} := ({workchain\_id},{account\_id})" />, где <BlockMath math= "{account\_id}:= {SHA256(BoC(StateInit))}" />, где BoC (bag of cells) или сумка с ячейками это формат сериализации, который кодирует ячейки в массивы байтов, как определено схемой TL-B.

Такой адрес соответствует определённому начальному состоянию контракта и может быть вычислен до его развёртывания. Это позволяет использовать его до фактической инициализации — например, для приёма токенов или подготовки взаимодействий.
