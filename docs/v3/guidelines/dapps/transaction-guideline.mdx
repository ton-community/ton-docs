...
**Смарт-контракт** — это аккаунт с программным кодом и персистентными данными, который автоматически выполняет логику при получении сообщений. Смарт-контракт — это частный случай аккаунта с кодом.
...
* Баланс в основной валюте (TON) — всегда должен быть > 0 для активности аккаунта.
* Баланс в других токенах (например, Jettons) — может быть нулевым.
* Код смарт-контракта (или его хеш) — присутствует только у аккаунтов со смарт-контрактом.
* Персистентные данные смарт-контракта (или их Merkle-хеш) — состояние, сохраняемое между вызовами.
* Статистика по занятости персистентного хранилища (число ячеек и байт).
* Номер блока мастерчейна, когда в последний раз оплачивалось хранение.
* Публичный ключ (или проверочный код подписи) для отправки сообщений.
* Очередь исходящих сообщений и список недавно обработанных сообщений.
* Не все поля обязательны для всех аккаунтов. Например, простой кошелёк не содержит кода и персистентных данных.

Все данные хранятся в TVM в виде набора ячеек персистентного хранения.

#### Адрес

Адрес в блокчейне TON — это уникальный идентификатор любого аккаунта или смарт-контракта. Он используется для получения сообщений, перевода средств и взаимодействия с другими участниками сети. Адрес формируется на основе следующих компонентов:
...
`state_init` — это минимальный набор данных, необходимый для создания аккаунта и генерации его адреса. Он включает:

* код смарт-контракта (если есть),
* начальные данные (data),
* публичный ключ (если есть),
* служебную (?) информацию (например, флаги и параметры инициализации).

Адрес определяется как хеш этого `state_init`. Поэтому даже небольшое изменение кода или данных приведёт к совершенно новому адресу.
...
Все перечисленные понятия — актор, аккаунт, смарт-контракт и адрес — представляют собой разные аспекты одной базовой сущности в TON. Для единообразия терминологии и во избежание неоднозначностей в данном документе используется термин **аккаунт** как общее обозначение всех подобных сущностей.
...
MasterChain критически важен для функционирования всей сети, но из-за высокой стоимости используется в основном для системных целей.
