...
**Аккаунт в TON** — это сущность, реализующая модель актора. Аккаунт состоит из трёх компонентов:

1. Уникальный адрес `addr` - идентификатор аккаунта в сети, используемый для отправки и получения сообщений.
2. `storage_stat:StorageInfo` - структура, отражающая текущее потребление ресурсов аккаунтом и его долговые обязательства.
3. `storage:AccountStorage` — текущее содержимое аккаунта, включающее:
  * `last_trans_lt:uint64` — логическое время последней успешной транзакции.
  * `balance:CurrencyCollection` — баланс аккаунта в TON и других токенах.
  * `state:AccountState` — внутреннее состояние аккаунта:
      * `uninit` — не иницилизирован, т.е., аккаунт не содержит кода;
      * `active` — активен, т.е., аккаунт содержит код и данные смарт-контракта;
      * `frozen` — аккаунт заморожен и не может исполняться.
...
**Смарт-контракт** — это аккаунт с программным кодом и персистентными данными, который выполняет логику при получении сообщений. Смарт-контракт — это аккаунт, в котором записан код и состояние.
