...
In TON, transactions are triggered by messages. How you construct and send them depends on where the signing key resides:

### For frontend

- No private key
- Prepare the **message body (payload)** for an internal or external message.
- The user's wallet (through TON Connect) **signs the message** and submits it to the blockchain.
- The DApp never accesses the private key directly.

**Summary:** prepares `InternalMsgBody` or external message payload → wallet signs → message submitted.

### For backend

- Has the private key
- Build and **sign the message** directly on the server.
- Send it to the blockchain without involving an external wallet.

**Summary:** prepares `InternalMsgBody` → server signs → message submitted.
