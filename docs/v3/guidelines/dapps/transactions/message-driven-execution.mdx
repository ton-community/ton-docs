...
**Note**: TON Connect (e.g., the `--tonconnect` flag) cannot be used here because wallet extensions like Tonkeeper do not allow DApps to control the `bounce` flag for security reasons. Wallets typically send bounceable messages by default, but the final behavior depends on the address format (e.g., a `UQ…` address results in a non-bounceable message). To maintain this flag, you need to create and sign the message manually using a mnemonic.
...
A pre-configured example is available for testing the sending process.
