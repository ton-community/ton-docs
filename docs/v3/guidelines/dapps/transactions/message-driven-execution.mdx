...
description: "Learn how TON executes smart contracts through message passing, actor-based processing, and how messages trigger transactions."
keywords:
  - TON messages
  - TON internal messages
  - TON external incoming messages
  - TON external-in messages
  - TON external outgoing messages
  - Bounce messages
  - Serializing messages
faq:
  - "Why does my contract not answer right away?"
  - "Why do transactions show up later than I expect?"
  - "What’s the difference between an internal message and an external one?"
  - "What happens if a message never gets answered?"
  - "Can I make my contract return a value instantly?"
  - "How to detect if a message bounced or failed?"
---
...
In TON, this data is represented using **cells**, and **TL-B schemas** define how it is serialized into these cells.


## Native message structure

All messages adhere to a unified schema that defines their structure and serialization:
...
Since an external-in message is sent into the blockchain from the outside, the initiator is an external actor. This can be either a wallet app or custom code interacting with blockchain API services.
...
Prepare a wallet application (e.g., Tonkeeper) that you’ll use to send the external message and set up a Blueprint project locally by following the [setup instructions](/v3/guidelines/quick-start/developing-smart-contracts/setup-environment).
:::
...
Use one of the methods below to run the script (examples use wallet V4R2, a widely adopted version):

#### Option 1: using environment variables on the command line
...
**Note**: [TON Connect](/v3/guidelines/ton-connect/overview) (e.g., the `--tonconnect` flag) cannot be used here because wallet extensions like Tonkeeper do not allow DApps to control the `bounce` flag for security reasons. Wallets typically send bounceable messages by default, but the final behavior depends on the address format (e.g., a `UQ…` address results in a non-bounceable message). To maintain this flag, you need to create and sign the message manually using a mnemonic.
...
const messageBody = beginCell()
    ...
    .storeAddress(destinationAddress) // new owner
    .storeAddress(destinationAddress) // response destination for excess funds
...
Suppose you need to compose a message for a smart contract. As an example, we'll use the Jetton wallet and wallet V3 contracts, with V3 highlighted as a standard version often used in examples for its straightforward structure.

There are two types of receiving functions commonly used:

1. [() recv_internal(cell in_msg_full, slice in_msg_body) impure](https://github.com/ton-blockchain/jetton-contract/blob/3d24b419f2ce49c09abf6b8703998187fe358ec9/contracts/jetton-wallet.fc#L187)
...
2. [() recv_external(slice msg_body) impure](https://github.com/aSpite/wallet-tutorial/blob/73c213add90ff115e45e58d0bcd9f1ea1fd44c89/JavaScript%20-%20TypeScript/src/wallet_v3.fc#L7)
...
In TON, typically, transactions are triggered by messages. The approach of preparing and sending messages depends on which side the signing key is used:

### For frontend

In this case, the service doesn't use its own private key to sign an external message in the blockchain; instead, it provides the user with a prepared payload that should be sent. These services never access the users' private key directly.

1. User confirms some business event in a service.
2. A service prepares the **external message body (payload)**.
3. A user's wallet app receives a prepared payload through TON Connect.
4. User **signs the message** in the wallet app and submits it to the blockchain.
5. Service track blockchain state and perform further steps for this business process.

In this approach, the service doesn't sign messages, but rather its users do.

Example: NFT market DApp - NFT sale.

### For backend

The service sends external messages signed by its own private key.

1. A user or event triggers a specific business event.
2. A service prepares and signs an external message according to the inner business logic.
3. A service sends a signed external message to the blockchain.
4. A service tracks blockchain state and reports on completed business events.

In this approach, the service signs its external messages without the involvement of a third party.

Example: Centralized exchange - deposit and withdrawals.
