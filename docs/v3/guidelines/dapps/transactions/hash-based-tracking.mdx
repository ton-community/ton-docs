...
The transaction structure is defined in TL‑B as follows.<br/>
...
For how a hash is computed from this structure, see:
...
- A practical section on retrieving hashes in the [DApp cookbook](/v3/guidelines/dapps/cookbook#how-to-find-transaction-or-message-hash).

:::info
A transaction hash is the representation hash (SHA‑256) of the root `Transaction` cell serialized per TL‑B exactly as it appears in the block, so any change to messages, fees, or state update alters the hash.
:::
