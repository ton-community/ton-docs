...
async function sendAndVerifyMessage() {
  ...
  // Log on-chain state before to be sure that the wallet is deployed
  console.log(
  ...
  // Build external-in message and compute hashes
  const message: Message = {
    info: {
      type: "external-in",
      src: null,
  ...
  // Send the external message using high-level helper with the prebuilt body (with retry)
  await retry(() => client.sendExternalMessage(walletContract, transfer), {
    retries: 6,
    delay: 1500,
    progress: true,
  ...
  // Wait until the transaction with matching normalized incoming message appears (using retry)
