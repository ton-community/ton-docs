...
description: "Learn how to find a transaction in TON by hash, the difference between message hash and transaction hash, and how to normalize hashes to avoid inconsistent hashes."
keywords:
...
This article explains how to find a transaction linked to an external-in message in the TON blockchain.
...
it involves serialization and internal formatting steps.

Each cell, message, or transaction has its unique hash, which acts as a cryptographic fingerprint or identifier.
...
import { storeMessage, beginCell, Message } from "@ton/ton";
...
**When to use the full message hash:**
...
Normalization is a standardization process that converts different representations into a consistent format. While messages across interfaces follow the TL-B scheme, structural differences in implementation sometimes lead to inconsistent hashes across implementations.
...
const client = new TonClient({
  endpoint: "https://testnet.toncenter.com/api/v2/jsonRPC",
  apiKey: "your api key", // Get your API key via the @toncenter bot
});

// Construct the message body — arbitrary opcode and payload string
const messageBody = beginCell()
  .storeUint(0, 32) // opcode — arbitrary, e.g., 0
...
async function sendAndVerifyMessage() {
  ...
  // Wallet selection
  // - V5R1 on Testnet:    walletId.networkGlobalId = -3   (may need to be set)
...
<details>
  <summary>Obtaining BoC and hash using a React example</summary>
...
export function TxForm() {
  ...
  const send = async () => {
    ...
    try {
      ...
      setBoc(receivedBoc);
      const externalMessageCell = Cell.fromBase64(receivedBoc);
