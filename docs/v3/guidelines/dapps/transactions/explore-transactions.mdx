---
title: "Transactions"
description: "Understand the structure and phases of TON transactions: credit, storage, compute, action, and bounce. Learn how to analyze and retrieve them via APIs."
keywords:
  - TON transaction
  - TON transaction phases
  - TON credit phase
  - TON storage phase
  - TON compute phase
  - TON action phase
  - TON bounce phase
  - TON transaction analysis
  - TON transaction API
  - TON TL-B
  - TON events
faq:
  - "What is a transaction in TON?"
  - "How to access transaction data?"
  - "Why does the transaction seem to have failed, while the transfer seems executed successfully?"
  - "How to determine if a transaction was successful or not?"
...
| `bounce`       | `Maybe TrBouncePhase`  | The bounce phase, responsible for handling errors that occurred during the `compute_ph` or `action` phases.                                                                                                                                                                                                                                                                                           |
...
These actions are executed _in the order in which they were created_ during code execution. A total of up to [255 actions](https://github.com/ton-blockchain/ton/blob/cac968f77dfa5a14e63db40190bda549f0eaf746/crypto/block/transaction.h#L164) can be made.
...
| `no_funds`          | `Bool`            | Indicates lack of funds to execute actions. If `true`, the action phase was interrupted due to insufficient funds.                                                                                  |
...
| `spec_actions`      | `uint16`          | Number of special actions (all except `action_send_msg`).                                                                                                                                            |
...
### Summary: determining transaction and event success

The success of a **transaction** is determined by the **successful completion** of all its phases, additionally:
...
The success of **events** depends on the operation type:

- **Ordinary TON transfer:** the inbound message must be internal; if a bounce occurs, it must be processed correctly.
- **Jetton transfers:** the transaction inited by an `internal_transfer` message must complete successfully.
- **NFT transfers:** the owner address must be updated to the expected new owner.

Bounce messages generated by the contract indicate failed attempts and return funds to the sender, which must also be considered when assessing success.
