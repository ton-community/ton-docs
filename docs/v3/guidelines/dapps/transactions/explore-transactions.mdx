...
In the TON blockchain, any change to an account's state is recorded via a **transaction**. Unlike messages, transactions do not _move_, _send_, or _receive_ anything. These terms are often confused, but it's important to understand that a transaction is simply a **record** of all changes that occurred to a specific account.
