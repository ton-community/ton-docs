...
### Summary: determining transaction success

The success of a transaction is determined by the **successful completion** of all its phases, especially:

- **Compute phase** - the smart contract executed without errors. Exit code `0` indicates standard (successful) execution.
- **Action phase** - actions were created and applied correctly. Exit code `0` indicates standard (successful) execution.

Additional criteria depend on the type of operation:

- **Ordinary TON transfer:** inbound message must be internal; if a bounce occurs, it must be processed correctly.
- **Jetton transfers:** the third transaction (`internal_transfer`) must complete successfully.
