...
:::note
Account balances in TON never become negative. [If storage fees exceed the available balance](https://toncenter.com/api/v3/transactions?hash=f3e8c9c48dd055a533e7c5d0735b2e305eaf18e571d842d607e0fc5ef0eb45ef&limit=1&offset=0&sort=desc), the balance is set to zero and the unpaid remainder is recorded as debt — `due_payment` in `StorageInfo` (account state) and `storage_fees_due` in transactions.
:::
...
Example: `(ID1, 0 TON, nonexist)` or `(ID2, 2 TON, active, last_tx, data, code)`
...
- Account properties (ID, balance, state, `last_tx`)
- Data
- Code
...
In TON, a smart contract is typically recognized as:

- The program code executed, as described in FunC, Tact, or Tolk, on the blockchain.
- The account that implements business logic and contains code.
...
- **Account ID** — a 256-bit hash of the `StateInit` structure, which includes the contract’s initialized `code`, `data`, and, optionally, a `library`. It is stored as an arbitrary `SliceData` and match the hash of the first deployed `StateInit`.
