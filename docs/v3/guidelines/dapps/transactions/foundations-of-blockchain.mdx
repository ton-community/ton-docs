...
While **account's state** captures the account’s underlying content— _its code and data_ — **account's status** abstracts these details to indicate whether an account is _uninitialized, active, frozen, or nonexistent_.
