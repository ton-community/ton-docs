...
| `frozen` | `01`      | The account is frozen: its code and data are deleted, but a hash of the previous state is retained. It can't be used, but it can be restored. The account has zero or unpaid balance, or doesn't meet the WorkChain’s minimum requirement. |
...
![](/img/docs/blockchain-fundamentals/account_status.svg)
