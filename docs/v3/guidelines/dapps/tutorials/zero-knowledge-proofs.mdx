...
Since December 2023, BLS12-381 operations have been available on Mainnet. Due to the complex computations required for the pairing algorithm, the TON Virtual Machine (TVM) needed to be upgraded with new opcodes to support proof verification. This functionality was added in the [TVM upgrade Jul 2023](/v3/documentation/tvm/changelog/tvm-upgrade-2023-07#bls12-381).
:::
...
- Performing a trusted setup using a [multi-party computation (MPC)](https://en.wikipedia.org/wiki/Secure_multi-party_computation) protocol to generate proving and verification keys (using Powers of Tau (PTAU)),
...
1. Create a new project called "simple-zk" using [Blueprint](https://github.com/ton-org/blueprint) by running the following command. After that, enter a name for your contract (e.g., ZkSimple) and then select the first option (using an empty contract).
...
3. Install the libraries needed for zk-SNARKs

```bash
npm i -D snarkjs ffjavascript
npm i -g circom
```
...
First, let’s create a folder named `simple-zk/circuits`. Inside this folder, create a new file `simple-zk/circuits/test.circom` and add the following code:
...
Next, we’ll create a folder for build artifacts and navigate into it. While inside the `simple-zk` folder, run the following commands:
...
### 💪 Creating a trusted setup with Powers of Tau (PTAU)
...
Then, finalize (generate) the final zkey:
...
After the above steps, the `build/circuits` folder should be displayed as follows:
...
Next, there are several simple utility functions. These functions process and load proof data sent to the contract.
...
The last part is the `groth16Verify` function that verifies the validity of the proof sent to the contract.
...
=======
Next, we need to edit two files inside the `wrappers` folder. The first file that needs our attention is the `ZkSimple.compile.ts` file (If a different contract name was chosen in Step 1, update the filename accordingly). We need to add `verifier.fc` to the list of contracts that must be compiled.
>>>>>>> pitaya/doc-fix/run_20250901_144329_2bdedc55/kb6219427
...
The other file that needs attention is `ZkSimple.ts`. We need to first add the `verify` opcode to the `Opcodes` object:
...
Let's open the `ZkSimple.spec.ts` file inside the `tests` folder and write a test for the verify function. The test is conducted as follows:
...
- If you run the test, the result will be a TypeScript error because we don't have a declaration file for the modules `snarkjs` and `ffjavascript`. This can be addressed by editing the `tsconfig.json` file in the root of the `simple-zk` folder. Change the `strict` option to `false` in that file.
...
- How to use MPC and the Powers of Tau (PTAU) to generate verification keys.
...
- [TVM upgrade Jul 2023](/v3/documentation/tvm/changelog/tvm-upgrade-2023-07)
...
- [Tonnel network: privacy protocol on TON](https://github.com/saberdotcoder/tonnel-network)
