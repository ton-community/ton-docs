...
![NFT collection preview](/img/tutorials/nft/collection.png)
...
We will use Pinata as our IPFS storage system, so you also need to create an account on [pinata.cloud](https://pinata.cloud) and get an API key and API secret (e.g., `PINATA_API_KEY` and `PINATA_API_SECRET`). The official Pinata [documentation](https://docs.pinata.cloud/account-management/api-keys) can help with that. Once you have these API tokens, I'll be waiting for you here!

## 💎 What is an NFT on TON?

Before starting the main part of our tutorial, we need to understand how NFTs work on TON. Unexpectedly, we will first explain how NFTs work on Ethereum (ETH), to highlight the uniqueness of NFT implementation on TON compared to other blockchains.

### NFT implementation on ETH

The NFT implementation on Ethereum is straightforward. There is 1 main contract for the collection, which stores a simple hashmap containing the NFT data for that collection. All requests related to this collection (such as transferring an NFT, putting it up for sale, etc.) are sent directly to the single contract.

![Ethereum NFT implementation diagram](/img/tutorials/nft/eth-collection.png)

### Problems with this implementation on TON
...
![TON NFT collection implementation diagram](/img/tutorials/nft/ton-collection.png)
...
```bash npm2yarn
npm install @pinata/sdk dotenv @ton/ton @ton/crypto @ton/core buffer
```
...
You can get a TON Center API key from [@tonapibot](https://t.me/tonapibot) and choose mainnet or testnet. Store the 24-word seed phrase of the collection owner's wallet in the MNEMONIC variable. Note: Do not commit `.env` files to version control, keep your mnemonic secure, and use test wallets for tutorials.

Great! Now we are ready to start writing code for our project.

### Write helper functions

First, let's create the `openWallet` function in `src/utils.ts`. This function will open our wallet using a mnemonic and return its public/secret key pair.
...
First, we need to store NFT images in `./data/images/` and name them `0.png`, `1.png`, ... for photos, and `logo.png` for avatars of our collection. You can either [download the pack](/img/tutorials/nft/ducks.zip) of duck images or use your own images. Store metadata files in `./data/metadata/`.
...
![Collection metadata example](/img/tutorials/nft/collection-metadata.png)
...
{
  "name": "Ducks on TON",
  "description": "This collection was created to demonstrate minting an NFT collection on TON. You can support the creator by buying one of these NFTs.",
...
![Item metadata example](/img/tutorials/nft/item-metadata.png)
...
Now let’s write some code that will upload our metadata files to IPFS. Create a `metadata.ts` file in the `src` directory and add all needed imports:
...
After that, we assign the value `ipfs://{imagesIpfsHash}/{index}.png` to the image field. If the file is the last one in the folder, use `ipfs://{imagesIpfsHash}/logo.png`. Then rewrite the file with the new data.
...
After that you can run the app and see the link to your deployed metadata!

### Encode off-chain content

How will our metadata files stored in the smart contract be referenced? This question can be fully answered by the [Token Data Standard](https://github.com/ton-blockchain/TEPs/blob/master/text/0064-token-data-standard.md).
...
Finally, we need to create a function that will encode the off-chain content into cells using these functions:
...
We will create a file to store all logic related to our collection in `src/contracts/NftCollection.ts`. As always, we start with imports:
...
| commonContentUrl     | The base URL for NFT item metadata                                            |
...
Now, let's add a few import statements to `app.ts`:
...
```ts
console.log("Start deploying the NFT collection...");
...
Create a new file: `src/contracts/NftSale.ts`. Declare a type that describes the sale contract data.
...
At the beginning of `app.ts`, add:
...
Go to `https://testnet.getgems.io/collection/{YOUR_COLLECTION_ADDRESS_HERE}` and look at these perfect ducks!
