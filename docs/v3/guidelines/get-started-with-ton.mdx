...
:::tip
Remember, if you are adding new modules with `import`, that you may need to install them by executing the command `npm i @orbs-network/ton-access` in the [terminal](/v3/guidelines/get-started-with-ton#running-scripts).
:::
...
// ... previous code
  ...
  console.log("If someone else sends a transaction before you, the seed will change, and you'll have to find the hash again!");
  console.log(' ');
...
As a solution, the most brilliant minds in the world created a QR code generator specifically for use in terminal. With this tool, you can easily scan the payment link from your Tonkeeper wallet to send a transaction.
