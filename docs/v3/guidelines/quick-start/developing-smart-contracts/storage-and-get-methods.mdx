...
If manually serializing and deserializing the storage cell becomes inconvenient, a common practice is to define two wrapper methods that handle this logic. If you haven't modified the smart contract code, it should include the following lines:
...
4. Add an additional 256-bit integer to our storage

The modified implementation should appear as follows:
...
The main purpose of `get methods` is to provide external read access to storage data through a convenient interface, primarily for extracting information needed to prepare **transactions**.
...
Next, implement a method to call the new `get_counters` smart contract function, which retrieves both counter values in a single request:
...
1. Initialize contract storage by creating a `helloWorldConfig` with test values
