...
Now we are ready to move on to the main functionality of smart contracts - **sending and receiving messages**. In TON, messages are not only used for sending currency, but also as a data-exchange mechanism between smart contracts, making them crucial for smart contract development.
...
Let's create and modify our smart contract following the standard steps described in the previous [Blueprint SDK overview](/v3/guidelines/quick-start/developing-smart-contracts/blueprint-sdk-overview/) section.

### Step 1: Creating and modifying Tact contract

First, let's create the Tact contract in the same directory as the previous one.
...
For accepting messages with an empty body, you can add a `receive` function with no arguments:
...
Tact also provides handy ways to share the same logic through [traits](https://docs.tact-lang.org/book/types/#traits). To ensure that only the contract owner can send messages, use the `Ownable` trait, which provides built-in ownership checks:
...
:::note
The get function cannot be called from another contract.
:::
...
Now let's ensure that our smart contract code fails when we try to send the `add` message from a non-owner:
 - Create `CounterInternal` with some owner.
 - Create another smart contract that will have a different address - `nonOwner`.
 - Try to send an internal message to `CounterInternal` and ensure that it fails with the expected `exitCode` and the counter field remains the same.
...
Don't forget to verify that all examples are correct by running the test script:
...
This function, upon receiving an external message, will increment our `ctxCounter` and also send an internal message to the specified address with the `increase` operation, which we will use to increment the counter on our `CounterInternal` smart contract.
