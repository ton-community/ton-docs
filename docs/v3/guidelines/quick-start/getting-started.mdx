...
:::tip
This might seem unintuitive, because you already created your **"wallet"** in `wallet app`, but, like any other `smart-contract`, wallets in TON need to be deployed to the network. This process requires some fees, and therefore, most `wallet apps` don't actually deploy the wallet `smart-contract` until you receive funds on your address and try to make your first transaction.
:::
