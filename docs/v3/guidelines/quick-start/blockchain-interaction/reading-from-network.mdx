...
- `state`: Can be one of:
  ...
  - `frozen`: The address is locked when storage fees exceed the balance. The balance becomes zero; unpaid storage fees are tracked as debt (see `StorageInfo.due_payment` and transaction `storage_fees_due`).
