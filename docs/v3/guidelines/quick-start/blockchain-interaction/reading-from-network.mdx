...
By the end, you'll understand how to interact with [TON HTTP-based APIs](/v3/guidelines/dapps/apis-sdks/api-types). In this guide, [TON Center](https://toncenter.com/) is used—a fast and reliable HTTP API for TON.
...
- `state`: Can be one of:
  ...
  - `frozen`: The address is locked when storage fees exceed the balance + 0.1 TON. The balance becomes zero; unpaid storage fees are tracked as debt (see `StorageInfo.due_payment` and transaction `storage_fees_due`).
- `code`: The contract's code in raw format.
- `data`: Serialized contract data stored in a [_Cell_](/v3/concepts/dive-into-ton/ton-blockchain/cells).
...
To get account information, you need the account's address. In TON, we can use a user-friendly address — a base64 or base64url-encoded string that represents a blockchain account in a human-readable and error-resistant format.
...
In the given code snippets, replace the example with your Testnet address in this line.
...
The result of calling a get method via the TON HTTP API is returned in a stack format, which can be deserialized step by step using functions like `readNumber()` or others, depending on the expected data type.
...
:::info
A more complex example of traversing the transaction graph may be found [here](/v3/guidelines/dapps/asset-processing/payments-processing/#retrieve-incomingoutgoing-transactions).
:::
