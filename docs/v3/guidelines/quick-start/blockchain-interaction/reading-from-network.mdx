...
```bash
npx ts-node 2-call-get-methods.ts
```
...
3. Insert address from example *0QD-SuoCHsCL2pIZfE8IAKsjc0aDpDUQAoo-ALHl2mje04A-*.
4. Press **Execute**.
...
Wrappers are classes that simplify interactions with smart contracts by turning complex blockchain operations into simple functions calls - instead of manually encoding cells and transactions, you just call methods like `jettonMaster.getWalletAddress()`. Here's an equivalent example of using a wrapper:
