...
- `state`: Can be one of:
  ...
  - **Frozen**: The address is locked when storage fees exceed the balance. The balance is zero; unpaid storage fees are tracked as debt (see `StorageInfo.due_payment` / transaction `storage_fees_due`).
