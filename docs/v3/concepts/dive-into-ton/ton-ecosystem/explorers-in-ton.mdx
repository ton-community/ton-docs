...
An advanced analytics platform focused on tokens, trader behavior, and on-chain activity. It offers detailed charts for token prices, liquidity, holders, market cap, and historical data.

- Mainnet: [dyor.io](https://dyor.io)

#### Features

- Real-time charts for price, liquidity, holders, and market data
- Advanced filters to track DEX swaps by wallet, volume, token, or timeframe
- Visual token explorer with trust scores and trend indicators
- Built-in token swap via DeDust Router
- Telegram alerts through mini app
