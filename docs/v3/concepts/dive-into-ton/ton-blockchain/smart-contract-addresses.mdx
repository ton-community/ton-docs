...
- `frozen`: occurs when storage fees exceed the balance. The account balance is zero; any unpaid storage fees are tracked as debt (see `StorageInfo.due_payment` / transaction `storage_fees_due`). The address cannot execute transactions until it is unfrozen by replenishing its balance and providing its previous state.
