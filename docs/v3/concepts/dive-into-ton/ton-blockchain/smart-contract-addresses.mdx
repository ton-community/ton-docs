...
These formats are not interchangeable. The user-friendly format includes critical metadata, such as the _bounceable flag_, which determines message-handling behavior for uninitialized accounts. A bounceable message sent to an address in `nonexist`, `uninit` or `frozen` state is returned to the sender. In contrast, a non-bounceable message will be credited to the account.
