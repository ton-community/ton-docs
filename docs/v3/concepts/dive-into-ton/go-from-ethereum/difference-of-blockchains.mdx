...
We can represent the sequence of transactions as a chain. In this representation, each smart contract has its own transactions, which means that each contract located in dedicated blockchain, so the network can process the transactions independently.
